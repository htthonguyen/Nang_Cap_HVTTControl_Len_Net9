...
{
	/// <summary>
	/// Summary description for HVTTStreamer.
	/// </summary>
	[Serializable()]
	public sealed class HVTTStreamer:IDisposable,ISerializable
	{
		private XmlDocument m_XmlDoc=null;
		private HVTTManager m_Owner=null;

		private HVTTStreamer(){}
		private HVTTStreamer(SerializationInfo info, StreamingContext c)
		{
			SerializationInfoEnumerator en=info.GetEnumerator();
			while(en.MoveNext())
			{
				//System.Windows.Forms.MessageBox.Show("Read stream: "+en.Name);
				if(en.Name=="HVTTCONTROLSdata")
				{
		...
		public HVTTStreamer(HVTTManager owner)
		{
			m_Owner=owner;
		}
		...
		void ISerializable.GetObjectData (SerializationInfo info, StreamingContext context)
		{
			...
			if(m_Owner!=null)
			{
				...
				info.AddValue("HVTTCONTROLSdata",xml.OuterXml);
			}
			else
				info.AddValue("HVTTCONTROLSdata",m_XmlDoc.OuterXml);
		}
