...
namespace HVTT.UI.Window.Forms
{
    ...
    public class MenuPanel : Control, IDesignTimeProvider, IAccessibilitySupport, IKeyTipsControl, IKeyTipsRenderer
    {
		...
		private HVTTPersonalizedMenus m_PersonalizedMenus=HVTTPersonalizedMenus.Disabled;
        ...
        private Rendering.BaseRenderer GetRenderer()
        {
            ...
            if (m_ParentItem != null && m_ParentItem.ContainerControl is IRenderingSupport)
            {
                Rendering.BaseRenderer renderer = ((IRenderingSupport)m_ParentItem.ContainerControl).GetRenderer();
                if (renderer != null)
                    r = renderer;
            }
		...
		[DefaultValue(true), Browsable(true), HVTTBrowsable(true)]
		...
		private void SetupAccessibility()
		{
			if(m_ParentItem!=null && m_ParentItem.Text!="")
				...
				this.AccessibleName="HVTTCONTROLS Popup Menu";
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			if(m_ParentItem!=null)
			{
				...
				if(m_ParentItem.Style==HVTTControlStyle.Office2000)
					PaintOffice(pa);
				else
					PaintDotNet(pa);
        ...
        private InternalMenuColors GetMenuColors(ItemPaintArgs pa)
        {
            ...
            if (m_ParentItem != null && m_ParentItem.Style == HVTTControlStyle.Office2007 && pa.Renderer is Rendering.Office2007Renderer)
            {
        ...
        protected void PaintDotNet(ItemPaintArgs pa)
		{
            ...
            if (m_ParentItem != null)
            {
                ...
                if (BarFunctions.IsOffice2007Style(m_ParentItem.Style))
                {
                    if (this.Region != null)
                    {
                ...
                if (m_ParentItem.Style != HVTTControlStyle.Office2007 && m_ParentItem is ButtonItem && m_ParentItem.Displayed && m_ParentItem.Visible && m_ParentItem.Orientation == HVTTOrientation.Horizontal && !(m_ParentItem.ContainerControl is ContextMenuBar))
                {
					...
					for(int i=m_ScrollTopPosition;i<m_ParentItem.SubItems.Count;i++)
					{
						...
						if(objTmp.Visible || m_IsCustomizeMenu)
						{
							if(objTmp.Displayed)
							{
								...
								{
									if(objTmp.Style==HVTTControlStyle.Office2000)
									{
									...
									else
									{
										if(m_IsCustomizeMenu)
										{
                                            ...
                                            if (BarFunctions.IsOffice2007Style(objTmp.Style))
                                                PaintMenuItemSide(pa, sideRect, menuColors);
										...
										else
										{
                                            ...
                                            if(paintSide)
                                            {
                                                ...
                                                sideRect.Inflate(0, 1);
											    // Paint side bar
                                                if (objTmp is IPersonalizedMenuItem && ((IPersonalizedMenuItem)objTmp).MenuVisibility == HVTTMenuVisibility.VisibleIfRecentlyUsed && !((IPersonalizedMenuItem)objTmp).RecentlyUsed)
							...
							if(objTmp.BeginGroup && !bFirst)
							{
								...
								else
								{
									...
									else
									{
                                        ...
                                        {
                                            DisplayHelp.DrawLine(g, objTmp.LeftInternal + (itemOnlySeparator ? (iSideBarWidth + 8 + 7) : 0), objTmp.TopInternal - 2, objTmp.DisplayRectangle.Right - 1, objTmp.TopInternal - 2, menuColors.SideBorder.Start, 1);
                                            if (BarFunctions.IsOffice2007Style(objTmp.Style) && !pa.Colors.ItemSeparatorShade.IsEmpty)
                                                DisplayHelp.DrawLine(g, objTmp.LeftInternal + (itemOnlySeparator ? (iSideBarWidth + 8 + 7) : 0), objTmp.TopInternal - 1, objTmp.DisplayRectangle.Right - 1, objTmp.TopInternal - 1, menuColors.SideBorderLight.Start, 1);
                                        }
									}
								}
							}
        ...
        {
            if (pa.Owner is HVTTManager) // Do not paint side lines for HVTTManager owned items...
                return;
            Graphics g = pa.Graphics;
            if (pa.RightToLeft)
            {
        ...
        private bool IsGradientStyle
        {
            get
            {
                if (m_ParentItem != null)
                {
                    return m_ParentItem.Style == HVTTControlStyle.OfficeXP || m_ParentItem.Style == HVTTControlStyle.Office2003 || m_ParentItem.Style == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(m_ParentItem.Style);
                }
                return true;
            }
        }
		...
		private void PaintExpandButton(ItemPaintArgs pa, InternalMenuColors menuColors)
		{
            ...
            if (IsGradientStyle)
			{
                ...
                if (m_ParentItem.Style == HVTTControlStyle.Office2003 || m_ParentItem.Style == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(m_ParentItem.Style))
				{
			...
			else if(m_ParentItem.Style==HVTTControlStyle.Office2000)
			{
				if(m_ExpandButton.MouseOver)
				{	
		...
		public void RecalcSize()
		{
            ...
            if (this.Visible)
            {
                if (m_ParentItem != null && BarFunctions.IsOffice2007Style(m_ParentItem.Style) && IsRoundRegion)
                {
                    if(this.Parent!=null)
		...
		private void RecalcSizeDotNet()
		{
			...
			foreach(BaseItem objItem in m_ParentItem.SubItems)
			{
				...
				{
					if(!m_ParentItem.DesignMode && m_PersonalizedMenus!=HVTTPersonalizedMenus.Disabled && !m_ExpandButton.PersonalizedAllVisible && !m_IsCustomizeMenu)
					{
						IPersonalizedMenuItem ipm=objItem as IPersonalizedMenuItem;
						if(ipm!=null && ipm.MenuVisibility==HVTTMenuVisibility.VisibleIfRecentlyUsed && !ipm.RecentlyUsed)
						{
                ...
                if (site != null && item.ContainerControl!=null)
                {
                    ...
                    if (dh != null)
                    {
                        ...
                        if (c is RibbonStrip && c.Site == null && c.Parent is RibbonControl)
                            c = c.Parent;
                        Design.BarBaseControlDesigner designer = dh.GetDesigner(c) as Design.BarBaseControlDesigner;
                        if (designer != null)
                        {
                            ((Design.BarBaseControlDesigner)designer).StartExternalDrag(this.FocusedItem());
                            return;
                        }
                    }
                }
		...
		internal void InternalMouseHover()
		{
			...
			{
				if(m_PersonalizedMenus!=HVTTPersonalizedMenus.DisplayOnClick)
				{
		...
		protected override void OnMouseUp(MouseEventArgs e)
		{
            ...
            else
            {
                ...
                {
                    if (m_PersonalizedMenus != HVTTPersonalizedMenus.DisplayOnHover && m_ExpandButton.Rect.Contains(e.X, e.Y))
                    {
                        ExpandRecentlyUsed();
                    }
                    else
		...
		{
			if(m_PersonalizedMenus!=HVTTPersonalizedMenus.DisplayOnHover)
			{
				m_ExpandButton.PersonalizedAllVisible=true;
        ...
        {
            BaseItem objItem = GetItemForMnemonic(container, key, true);
            if (objItem!=null && objItem.GetEnabled() && objItem.Visible)
            {
                if (objItem.SubItems.Count > 0 && objItem.ShowSubItems && !objItem.Expanded)
                {
                    if (m_HotSubItem != objItem)
                    {
                        if (m_HotSubItem != null)
                            m_HotSubItem.InternalMouseLeave();
                    ...
                    if (objItem is PopupItem && ((PopupItem)objItem).PopupControl is MenuPanel)
                    {
                        ((MenuPanel)((PopupItem)objItem).PopupControl).SelectFirstItem();
                    }
                    m_IgnoreDuplicateMouseMove = true;
                }
                else
        ...
        private bool IsRoundRegion
        {
            get
            {
                if (m_ParentItem != null)
                {
                    ...
                    if (c is RibbonStrip || c is RibbonBar || c is ContextMenuBar && ((ContextMenuBar)c).Style == HVTTControlStyle.Office2007)
                        return true;
                }
                return false;
            }
		...
		public new void Show()
		{
            ...
            if (BarFunctions.IsOffice2007Style(m_ParentItem.Style) && IsRoundRegion)
            {
                SetRoundRegion(this);
		...
		internal bool DisplayShadow
		{
			get
			{
				...
				if(ownermenu!=null)
				{
					...
					{
						if(ownermenu.MenuDropShadow==HVTTMenuDropShadow.Show)
							return true;
						else
							return false;
					}
...
//		}
		...
		public HVTTPersonalizedMenus PersonalizedMenus
		{
			get
			{
		...
		/// <summary>
		/// Gets or sets the HotSubItem on the menu. This method is used internally by HVTTCONTROLS and should not be used in your application.
		/// </summary>
		public BaseItem HotSubItem
		{
			...
			set
			{
				if(m_HotSubItem!=null)
					m_HotSubItem.InternalMouseLeave();
				m_HotSubItem=null;
				if(this.ParentItem.SubItems.Contains(value))
				{
					m_HotSubItem=value;
					if(m_HotSubItem!=null)
					{
		...
		public InsertPosition GetInsertPosition(Point pScreen, BaseItem DragItem)
		{
            ...
            //            //if(objItem.Orientation==HVTTOrientation.Horizontal)
            //            //{
