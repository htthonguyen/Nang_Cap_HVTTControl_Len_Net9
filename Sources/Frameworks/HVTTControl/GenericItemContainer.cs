namespace HVTT.UI.Window.Forms
{
    ...
    public class GenericItemContainer:ImageItem, IDesignTimeProvider
    {
		...
		private HVTTLayoutType m_LayoutType=HVTTLayoutType.Toolbar;
		...
		public override void Paint(ItemPaintArgs pa)
		{
			...
			try
			{
				...
				//else if(m_Style==HVTTControlStyle.Office2000 || this.IsOnMenuBar)
				...
				if(m_SubItems!=null)
				{
					...
					for(int iCurrentItem=0;iCurrentItem<m_SubItems.Count;iCurrentItem++)
					{
						...
						{
							if(objItem.BeginGroup && iDisplayed>0 && m_LayoutType!=HVTTLayoutType.DockContainer && !(objItem is ItemContainer && BarFunctions.IsOffice2007Style(objItem.Style)))
							{
								if(m_Orientation==HVTTOrientation.Vertical || m_LayoutType==HVTTLayoutType.TaskList)
								{
									...
									else
									{
                                        ...
                                        if (this.LayoutType == HVTTLayoutType.TaskList)
                                        {
						...
						//					if(m_LayoutType==HVTTLayoutType.TaskList && m_Scroll)
						//					{
						//
						//					}
					}
		...
		public override void RecalcSize()
		{
			...
			switch(m_LayoutType)
			{
				case HVTTLayoutType.Toolbar:
                    RecalcSizeToolbar();
					break;
				case HVTTLayoutType.TaskList:
					RecalcSizeTaskList();
					break;
				case HVTTLayoutType.DockContainer:
		...
		private void RecalcSizeDockContainer(bool bFitContainer)
		{
			...
			if((bFitContainer || this.Stretch))
			{
				...
				if(bar!=null && (bar.BarState==HVTTBarState.Docked || bar.BarState==HVTTBarState.AutoHide))
				{
					if(MinWidth>0 && iMaxWidth+m_PaddingLeft+m_PaddingRight<MinWidth && this.Orientation==HVTTOrientation.Vertical)
						iMaxWidth=MinWidth-(m_PaddingLeft+m_PaddingRight);
					else if(MinHeight>0 && iMaxHeight+m_PaddingTop+m_PaddingBottom<MinHeight && this.Orientation==HVTTOrientation.Horizontal)
			...
			if(this.Stretch)
			{
				...
				else if(m_Orientation==HVTTOrientation.Vertical)
				{
					m_Rect.Width=m_PaddingLeft+iMaxWidth+m_PaddingRight;
		...
		private void RecalcSizeToolbar()
		{
			...
			if(m_SubItems!=null)
			{
				...
				do
				{
					...
					for (int iCurrentItem=0;iCurrentItem<m_SubItems.Count;iCurrentItem++)
					{
						...
						{
							iVisible++;
							if((objItem.SupportedOrientation==HVTTSupportedOrientation.Horizontal && m_Orientation==HVTTOrientation.Vertical) || (objItem.SupportedOrientation==HVTTSupportedOrientation.Vertical && m_Orientation==HVTTOrientation.Horizontal))
							{
							...
							{
								case HVTTOrientation.Horizontal:
								{
							...
							if(bDisplayed)
							{
								...
								{
									if(m_Orientation==HVTTOrientation.Vertical && iMaxItemWidth!=0 && !bSetSize)
										bAdjustSize=true;
									if(objItem.WidthInternal>iMaxItemWidth)
								...
								{
									if(m_Orientation==HVTTOrientation.Horizontal && iMaxItemHeight!=0 && !bSetSize)
										bAdjustSize=true;
									if(objItem.HeightInternal>iMaxItemHeight)
					...
					{
						if(m_Orientation==HVTTOrientation.Horizontal && (iWidth>m_Rect.Width || bAdjustSize))
						{
							bRepeat=true;
						...
						else if(m_Orientation==HVTTOrientation.Vertical && (iHeight>m_Rect.Height || bAdjustSize))
						{
							bRepeat=true;
            ...
            if (isRightToLeft && m_Orientation == HVTTOrientation.Horizontal)
                MirrorPositionItems();
		...
		private void RecalcSizeTaskList()
		{
			...
			if(m_EqualButtonSize)
			{
				foreach(BaseItem item in m_SubItems)
				{
					item.Orientation=HVTTOrientation.Horizontal;
					item.RecalcSize();
			...
			if(this.Stretch)
			{
				if(bDisplayed && FarAlignItems!=null && FarAlignItems.Count>0)
				{
					...
					AlignItemsFar(FarAlignItems, HVTTOrientation.Vertical); // Vertical becouse this is "vertical layout"
				}
				HVTTMarkStatus bar=this.ContainerControl as HVTTMarkStatus;
			...
			else
			{
				if(m_TopScrollButton==null)
				{
					...
					if(ctrl!=null)
					{
						...
						m_TopScrollButton.Orientation=HVTTOrientation.Vertical;
			...
			else
			{
				if(m_BottomScrollButton==null)
				{
					...
					if(ctrl!=null)
					{
						...
						m_BottomScrollButton.Orientation=HVTTOrientation.Vertical;
		...
		private void AlignItemsFar(Stack FarAlignItems, HVTTOrientation orientation)
		{
			int iPos=0;
			int iGroupLineSize=this.GroupLineSize;
			if(orientation==HVTTOrientation.Horizontal)
			{
		...
		private void AlignItemsFar(ArrayList FarAlignLines, HVTTOrientation orientation)
		{
			if(orientation==HVTTOrientation.Horizontal)
			{
		...
		public override void InternalMouseUp(System.Windows.Forms.MouseEventArgs objArg)
		{
			...
			if(m_HotSubItem==null && (this.IsOnMenuBar || this.LayoutType==HVTTLayoutType.Toolbar))
			{
		...
		public override void InternalMouseMove(System.Windows.Forms.MouseEventArgs objArg)
		{
			...
			if(m_MoreItems==null || !m_MoreItems.DisplayRectangle.Contains(objArg.X,objArg.Y) || this.DesignMode)
			{
                ...
                if (bMenuBar || this.LayoutType == HVTTLayoutType.Toolbar)
                {
                    BaseItem expanded = this.ExpandedItem();
                    if (expanded != null)
				...
				if(objArg.Button==System.Windows.Forms.MouseButtons.None || this.DesignMode || (bMenuBar || this.LayoutType==HVTTLayoutType.Toolbar))
		...
		public override void InternalKeyDown(System.Windows.Forms.KeyEventArgs objArg)
		{
			...
			else if(objArg.KeyCode==System.Windows.Forms.Keys.Escape)
			{
				...
				else
				{
					...
					if(cc is HVTTMarkStatus)
					{
                        HVTTMarkStatus bar = cc as HVTTMarkStatus;
		...
		private bool AccessKeyDown(int KeyCode)
		{
			...
			foreach(BaseItem objItem in this.SubItems)
			{
				...
				{
					if(objItem is ButtonItem && ((ButtonItem)objItem).ButtonStyle==HVTTButtonStyle.Default && !this.IsOnMenuBar && !this.IsOnMenu)
						continue;
...
//			return false;
		...
		private void ExpandItem(BaseItem objItem)
		{
			...
			//	bar.Focus();
            if (cc is HVTTMarkStatus && !cc.Focused)
                ((HVTTMarkStatus)cc).MenuFocus = true;
		...
		{
			base.OnOwnerChanged();
			if(this.GetOwner() is HVTTMarkStatus)
			{
		...
		/// <summary>
		/// Gets or sets whether container is system container used internally by HVTTCONTROLS.
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(false),System.ComponentModel.DesignerSerializationVisibility(System.ComponentModel.DesignerSerializationVisibility.Hidden),System.ComponentModel.EditorBrowsable(System.ComponentModel.EditorBrowsableState.Never)]
...
//							bar.ReleaseFocus();
		...
		public virtual HVTTLayoutType LayoutType
		{
			get
			{
				return m_LayoutType;
			...
			set
			{
				...
				m_LayoutType=value;
				NeedRecalcSize=true;
				if(m_LayoutType==HVTTLayoutType.TaskList)
				{
					base.Orientation=HVTTOrientation.Horizontal;
				}
				else
				{
					if(this.ContainerControl!=null && this.ContainerControl is HVTTMarkStatus)
					{
						HVTTMarkStatus bar=this.ContainerControl as HVTTMarkStatus;
		...
		public override HVTTOrientation Orientation
		{
			...
			{
				if(m_LayoutType==HVTTLayoutType.TaskList)
					return HVTTOrientation.Vertical;
