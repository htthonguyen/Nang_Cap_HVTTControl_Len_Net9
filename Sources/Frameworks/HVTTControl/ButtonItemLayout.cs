	...
	internal class ButtonItemLayout
	{
        ...
        public static void Arrange(ButtonItem button)
        {
            ...
            if (!button.SubItemsRect.IsEmpty)
            {
                ...
                if (button.ContainerControl is RibbonBar && (button.ImagePosition == ImagePosition.Top || button.ImagePosition == ImagePosition.Bottom))
                {
            ...
            if (!hasImage || button.ButtonStyle == HVTTButtonStyle.TextOnlyAlways)
            {
            ...
            else if (button.ButtonStyle == HVTTButtonStyle.Default && !(button.ImagePosition == ImagePosition.Top || button.ImagePosition == ImagePosition.Bottom) || bounds.Width < imageSize.Width + minTextSize)
            {
            ...
            if (button.ImagePosition == ImagePosition.Left && !rightToLeft || button.ImagePosition == ImagePosition.Right && rightToLeft)
            {
            ...
            else if (button.ImagePosition == ImagePosition.Right && !rightToLeft || button.ImagePosition == ImagePosition.Left && rightToLeft)
            {
            ...
            else if (button.ImagePosition == ImagePosition.Top)
            {
                int y = Math.Max(2, (bounds.Height - (button.TextDrawRect.Height + button.ImageDrawRect.Height - 2)) / 2);
                if (button.Name != "sysOverflowButton" && button.Parent is ItemContainer && ((ItemContainer)button.Parent).LayoutOrientation == HVTTOrientation.Horizontal && ((ItemContainer)button.Parent).VerticalItemAlignment == eVerticalItemsAlignment.Top)
                    y = 2;
            ...
            else if (button.ImagePosition == ImagePosition.Bottom)
            {
        ...
        {
            return MeasureItemText(item, g, containerWidth, font, stringFormat, rightToLeft, false, ImagePosition.Left);
        }

        public static Size MeasureItemText(BaseItem item, Graphics g, int containerWidth, Font font, eTextFormat stringFormat, bool rightToLeft, bool ribbonBarButton, ImagePosition imagePosition)
        {
            ...
            else
            {
                ...
                if (containerWidth != 0 && !(ribbonBarButton && imagePosition== ImagePosition.Top))
        ...
        private static bool UseRibbonWordBreak(ButtonItem button)
        {
            ...
            return (button.Text.IndexOf(' ') > 0 && !button.TextMarkupBody.HasExpandElement || button.Text.Split(' ').Length > 2 && button.TextMarkupBody.HasExpandElement);
        }
		...
		public static void LayoutButton(ButtonItem button, bool startButtonType)
		{
			...
			if(objCtrl==null || objCtrl.Disposing || objCtrl.IsDisposed) //if(!BarFunctions.IsHandleValid(objCtrl))
				return;
            if (objCtrl is HVTTButton && button._FitContainer)
            {
                LayoutHVTTButton(button);
                return;
            }
            ...
            if (button.GetImage() != null || startButtonType)
                bHasImage = true;

            ImagePosition imagePosition = button.ImagePosition;
            ...
            if (rightToLeft && button.Orientation == HVTTOrientation.Horizontal)
            {
            ...
            try
            {
                ...
                if (button._FitContainer && bHasImage && (imagePosition == ImagePosition.Left || imagePosition == ImagePosition.Right))
                {
                    measureStringWidth -= (imageSize.Width + 10);
                }
                else if (button.RibbonWordWrap && bHasImage && imagePosition == ImagePosition.Top && objCtrl is RibbonBar && UseRibbonWordBreak(button))
                {
                ...
                if ((button.Text != "" || button.TextMarkupBody!=null) && (!bHasImage || isOnMenu || button.ButtonStyle != HVTTButtonStyle.Default || button.ImagePosition != ImagePosition.Left && bHasImage))
                {
                    ...
                    {
                        measureStringWidth += imageSize.Width/2;
                        textSize = ButtonItemLayout.MeasureItemText(button, g, measureStringWidth, font, stringFormat, rightToLeft, ribbonBarButton, imagePosition);
                        maxItt++;
                    }
                }
                ...
                else
                {
                    ...
                    if (button.Orientation == HVTTOrientation.Horizontal && (imagePosition == ImagePosition.Left || imagePosition == ImagePosition.Right))
                    {
                        ...
                        if (button.ButtonStyle != HVTTButtonStyle.TextOnlyAlways && bHasImage)
                        {
                            // We know the image position now, we will center it into this area
                            imageDrawRect = new Rectangle(0, (bounds.Height - imageSize.Height) / 2, imageSize.Width, imageSize.Height);
                        }
                        ...
                        if (button.ButtonStyle != HVTTButtonStyle.Default || !bHasImage)
                        {
                        ...
                        if (imagePosition == ImagePosition.Right && imageDrawRect.Right > 0 && bHasImage)
                        {
                            textDrawRect.X = 3;
                    ...
                    else
                    {
                        ...
                        {
                            if (textSize.Height > imageSize.Width && button.ButtonStyle != HVTTButtonStyle.Default)
                            ...
                            if (imagePosition == ImagePosition.Top || imagePosition == ImagePosition.Left)
                            {
                                if (bHasImage)
                    ...
                    if (HasExpandPart(button))
                    {
                        ...
                        if (rightToLeft && !(objCtrl is RibbonBar &&
                            (button.ImagePosition == ImagePosition.Top || button.ImagePosition == ImagePosition.Bottom)))
                        {
                            if (!textDrawRect.IsEmpty)
                                textDrawRect.Offset(subItemsRect.Width + 1, 0);
                            if (!imageDrawRect.IsEmpty && (button.Orientation == HVTTOrientation.Horizontal && (imagePosition == ImagePosition.Left || imagePosition == ImagePosition.Right)))
        ...
        private static bool HasExpandPart(ButtonItem button)
        {
            return (button.SubItems.Count > 0 || button.PopupType == ePopupType.Container) && button.ShowSubItems && !button.IsOnMenuBar &&
                        !(button.TextMarkupBody != null && button.TextMarkupBody.HasExpandElement);
        }
		...
		public static Rectangle GetSubItemsButtonBounds(ButtonItem button, Rectangle buttonBounds, bool rightToLeft)
		{
			...
			if(button.ContainerControl is RibbonBar && (button.ImagePosition==ImagePosition.Top || button.ImagePosition==ImagePosition.Bottom))
			{
				subItemsRect=new Rectangle(0,buttonBounds.Height+1,buttonBounds.Width,button.SubItemsExpandWidth);
			}
			else
			{
				// Add small button to expand the item
        ...
        public static void LayoutHVTTButton(ButtonItem button)
        {
            Control objCtrl = button.ContainerControl as Control;
            HVTTButton btnX = button.ContainerControl as HVTTButton;
            ...
            try
            {
                ...
                if (bHasImage && (imagePosition == ImagePosition.Left || imagePosition == ImagePosition.Right))
                {
                ...
                if (btnX != null) stringFormat |= eTextFormat.WordBreak;
