...
namespace HVTT.UI.Window.Forms.Editors
{
    ...
    public class HVTTTreeGridNode : DataGridViewRow//, IComponent
    {
        internal HVTTTreeGridView _grid;
		internal HVTTTreeGridNode _parent;
        internal HVTTTreeGridNodeCollection _owner;
        ...
        HVTTTreeGridCell _treeCell;
        HVTTTreeGridNodeCollection childrenNodes;
        ...
        internal HVTTTreeGridNode(HVTTTreeGridView owner)
			: this()
		{
			this._grid = owner;
		...
		{
            HVTTTreeGridNode r = (HVTTTreeGridNode)base.Clone();
			r.UniqueValue = -1;
		...
		internal protected virtual void UnSited()
		{
            ...
            HVTTTreeGridCell cell;
			foreach (DataGridViewCell DGVcell in this.Cells)
			{
                cell = DGVcell as HVTTTreeGridCell;
				if (cell != null)
				{
					cell.UnSited();
				}
		...
		void cells_CollectionChanged(object sender, System.ComponentModel.CollectionChangeEventArgs e)
		{
			...
			{
                HVTTTreeGridCell treeCell = null;

				if (e.Element == null)
				{
					foreach (DataGridViewCell cell in base.Cells)
					{
                        if (cell.GetType().IsAssignableFrom(typeof(HVTTTreeGridCell)))
						{
                            if (cell != null)
                            {
                                treeCell = (HVTTTreeGridCell)cell;
                            }
							break;
						}
				...
				{
                    treeCell = e.Element as HVTTTreeGridCell;
				}
        ...
        public HVTTTreeGridNodeCollection Nodes
        {
            get
            {
                if (childrenNodes == null)
                {
                    childrenNodes = new HVTTTreeGridNodeCollection(this);
                }
		...
		public int Level
		{
			get {
				if (this._level == -1)
				{
					// calculate level
					int walk = 0;
                    HVTTTreeGridNode walkRow = this.Parent;
					while (walkRow != null)
					{
						walk++;
		...
		[Browsable(false),
		 DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public HVTTTreeGridNode Parent
		{
			get
			{
				return this._parent;
		...
		public bool IsLastSibling
		{
			get
			{
                HVTTTreeGridNode parent = this.Parent;
        ...
        internal protected virtual bool InsertChildNode(int index, HVTTTreeGridNode node)
		{
			node._parent = this;
        ...
        internal protected virtual bool InsertChildNodes(int index, params HVTTTreeGridNode[] nodes)
		{
            foreach (HVTTTreeGridNode node in nodes)
			{
        ...
        internal protected virtual bool AddChildNode(HVTTTreeGridNode node)
		{
			node._parent = this;
			node._grid = this._grid;
        ...
        internal protected virtual bool AddChildNodes(params HVTTTreeGridNode[] nodes)
		{
        ...
        internal protected virtual bool RemoveChildNode(HVTTTreeGridNode node)
		{
        ...
        private void UpdateChildNodes(HVTTTreeGridNode node)
        {
            if (node.HasChildren)
            {
                foreach (HVTTTreeGridNode childNode in node.Nodes)
                {
