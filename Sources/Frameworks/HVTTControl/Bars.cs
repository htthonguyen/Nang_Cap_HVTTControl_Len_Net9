...
namespace HVTT.UI.Window.Forms
{
	...
	{
		private HVTTManager m_Owner;
		internal Bars(HVTTManager Owner)
		{
			m_Owner=Owner;
		}

		internal HVTTManager Owner
		{
			get
			{
				return m_Owner;
		...
		public void Add(HVTTMarkStatus bar)
		{
		...
		protected override void OnInsertComplete(int index,object value)
		{
			...
			HVTTMarkStatus bar=(HVTTMarkStatus)value;
			bar.Owner=m_Owner;
		...
		/// </summary>
		public HVTTMarkStatus this[int Index]
		{
			get
			{
				return List[Index] as HVTTMarkStatus;
			}
		}
		...
		/// </summary>
		public HVTTMarkStatus this[string Name]
		{
			get
			{
				foreach(HVTTMarkStatus bar in List)
					if(bar.Name==Name)
						return bar;
				return null;
			}
		...
		public virtual void Remove(HVTTMarkStatus bar) 
		{
			List.Remove(bar);
		}
		...
		public bool Contains(HVTTMarkStatus bar)
		{
		...
		{
			foreach(HVTTMarkStatus bar in this.List)
				if(bar.Name==name)
					return true;
			return false;
		}
		...
		/// <returns></returns>
		public int IndexOf(HVTTMarkStatus bar)
		{
		...
		internal void ClearNonDocumentBars()
		{
			...
			foreach(HVTTMarkStatus bar in removeList)
				this.Remove(bar);
		}
	}
}
