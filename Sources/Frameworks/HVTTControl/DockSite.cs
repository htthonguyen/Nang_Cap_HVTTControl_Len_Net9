namespace HVTT.UI.Window.Forms
{
	...
	public interface IDockInfo
	{
		...
		/// </summary>
		System.Drawing.Size MinimumDockSize(HVTTOrientation dockOrientation);
		/// <summary>
		/// Returns Preferrred size of the docked control.
		/// </summary>
		System.Drawing.Size PreferredDockSize(HVTTOrientation dockOrientation);
		/// <summary>
		...
		/// </summary>
		HVTTOrientation DockOrientation {get;set;}
		/// <summary>
	...
	public struct DockSiteInfo
	{
		...
		public HVTTMarkStatus TabDockContainer;
		/// <summary>
		...
		/// </summary>
		public HVTTMarkStatus MouseOverBar;
		/// <summary>
    ...
    /// <summary>
    ///    Dock Sites are created by HVTTCONTROLS control on each edge of the
    ///    HVTTCONTROLS container control and are used for docking purposes.
    ///    If Dock Site does not contain any controls it will be invisible.
    /// </summary>
	[ToolboxItem(false),DesignTimeVisible(false),Designer(typeof(Design.DockSiteDesigner))]
	public class DockSite : System.Windows.Forms.Control
	{
		private HVTTOrientation m_DockOrientation;
		private bool m_Layingout=false;
		private bool m_OversizeLayout=false;
		//private int m_DockMargin;
		private HVTTManager m_Owner=null;
        ...
        internal bool _DesignerLoading = false;
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Appearance"),DefaultValue((byte)255),Description("Specifies the transparency of background image.")]
...
//		/// </summary>
//		[DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),Browsable(false),HVTTBrowsable(false)]
...
//		}
		...
		protected override void OnPaintBackground(PaintEventArgs pevent)
		{
			...
			if(this.Controls.Count>0 && this.Controls[0] is HVTTMarkStatus && ((HVTTMarkStatus)this.Controls[0]).ThemedBackground)
			{
				ThemeRebar theme=this.ThemeRebar;
				theme.DrawBackground(pevent.Graphics,ThemeRebarParts.Background,ThemeRHVTTBarStates.Normal,this.ClientRectangle);
                return;
			}
        ...
        private ColorScheme GetColorScheme()
        {
            ...
            {
                if (m_Owner.Style == HVTTControlStyle.Office2007 && GlobalManager.Renderer is Office2007Renderer)
                {
                    return ((Office2007Renderer)GlobalManager.Renderer).ColorTable.LegacyColors;
                }
                return m_Owner.ColorScheme;
            }
            else if (this.Controls.Count > 0 && this.Controls[0] is HVTTMarkStatus && (((HVTTMarkStatus)this.Controls[0]).Style == HVTTControlStyle.Office2003 || ((HVTTMarkStatus)this.Controls[0]).Style == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(((HVTTMarkStatus)this.Controls[0]).Style)) )
            {
                HVTTMarkStatus bar = this.Controls[0] as HVTTMarkStatus;
		...
		private HVTT.UI.Window.Forms.ThemeRebar ThemeRebar
		{
			get
			{
			...
			set
			{
                ...
                if(value==System.Windows.Forms.DockStyle.Top || value==System.Windows.Forms.DockStyle.Bottom || value==DockStyle.Fill)
				{
					if(this.Controls.Count==0)
						this.Height=0;
					m_DockOrientation=HVTTOrientation.Horizontal;
				}
				else
				{
					if(this.Controls.Count==0)
						this.Width=0;
					m_DockOrientation=HVTTOrientation.Vertical;
				}
			}
		}
		...
		protected override void OnControlAdded(ControlEventArgs e)
		{
			...
			if(pDockInfo==null)
			{
                ...
                throw new System.ArgumentException("Only Bar objects can be added to DockSite through HVTTCONTROLS designer.");
			}
        ...
        internal bool IsAnyControlVisible
        {
            get
            {
                ...
                {
                    if (c is HVTTMarkStatus)
                    {
                        if(((HVTTMarkStatus)c).IsVisible) return true;
                    }
		...
		internal void AddBar(Control objBar)
		{
			...
			if(objBar is HVTTMarkStatus && ((HVTTMarkStatus)objBar).LayoutType==HVTTLayoutType.DockContainer)
			{
				HVTTMarkStatus bar=objBar as HVTTMarkStatus;
				if(m_DockOrientation==HVTTOrientation.Horizontal && bar.ItemsContainer.MinHeight>0)
					((HVTTMarkStatus)objBar).SyncLineMinHeight();
				else if(m_DockOrientation==HVTTOrientation.Vertical && bar.ItemsContainer.MinWidth>0)
					((HVTTMarkStatus)objBar).SyncLineMinWidth();
			}
			NormalizeBars();
			LayoutBars();
		}
		...
		private void NormalizeBars()
		{
			...
			for(int i=0;i<this.Controls.Count;i++)
			{
				HVTTMarkStatus bar=this.Controls[i] as HVTTMarkStatus;
		...
		private void LayoutBars()
		{
			...
			for(int i=0;i<this.Controls.Count;i++)
			{
				objBar=this.Controls[i] as HVTTMarkStatus;
				if(objBar!=null && !objBar.Visible)
					continue;
				if(objBar!=null && objBar.LayoutType==HVTTLayoutType.DockContainer && objBar.Stretch)
				{
					if(objBar.DockLine>=0)
					{
						lineDockCounts[objBar.DockLine]++;
						lineDockProcessed[objBar.DockLine]++;
						if(m_DockOrientation==HVTTOrientation.Horizontal)
						{
            ...
            if (this.Controls.Count > 0 && this.Controls[0] is HVTTMarkStatus && ((HVTTMarkStatus)this.Controls[0]).LayoutType == HVTTLayoutType.Toolbar && (((HVTTMarkStatus)this.Controls[0]).Style == HVTTControlStyle.Office2003 || ((HVTTMarkStatus)this.Controls[0]).Style == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(((HVTTMarkStatus)this.Controls[0]).Style)))
			{
				iHorSpacing=2;
				iVerSpacing=1;
			}
			...
			do
			{
				...
				for(int i=0;i<this.Controls.Count;i++)
				{
					...
					objBar=objCtrl as HVTTMarkStatus;
					...
					if(m_DockOrientation==HVTTOrientation.Horizontal)
					{
                        ...
                        if(objBar.LayoutType == HVTTLayoutType.Toolbar)
						...
						{
							if(objBar.Stretch && objBar.LayoutType==HVTTLayoutType.DockContainer && objBar.DockLine>=0 && lineDockCounts[objBar.DockLine]>0)
							{
...
//											objBar.Size=new Size(this.Width-lineDockSize[objBar.DockLine],1);
						...
						if(pDockInfo.Stretch)
						{
							if(objBar.LayoutType==HVTTLayoutType.DockContainer && objBar.DockLine>=0 && lineDockProcessed[objBar.DockLine]>0)
							{
			...
			foreach(HVTTMarkStatus bar in arrBarsInSameLine)
			{
				if(m_DockOrientation==HVTTOrientation.Horizontal)
				{
...
//		}
		...
		private DockSiteInfo GetDockSiteInfoH(IDockInfo pDock, int x, int y)
		{
			...
			for(int i=0;i<this.Controls.Count;i++)
			{
				...
				if(!objCtrl.Visible)
					continue;
				IDockInfo pCtrlDockInfo=objCtrl as IDockInfo;
				if(pCtrlDockInfo.LockDockPosition && objCtrlToDock is HVTTMarkStatus && !((HVTTMarkStatus)objCtrl).CanUndock)
				{
				...
				if(pInsertionPoint.Y>objCtrl.Top+4 && ((pInsertionPoint.Y<objCtrl.Top+objDockSize.Height-4 && objDockSize.Height<=objCtrl.Height) || pInsertionPoint.Y<objCtrl.Bottom-4 && objDockSize.Height>objCtrl.Height))
				{
					...
					{
						if(objCtrl is HVTTMarkStatus && objCtrlToDock is HVTTMarkStatus && ((HVTTMarkStatus)objCtrl).LayoutType==HVTTLayoutType.DockContainer && ((HVTTMarkStatus)objCtrlToDock).LayoutType==HVTTLayoutType.DockContainer && ((HVTTMarkStatus)objCtrl).CanTearOffTabs)
						{
							if(!objCtrl.Bounds.Contains(pInsertionPoint))
								continue;
							iDockLine=pCtrlDockInfo.DockLine;
							if(((HVTTMarkStatus)objCtrl).IsPositionOnDockTab(x,y))
							{
								...
								pInfo.TabDockContainer=objCtrl as HVTTMarkStatus;
								return pInfo;
							}
						}
						else
					...
					{
						if(objCtrl!=objCtrlToDock && objCtrlToDock is HVTTMarkStatus && ((HVTTMarkStatus)objCtrlToDock).Stretch)
						{
		...
		internal Rectangle GetBarDockRectangle(HVTTMarkStatus bar, ref DockSiteInfo pDockInfo)
		{
			...
			else if(pDockInfo.DockLine==999 || pDockInfo.DockLine==-1) // Handle the edge cases
			{
				...
				switch(this.Dock)
				{
					case DockStyle.Top:
					{
						...
						r.Height=bar.GetBarDockedSize(HVTTOrientation.Horizontal);
					...
					case DockStyle.Right:
					{
						...
						r.Width=bar.GetBarDockedSize(HVTTOrientation.Vertical);
						Point p=Point.Empty;
                        if (pDockInfo.DockLine == -1)
						{
			...
			{
				HVTTMarkStatus barInsertPos=null;
				if(pDockInfo.InsertPosition>=0 && pDockInfo.InsertPosition<this.Controls.Count)
				{
					barInsertPos=this.Controls[pDockInfo.InsertPosition] as HVTTMarkStatus;
					if(!pDockInfo.NewLine)
					{
						...
						if(barInsertPos.DockLine>pDockInfo.DockLine)
						{
							...
							while(ipos>=0)
							{
								HVTTMarkStatus b=this.Controls[ipos] as HVTTMarkStatus;
								if(b!=null && b.LayoutType==HVTTLayoutType.DockContainer && b.DockLine==pDockInfo.DockLine)
								{
									barInsertPos=b;
									break;
								}
					...
					barInsertPos=this.Controls[0] as HVTTMarkStatus;
				else if(pDockInfo.InsertPosition>=this.Controls.Count)
					barInsertPos=this.Controls[this.Controls.Count-1] as HVTTMarkStatus;

				int i=pDockInfo.InsertPosition+1;
				while(i<this.Controls.Count && (bar==null || barInsertPos.LayoutType!=HVTTLayoutType.DockContainer || !barInsertPos.Visible))
				{
					barInsertPos=this.Controls[i] as HVTTMarkStatus;
					i++;
				}
		...
		internal HVTTOrientation DockOrientation
		{
		...
		private void UpdateBarsCollection()
		{
			...
			foreach(Control c in this.Controls)
			{
				if(c is HVTTMarkStatus)
				{
					HVTTMarkStatus bar=c as HVTTMarkStatus;
		...
		internal void SetOwner(HVTTManager owner)
		{
			m_Owner=owner;
		...
		/// <summary>
		/// Gets the reference to the HVTTManager that uses this dock site.
		/// </summary>
		[Browsable(false),HVTTBrowsable(false)]
		public HVTTManager Owner
		{
			get {return m_Owner;}
		}
