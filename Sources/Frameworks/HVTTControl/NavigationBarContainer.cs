	...
	public class NavigationBarContainer:ImageItem
	{
		...
		public override void RecalcSize()
		{
			...
			foreach(BaseItem item in this.SubItems)
			{
				...
				{
					((ButtonItem)item).ButtonStyle=HVTTButtonStyle.ImageAndText;
				...
				if(item.Visible)
				{
					...
					else
					{
						if(m_SummaryLineVisible)
						{
							...
							if(item is ButtonItem)
							{
								((ButtonItem)item).ButtonStyle=HVTTButtonStyle.Default;
								if(m_AutoSizeButtonImage && !m_ImageSizeSummaryLine.IsEmpty)
									((ButtonItem)item).ImageFixedSize=m_ImageSizeSummaryLine;
		...
		private void PaintLineBackground(ItemPaintArgs pa,Rectangle r, HVTTControlStyle style)
		{
            if (style == HVTTControlStyle.Office2007 && pa.Renderer != null)
            {
		...
		public override void Paint(ItemPaintArgs pa)
		{
			...
			if(m_ConfigureItem!=null)
			{
                ...
                PaintLineBackground(pa,rBottomLine, this.Style);
			}
			...
			for(int i=0;i<this.SubItems.Count;i++)
			{
				...
				if(item.Visible && item.Displayed)
				{
                    ...
                    {
                        if (this.Style == HVTTControlStyle.Office2007 && pa.Renderer != null)
		...
		private void CreateConfigureButton(int y, int height)
		{
			...
			if(m_ConfigureItem==null)
			{
				...
				m_ConfigureItem.Image=new System.Drawing.Bitmap(typeof(HVTT.UI.Window.Forms.HVTTManager),"SystemImages.NavBarConfigure.png");
				...
				if(m_ConfigureShowHideVisible)
				{
					...
					item.Image=new System.Drawing.Bitmap(typeof(HVTT.UI.Window.Forms.HVTTManager),"SystemImages.NavBarShowMore.png");
					...
					item.Image=new System.Drawing.Bitmap(typeof(HVTT.UI.Window.Forms.HVTTManager),"SystemImages.NavBarShowLess.png");
				...
				m_ConfigureItem.PopupSide=PopupSide.Right;
		...
		{
            if (this.ContainerControl is NavigationBar && this.Style == HVTTControlStyle.Office2007)
                ((MenuPanel)m_ConfigureItem.PopupControl).ColorScheme = ((NavigationBar)this.ContainerControl).GetColorScheme();
            else
			    ((MenuPanel)m_ConfigureItem.PopupControl).ColorScheme=new ColorScheme(this.Style);
		}
		#endregion
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Behavior"),Description("Indicates Configure Buttons button is visible."),System.ComponentModel.DefaultValue(true)]
		...
		public bool ConfigureShowHideVisible
		{
			...
			set
			{
				if(m_ConfigureShowHideVisible!=value)
				{
					...
					if(!m_ConfigureShowHideVisible && m_ConfigureItem!=null)
						m_ConfigureItem=null;
				}
			}
		}
		...
		public bool ConfigureNavOptionsVisible
		{
			...
			set
			{
				if(m_ConfigureNavOptionsVisible!=value)
				{
					...
					if(!m_ConfigureNavOptionsVisible && m_ConfigureItem!=null)
						m_ConfigureItem=null;
					NeedRecalcSize=true;
				}
			}
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Layout"),Description("Indicates the padding in pixels at the top portion of the item."),DefaultValue(2)]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Layout"),Description("Indicates the padding in pixels at the bottom of the item."),DefaultValue(2)]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Appearance"),DefaultValue(true),Description("Indicates whether summary line is visible.")]
