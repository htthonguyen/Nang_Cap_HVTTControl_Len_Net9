...
namespace HVTT.UI.Window.Forms
{
	...
	/// </summary>
	[ToolboxItem(true),System.Runtime.InteropServices.ComVisible(false),Designer(typeof(SideBarDesigner))]
        ...
        ICustomSerialization
	{
		...
		/// </summary>
		[System.ComponentModel.Description("Occurs after HVTTCONTROLS definition is loaded.")]
		...
		private SideBarColorScheme m_PredefinedColorScheme=SideBarColorScheme.Blue;
...
//				EventMouseMove(this,e);
		...
		protected override void OnPaint(PaintEventArgs e)
		{
            ...
            ItemPaintArgs pa = new ItemPaintArgs(this as IOwner, this, e.Graphics, m_ColorScheme);
			...
			else if(this.BackColor==SystemColors.Control && this.IsThemed)
			{
				...
				theme.DrawBackground(e.Graphics,ThemeRebarParts.Background,ThemeRHVTTBarStates.Normal,r);
			}
			else
			{
		...
		/// </summary>
		[Browsable(true),DefaultValue(SideBarAppearance.Traditional),Category("Appearance"),Description("Indicates visual appearance for the control.")]
		public SideBarAppearance Appearance
		{
			...
			set
			{
				if(m_Appearance!=value)
				{
					...
					if(m_Appearance==SideBarAppearance.Flat && this.Panels.Count>0 && this.Panels[0] is SideBarPanelItem)
					{
						if(!((SideBarPanelItem)this.Panels[0]).BackgroundStyle.Custom)
						{
							this.PredefinedColorScheme=SideBarColorScheme.SystemColors;
							if(this.DesignMode)
                                ApplyPredefinedColorScheme(SideBarColorScheme.SystemColors);	
						}
					}
					this.Refresh();
				}
		...
		public SideBarColorScheme PredefinedColorScheme
		{
			get {return m_PredefinedColorScheme;}
			set
			{
				m_PredefinedColorScheme=value;
				if(m_PredefinedColorScheme==SideBarColorScheme.SystemColors)
		...
		internal void ApplyPredefinedColorScheme(SideBarColorScheme scheme)
		{
			if(m_ItemContainer==null)
				return;
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Appearance"),System.ComponentModel.Description("Specifies the visual style of the side bar."),DefaultValue(HVTTControlStyle.OfficeXP)]
		...
		internal void ResetTraditional()
		{
			m_Appearance=SideBarAppearance.Traditional;
		...
		protected bool IsThemed
		{
			get
			{
                if (m_ThemeAware && (m_ItemContainer.Style == HVTTControlStyle.OfficeXP || m_ItemContainer.Style == HVTTControlStyle.Office2003 || m_ItemContainer.Style == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(m_ItemContainer.Style)) && BarFunctions.ThemedOS && Themes.ThemesActive)
					return true;
				return false;
			}
		}
		...
		protected override void OnSystemColorsChanged(EventArgs e)
		{
			...
			if(m_UsingSystemColors && this.Appearance!=SideBarAppearance.Traditional)
			{
				this.PredefinedColorScheme=SideBarColorScheme.SystemColors;
				this.Invalidate(true);
			}
...
//			}
		...
		HVTT.UI.Window.Forms.ThemeWindow IThemeCache.ThemeWindow
		{
			get
			{
				if(m_ThemeWindow==null)
		...
		HVTT.UI.Window.Forms.ThemeRebar IThemeCache.ThemeRebar
		{
			get
			{
				if(m_ThemeRebar==null)
		...
		HVTT.UI.Window.Forms.ThemeToolbar IThemeCache.ThemeToolbar
		{
			get
			{
				if(m_ThemeToolbar==null)
		...
		HVTT.UI.Window.Forms.ThemeHeader IThemeCache.ThemeHeader
		{
			get
			{
				if(m_ThemeHeader==null)
		...
		HVTT.UI.Window.Forms.ThemeScrollBar IThemeCache.ThemeScrollBar
		{
			get
			{
				if(m_ThemeScrollBar==null)
		...
		HVTT.UI.Window.Forms.ThemeExplorerBar IThemeCache.ThemeExplorerBar
		{
			get
			{
				if(m_ThemeExplorerBar==null)
		...
		HVTT.UI.Window.Forms.ThemeProgress IThemeCache.ThemeProgress
		{
			get
			{
				if(m_ThemeProgress==null)
        ...
        HVTT.UI.Window.Forms.ThemeButton IThemeCache.ThemeButton
        {
            get
            {
                if (m_ThemeButton == null)
		...
		protected virtual void RecalcSize()
		{
			...
			if(m_BorderStyle==eBorderType.None)
			{
				...
				else if(m_Appearance==SideBarAppearance.Traditional)
				{
					m_ItemContainer.WidthInternal =this.Width-2;
					m_ItemContainer.HeightInternal=this.Height-2;
				}
		...
		/// </summary>
		[System.ComponentModel.Editor(typeof(SideBarPanelsEditor), typeof(System.Drawing.Design.UITypeEditor)),DesignerSerializationVisibility(DesignerSerializationVisibility.Content),System.ComponentModel.Category("Data"),System.ComponentModel.Description("Returns the collection of side-bar Panels.")]
...
//		}
		...
		internal void Deserialize(System.Xml.XmlElement xmlThisBar)
		{
			...
			m_ItemContainer.Style=(HVTTControlStyle)System.Xml.XmlConvert.ToInt32(xmlThisBar.GetAttribute("style"));
			...
			if(xmlThisBar.HasAttribute("app"))
				this.Appearance=(SideBarAppearance)System.Xml.XmlConvert.ToInt32(xmlThisBar.GetAttribute("app"));
			xmlThisBar.SetAttribute("app",System.Xml.XmlConvert.ToString((int)m_Appearance));
			if(xmlThisBar.HasAttribute("psc"))
			{
				if(this.Appearance!=SideBarAppearance.Traditional)
					this.PredefinedColorScheme=(SideBarColorScheme)System.Xml.XmlConvert.ToInt32(xmlThisBar.GetAttribute("psc"));
			}
		}
...
//				{
//					HVTT.UI.Window.Forms.Bar bar=null;
//					if(this.MenuBar)
//						bar=this;
...
//			}
		...
		HVTTMenuDropShadow IOwnerMenuSupport.MenuDropShadow{get{return HVTTMenuDropShadow.Hide;}set{}}
		...
		public static void ApplyColorScheme(SideBarPanelItem item, SideBarColorScheme scheme)
		{
			SideBarColors colors=SideBar.GetColorScheme(scheme);
			colors.Apply(item);
		}

		public static void ApplyColorScheme(ColorScheme cs, SideBarColorScheme scheme)
		{
			SideBarColors colors=SideBar.GetColorScheme(scheme);
			colors.Apply(cs);
		}

		private static SideBarColors GetColorScheme(SideBarColorScheme scheme)
		{
			SideBarColors colors=null;
			switch(scheme)
			{
				case SideBarColorScheme.Brick:
				case SideBarColorScheme.Marine:
				case SideBarColorScheme.Plum:
				case SideBarColorScheme.Pumpkin:
				case SideBarColorScheme.Fire:
				case SideBarColorScheme.Rose:
				case SideBarColorScheme.Slate:
				case SideBarColorScheme.Spruce:
				case SideBarColorScheme.Storm:
				case SideBarColorScheme.Sunset:
				case SideBarColorScheme.Wheat:
				{
					colors=GetFlatColorScheme(scheme);
					break;
				}
				case SideBarColorScheme.Blue:
				case SideBarColorScheme.Silver:
				case SideBarColorScheme.Green:
				case SideBarColorScheme.Orange:
				case SideBarColorScheme.Red:
				case SideBarColorScheme.LightBlue:
				case SideBarColorScheme.Money:
				{
					colors=GetGradientColorScheme(scheme);
					break;
				}
				case SideBarColorScheme.SystemColors:
				{
		...
		private static SideBarColors GetGradientColorScheme(SideBarColorScheme scheme)
		{
		...
		private static SideBarColors GetFlatColorScheme(SideBarColorScheme scheme)
		{
			Color border=Color.Empty;
	...
	internal class SideBarColors
	{
		...
		public void Apply(SideBarPanelItem item)
		{
			...
			item.BackgroundStyle.BorderSide=BorderSide.All;
			...
			item.HeaderStyle.BorderSide=BorderSide.Right | BorderSide.Top | BorderSide.Bottom;
			...
			item.HeaderSideStyle.BorderSide=BorderSide.Left | BorderSide.Top | BorderSide.Bottom;
	...
	{
		public SideBarPanelsEditor(Type type):base(type)
		{
		}
