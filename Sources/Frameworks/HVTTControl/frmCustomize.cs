namespace HVTT.UI.Window.Forms
{
	...
	public class frmCustomize : System.Windows.Forms.Form
	{
		...
		private HVTTManager m_HVTTCONTROLS;
		...
		private HVTT.UI.Editors.ComboItem comboItem1;
		private HVTT.UI.Editors.ComboItem comboItem2;
		private HVTT.UI.Editors.ComboItem comboItem3;
		private HVTT.UI.Editors.ComboItem comboItem4;
		private HVTT.UI.Editors.ComboItem comboItem5;
		private HVTT.UI.Window.Forms.Controls.HVTTComboBox cboAnimations;
		private HVTT.UI.Editors.ComboItem comboItem6;
		...
		public frmCustomize(HVTTManager ctrl)
		{
			//
			...
			try
			{
				m_MoveCursor=new Cursor(typeof(HVTT.UI.Window.Forms.HVTTManager),"DRAGMOVE.CUR");
				m_CopyCursor=new Cursor(typeof(HVTT.UI.Window.Forms.HVTTManager),"DRAGCOPY.CUR");
				m_NACursor=new Cursor(typeof(HVTT.UI.Window.Forms.HVTTManager),"DRAGNONE.CUR");
			}
			catch(Exception)
			{
				m_MoveCursor=null;
			...
			using(LocalizationManager lm=new LocalizationManager(m_HVTTCONTROLS))
			{
			...
			this.cmdReset.Visible=m_HVTTCONTROLS.ShowResetButton;
...
//			}
			...
			lstCommands.Style=m_HVTTCONTROLS.Style;
		...
		public void RefreshBars()
		{
			...
			try
			{
				...
				foreach(HVTT.UI.Window.Forms.HVTTMarkStatus objBar in m_HVTTCONTROLS.Bars)
				{
					if(!objBar.CanHide && !objBar.CanCustomize && objBar.LayoutType==HVTTLayoutType.Toolbar || objBar.LayoutType!=HVTTLayoutType.Toolbar)
					{
		...
		public void RefreshCategories()
		{
			...
			for(int i=0;i<m_HVTTCONTROLS.Items.Count;i++)
			{
				BaseItem objItem=m_HVTTCONTROLS.Items[i];
...
//				lstCategories.SelectedIndex=0;
		...
		protected void BarsCheck(object sender,ItemCheckEventArgs ec)
		{
			...
			HVTT.UI.Window.Forms.HVTTMarkStatus objBar=lstBars.Items[ec.Index] as HVTTMarkStatus;
			if(objBar==null)
				return;
			...
			((IOwner)m_HVTTCONTROLS).InvokeUserCustomize(objBar,new EventArgs());
			((IOwner)m_HVTTCONTROLS).InvokeEndUserCustomize(objBar,new EndUserCustomizeEventArgs(EndUserCustomizeAction.BarVisibilityChanged));
		}
		...
		protected override void OnLoad(EventArgs e)
		{
			...
			chkShowFullMenus.Checked=m_HVTTCONTROLS.AlwaysShowFullMenus;
			chkFullAfterDelay.Checked=m_HVTTCONTROLS.ShowFullMenusOnHover;
			chkShowScreenTips.Checked=m_HVTTCONTROLS.ShowToolTips;
			chkTipsShowShortcuts.Checked=m_HVTTCONTROLS.ShowShortcutKeysInToolTips;
			switch(m_HVTTCONTROLS.PopupAnimation)
			{
				case HVTTPopupAnimation.Fade:
					cboAnimations.SelectedItem=this.comboItem5;
					break;
				case HVTTPopupAnimation.None:
					cboAnimations.SelectedItem=this.comboItem1;
					break;
				case HVTTPopupAnimation.Random:
					cboAnimations.SelectedItem=this.comboItem2;
					break;
				case HVTTPopupAnimation.Slide:
					cboAnimations.SelectedItem=this.comboItem4;
					break;
				case HVTTPopupAnimation.SystemDefault:
					cboAnimations.SelectedItem=this.comboItem6;
					break;
				case HVTTPopupAnimation.Unfold:
		...
		{
			IOwner owner=m_HVTTCONTROLS as IOwner;
			...
			m_HVTTCONTROLS.AlwaysShowFullMenus=chkShowFullMenus.Checked;
			m_HVTTCONTROLS.ShowFullMenusOnHover=chkFullAfterDelay.Checked;
			m_HVTTCONTROLS.ShowToolTips=chkShowScreenTips.Checked;
			m_HVTTCONTROLS.ShowShortcutKeysInToolTips=chkTipsShowShortcuts.Checked;
			if(cboAnimations.SelectedItem==this.comboItem5)
			{
				m_HVTTCONTROLS.PopupAnimation=HVTTPopupAnimation.Fade;
			}
			else if(cboAnimations.SelectedItem==this.comboItem1)
			{
				m_HVTTCONTROLS.PopupAnimation=HVTTPopupAnimation.None;
			}
			else if(cboAnimations.SelectedItem==this.comboItem2)
			{
				m_HVTTCONTROLS.PopupAnimation=HVTTPopupAnimation.Random;
			}
			else if(cboAnimations.SelectedItem==this.comboItem4)
			{
				m_HVTTCONTROLS.PopupAnimation=HVTTPopupAnimation.Slide;
			}
			else if(cboAnimations.SelectedItem==this.comboItem6)
			{
				m_HVTTCONTROLS.PopupAnimation=HVTTPopupAnimation.SystemDefault;
			}
			else if(cboAnimations.SelectedItem==this.comboItem3)
			{
				m_HVTTCONTROLS.PopupAnimation=HVTTPopupAnimation.Unfold;
			}
			else
			{
				m_HVTTCONTROLS.PopupAnimation=HVTTPopupAnimation.SystemDefault;
			...
			m_HVTTCONTROLS.CustomizeClosing();
			base.OnClosing(e);
		}
		...
		{
			base.OnClosed(e);
			if(m_HVTTCONTROLS!=null && m_HVTTCONTROLS.ParentForm!=null)
				m_HVTTCONTROLS.ParentForm.BringToFront();
		}
		...
		protected void CatSelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			for(int i=0;i<m_HVTTCONTROLS.Items.Count;i++)
			{
				...
				objCopy.SetOwner(m_HVTTCONTROLS);
				lst.Add(objCopy);
			}
		...
		private void MouseMoveDrag(Point pScreen)
		{
			if(m_ItemDrag)
			{
				...
				foreach(HVTT.UI.Window.Forms.HVTTMarkStatus bar in m_HVTTCONTROLS.Bars)
				{
			...
			if(m_ItemDrag)
			{
				...
				if(m_DesignTimeProvider!=null)
				{
					...
					if(objParent!=null)
					{
						...
						{
							HVTTMarkStatus bar=objParent.ContainerControl as HVTTMarkStatus;
					...
					m_DesignTimeProvider=null;
					((IOwner)m_HVTTCONTROLS).InvokeUserCustomize(this.DragItem,new EventArgs());
					((IOwner)m_HVTTCONTROLS).InvokeEndUserCustomize(this.DragItem,new EndUserCustomizeEventArgs(EndUserCustomizeAction.ItemMoved));
				}
				else if(this.DragItem.Parent!=null)
				{
					// Remove item if item is out of bounds of the main form
					Point p=pScreen;
					if(m_HVTTCONTROLS.ParentForm!=null && !m_HVTTCONTROLS.ParentForm.Bounds.Contains(p))
					{
						BaseItem objParent=this.DragItem.Parent;
						objParent.SubItems.Remove(this.DragItem);
						Control ctrl=objParent.ContainerControl as Control;
						if(ctrl is HVTTMarkStatus)
							...
							((MenuPanel)ctrl).RecalcSize();
						((IOwner)m_HVTTCONTROLS).InvokeUserCustomize(this.DragItem,new EventArgs());
						((IOwner)m_HVTTCONTROLS).InvokeEndUserCustomize(this.DragItem,new EndUserCustomizeEventArgs(EndUserCustomizeAction.ItemDeleted));
					}
				}
				m_ItemDrag=false;
		...
		public void DesignTimeContextMenu(BaseItem objItem)
		{
			using(LocalizationManager lm=new LocalizationManager(m_HVTTCONTROLS))
			{
				...
				if(m_EditItem is ButtonItem)
				{
					...
					if(objButton.ButtonStyle==HVTTButtonStyle.Default)
						btn.Checked=true;
					m_PopupMenu.SubItems.Add(btn);
					...
					btn.Click+=new System.EventHandler(this.ButtonStyleClick);
					if(objButton.ButtonStyle==HVTTButtonStyle.TextOnlyAlways)
						btn.Checked=true;
					m_PopupMenu.SubItems.Add(btn);
					...
					btn.Click+=new System.EventHandler(this.ButtonStyleClick);
					if(objButton.ButtonStyle==HVTTButtonStyle.ImageAndText)
						btn.Checked=true;
					m_PopupMenu.SubItems.Add(btn);
				...
				m_HVTTCONTROLS.OnCustomizeContextMenu(this,m_PopupMenu);
		        
				//m_HVTTCONTROLS.RegisterPopup(m_PopupMenu);
				m_PopupMenu.SetOwner(m_HVTTCONTROLS);
		...
		{
			((IOwner)m_HVTTCONTROLS).InvokeResetDefinition(m_EditItem, e);
		}
		...
		private void ItemNameLostFocus(object sender, System.EventArgs arg)
		{
			...
			m_EditItem.Refresh();
			((IOwner)m_HVTTCONTROLS).InvokeUserCustomize(m_EditItem,new EventArgs());
			((IOwner)m_HVTTCONTROLS).InvokeEndUserCustomize(m_EditItem,new EndUserCustomizeEventArgs(EndUserCustomizeAction.ItemTextChanged));
		}
		...
		private void ButtonStyleClick(object sender, System.EventArgs e)
		{
			...
			{
				objEditItem.ButtonStyle=HVTTButtonStyle.Default;
				objEditItem.Refresh();
				((IOwner)m_HVTTCONTROLS).InvokeUserCustomize(m_EditItem,new EventArgs());
				((IOwner)m_HVTTCONTROLS).InvokeEndUserCustomize(m_EditItem,new EndUserCustomizeEventArgs(EndUserCustomizeAction.ItemStyleChanged));
			}
			else if(objButton.Name=="textonly" && !objButton.Checked)
			{
				objEditItem.ButtonStyle=HVTTButtonStyle.TextOnlyAlways;
				objEditItem.Refresh();
				((IOwner)m_HVTTCONTROLS).InvokeUserCustomize(m_EditItem,new EventArgs());
				((IOwner)m_HVTTCONTROLS).InvokeEndUserCustomize(m_EditItem,new EndUserCustomizeEventArgs(EndUserCustomizeAction.ItemStyleChanged));
			}
			...
			{
				objEditItem.ButtonStyle=HVTTButtonStyle.ImageAndText;
				objEditItem.Refresh();
				((IOwner)m_HVTTCONTROLS).InvokeUserCustomize(m_EditItem,new EventArgs());
				((IOwner)m_HVTTCONTROLS).InvokeEndUserCustomize(m_EditItem,new EndUserCustomizeEventArgs(EndUserCustomizeAction.ItemStyleChanged));
			}
		...
		private void BeginGroupClick(object sender, System.EventArgs e)
		{
			...
			m_EditItem.Refresh();
			((IOwner)m_HVTTCONTROLS).InvokeUserCustomize(m_EditItem,new EventArgs());
			((IOwner)m_HVTTCONTROLS).InvokeEndUserCustomize(m_EditItem,new EndUserCustomizeEventArgs(EndUserCustomizeAction.ItemBeginGroupChanged));
		}
		...
		private void DeleteClick(object sender, System.EventArgs e)
		{
			...
			if(bar!=null)
				bar.RecalcLayout();
			((IOwner)m_HVTTCONTROLS).InvokeUserCustomize(m_EditItem,new EventArgs());
			((IOwner)m_HVTTCONTROLS).InvokeEndUserCustomize(m_EditItem,new EndUserCustomizeEventArgs(EndUserCustomizeAction.ItemDeleted));
		}
		...
		private void InitializeComponent()
		{
			...
			this.lstCommands = new HVTT.UI.Window.Forms.ItemsListBox();
			this.lstCategories = new System.Windows.Forms.ListBox();
			this.label4 = new System.Windows.Forms.Label();
			this.label3 = new System.Windows.Forms.Label();
			this.label2 = new System.Windows.Forms.Label();
			this.tabPage3 = new System.Windows.Forms.TabPage();
            this.cboAnimations = new Controls.HVTTComboBox();
			this.comboItem1 = new HVTT.UI.Editors.ComboItem();
			this.comboItem6 = new HVTT.UI.Editors.ComboItem();
			this.comboItem2 = new HVTT.UI.Editors.ComboItem();
			this.comboItem3 = new HVTT.UI.Editors.ComboItem();
			this.comboItem4 = new HVTT.UI.Editors.ComboItem();
			this.comboItem5 = new HVTT.UI.Editors.ComboItem();
			...
			this.cboAnimations.Style = HVTT.UI.Window.Forms.HVTTControlStyle.OfficeXP;
		...
		private void NewBar(object sender, System.EventArgs e)
		{
			...
			{
				HVTTMarkStatus bar=new HVTTMarkStatus(nt.txtName.Text);
				bar.CustomBar=true;
				bar.CanHide=true;
				bar.SetDesignMode(true);
				bar.GrabHandleStyle=HVTTGrabHandleStyle.StripeFlat;

				string name="userBar";
				int i=0;
				while(m_HVTTCONTROLS.Bars.Contains(name+i.ToString()))
					i++;
				bar.Name=name+i.ToString();

				m_HVTTCONTROLS.Bars.Add(bar);
				bar.DockSide=eDockSide.None;
				lstBars.Items.Add(bar,CheckState.Checked);
				if(m_HVTTCONTROLS.AllowUserBarCustomize)
					bar.Items.Add(new CustomizeItem());
				((IOwner)m_HVTTCONTROLS).InvokeUserCustomize(bar,new EventArgs());
				((IOwner)m_HVTTCONTROLS).InvokeEndUserCustomize(bar,new EndUserCustomizeEventArgs(EndUserCustomizeAction.NewBarCreated));
			}
			nt.Close();
			nt.Dispose();
		}
		...
		private void BarSelectionChanged(object sender, System.EventArgs e)
		{
			...
			{
				HVTTMarkStatus bar=lstBars.SelectedItem as HVTTMarkStatus;
				if(bar.CustomBar)
				{
				...
				else
				{
					...
					cmdReset.Enabled=m_HVTTCONTROLS.ShowResetButton;
		...
		private void RenameBar(object sender, System.EventArgs e)
		{
			...
			if(tn.ShowDialog(this)==DialogResult.OK)
			{
				...
				lstBars.Refresh();
				((IOwner)m_HVTTCONTROLS).InvokeUserCustomize(bar,new EventArgs());
				((IOwner)m_HVTTCONTROLS).InvokeEndUserCustomize(bar,new EndUserCustomizeEventArgs(EndUserCustomizeAction.BarRenamed));
			}
			tn.Close();
			tn.Dispose();
		}
		...
		private void DeleteBar(object sender, System.EventArgs e)
		{
			...
			using(LocalizationManager lm=new LocalizationManager(m_HVTTCONTROLS))
			{
				if(MessageBox.Show(lm.GetLocalizedString(LocalizationKeys.CustomizeDialogOptionsConfirmDelete).Replace("<barname>","'"+bar.Text+"'"),this.Text,MessageBoxButtons.YesNo)==DialogResult.Yes)
				{
					...
					m_HVTTCONTROLS.Bars.Remove(bar);
					((IOwner)m_HVTTCONTROLS).InvokeUserCustomize(bar,new EventArgs());
					((IOwner)m_HVTTCONTROLS).InvokeEndUserCustomize(bar,new EndUserCustomizeEventArgs(EndUserCustomizeAction.BarDeleted));
					bar.Dispose();
				}
			}
		}
		...
		private void ResetBar(object sender, System.EventArgs e)
		{
			...
			{
				HVTTMarkStatus bar=lstBars.SelectedItem as HVTTMarkStatus;
				if(bar!=null && (bar.CustomBar || m_HVTTCONTROLS.ShowResetButton))
				{
					if(owner!=null)
		...
		internal HVTTManager GetHVTTManager()
		{
			return m_HVTTCONTROLS;
		}
	}
}
