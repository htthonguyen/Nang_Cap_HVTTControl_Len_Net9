namespace HVTT.UI.Window.Forms
{
	...
	public abstract class PopupItem:ImageItem,IDesignTimeProvider
	{
		...
		private HVTTMarkStatus m_PopupBar;
		...
		private PopupSide m_PopupSide=PopupSide.Default;
		...
		protected internal override void Deserialize(ItemSerializationContext context)
		{
			...
			m_PersonalizedMenus=(HVTTPersonalizedMenus)System.Xml.XmlConvert.ToInt32(ItemXmlSource.GetAttribute("PersonalizedMenus"));

			if(ItemXmlSource.HasAttribute("panim"))
				m_PopupAnimation=(HVTTPopupAnimation)System.Xml.XmlConvert.ToInt32(ItemXmlSource.GetAttribute("panim"));
			else
		...
		public override void InternalMouseMove(System.Windows.Forms.MouseEventArgs objArg)
		{
			...
			if(this.Expanded && m_PopupMenu!=null && !this.DesignMode)
			{
				...
				if(container!=null)
				{
					...
					m_PopupMenu.InternalMouseMove(new System.Windows.Forms.MouseEventArgs(objArg.Button,objArg.Clicks,p.X,p.Y,objArg.Delta));
				}
			}
			else if(this.Expanded && m_PopupBar!=null && !this.DesignMode)
			{
				...
				if(container!=null)
				{
					...
					m_PopupBar.InternalMouseMove(new System.Windows.Forms.MouseEventArgs(System.Windows.Forms.MouseButtons.None,objArg.Clicks,p.X,p.Y,objArg.Delta));
				}
			}
		}
		...
		protected internal override void OnExpandChange()
		{
			...
			if(!BaseItem.IsOnPopup(this) || this.Parent==null)
			{
				...
				if(objCtrl is HVTTMarkStatus && !this.DesignMode)
				{
					if(this.Expanded)
					{
						m_WasParentFocused=false;
						...
						else
						{
							...
							{
								HVTTMarkStatus bar=objCtrl as HVTTMarkStatus;
								if(bar.BarState!=HVTTBarState.Floating || bar.MenuBar)
								{
									bar.MenuFocus=true;
									//objCtrl.Focus();
								}
					...
					else
					{
						if(!m_WasParentFocused)
							((HVTTMarkStatus)objCtrl).ReleaseFocus();
						HVTTMarkStatus bar=objCtrl as HVTTMarkStatus;
						if(bar.BarState!=HVTTBarState.Floating && !bar.MenuBar)
							bar.MenuFocus=false;
						m_WasParentFocused=false;
...
//				return;
				...
				if(IsHandleValid(objCtrl))
				{
					if(m_PopupType==ePopupType.Menu)
					{
						...
						if(!m_PopupLocation.IsEmpty)
							p=m_PopupLocation;
						else if(m_PopupSide!=PopupSide.Default)
		...
		public System.Drawing.Size PopupSize
		{
			get
			{
				...
				foreach(BaseItem objItem in this.SubItems)
					objItem.Orientation=HVTTOrientation.Horizontal;
				...
				if(m_PopupFont!=null)
					...
					popupMenu.Font=(System.Drawing.Font)objCtrl.Font.Clone();
				
				if(ownerMenu!=null && ownerMenu.AlwaysShowFullMenus)
					popupMenu.PersonalizedMenus=HVTTPersonalizedMenus.Disabled;
				else
					popupMenu.PersonalizedMenus=m_PersonalizedMenus;
				if(ownerMenu!=null && !ownerMenu.ShowFullMenusOnHover && (popupMenu.PersonalizedMenus==HVTTPersonalizedMenus.DisplayOnHover || popupMenu.PersonalizedMenus==HVTTPersonalizedMenus.Both))
					popupMenu.PersonalizedMenus=HVTTPersonalizedMenus.DisplayOnClick;
				...
				if(this.GetOwner() is HVTTManager && ((HVTTManager)this.GetOwner()).UseGlobalColorScheme)
					popupMenu.ColorScheme=((HVTTManager)this.GetOwner()).ColorScheme;
				if(objCtrl is HVTTMarkStatus)
					popupMenu.ColorScheme=((HVTTMarkStatus)objCtrl).ColorScheme;
				else if(objCtrl is MenuPanel)
					popupMenu.ColorScheme=((MenuPanel)objCtrl).ColorScheme;
				else
					popupMenu.ColorScheme=new ColorScheme(this.Style);
		...
		/// <param name="x">Horizontal coordinate in pixels of the upper left corner of a popup.</param>
		/// <param name="y">Vertical coordinate in pixels of the upper left corner of a popup.</param>
        /// <param name="verifyScreenPosition">Indicates whether screen position of the menu is verifed so the menu always appears on the screen.</param>
		public virtual void PopupMenu(int x, int y, bool verifyScreenPosition)
		{
			...
			if(m_PopupMenu==null)
			{
				...
				if(m_PopupFont!=null)
					m_PopupMenu.Font=m_PopupFont;
						...
						m_PopupMenu.Font=(System.Drawing.Font)objCtrl.Font.Clone();
				
				if(ownerMenu!=null && ownerMenu.AlwaysShowFullMenus)
					m_PopupMenu.PersonalizedMenus=HVTTPersonalizedMenus.Disabled;
				else
                    m_PopupMenu.PersonalizedMenus=m_PersonalizedMenus;
				if(ownerMenu!=null && !ownerMenu.ShowFullMenusOnHover && (m_PopupMenu.PersonalizedMenus==HVTTPersonalizedMenus.DisplayOnHover || m_PopupMenu.PersonalizedMenus==HVTTPersonalizedMenus.Both))
					m_PopupMenu.PersonalizedMenus=HVTTPersonalizedMenus.DisplayOnClick;
				...
				if(objCtrl is HVTTMarkStatus)
				{
                    HVTTMarkStatus bar=(HVTTMarkStatus)objCtrl;
                    m_PopupMenu.ColorScheme = bar.ColorScheme;
                ...
                else if (objCtrl is ItemControl)
                {
                    ...
                    m_PopupMenu.ColorScheme = ic.ColorScheme;
                    m_PopupMenu.AntiAlias = ic.AntiAlias;
                }
                else if (objCtrl is HVTTButton)
                {
                    HVTTButton bx = (HVTTButton)objCtrl;
                    m_PopupMenu.ColorScheme = bx.ColorScheme;
                    m_PopupMenu.AntiAlias = bx.AntiAlias;
                }
                else
                {
                    m_PopupMenu.ColorScheme = new ColorScheme(this.Style);
                    object own = this.GetOwner();
                    if (own is ItemControl)
                    {
                        m_PopupMenu.AntiAlias = ((ItemControl)own).AntiAlias;
                    }
                    else if (own is HVTTMarkStatus)
                    {
                        m_PopupMenu.AntiAlias = ((HVTTMarkStatus)own).AntiAlias;
                    }
                }
			...
			{
                if (objScreen != null && x + m_PopupMenu.Width > workingArea.Right && verifyScreenPosition)
				{
                    PopupPositionAdjusted = true;
					// Push it to the right side
			...
			if(ownerMenu!=null)
			{
				if(PopupShowing!=null)
					PopupShowing(this,new EventArgs());
			...
			if(ownerMenu!=null)
			{
				if(!(objCtrl is HVTTMarkStatus) && !(objCtrl is MenuPanel))
				{
				...
				{
					HVTTMarkStatus bar=objCtrl as HVTTMarkStatus;
					if(bar.BarState!=HVTTBarState.Popup)
					{
						ownerMenu.RegisterPopup(this);
						m_FilterInstalled=true;
					}		
		...
		{
			//HVTTManager owner=this.GetOwner();
			IOwnerMenuSupport ownerMenu=this.GetIOwnerMenuSupport();
			//if(ownerMenu==null)
			...
			if(m_PopupBar==null)
			{
				m_PopupBar=new HVTTMarkStatus();

				if(m_PopupFont!=null)
					m_PopupBar.Font=m_PopupFont;
				else if(objCtrl!=null && objCtrl.Font!=null)
					m_PopupBar.Font=(System.Drawing.Font)objCtrl.Font.Clone();
				if(objCtrl is HVTTMarkStatus && ((HVTTMarkStatus)objCtrl).ColorScheme!=null)
					m_PopupBar.ColorScheme=((HVTTMarkStatus)objCtrl).ColorScheme;
                ...
                else if (objCtrl is HVTTButton && ((HVTTButton)objCtrl).ColorScheme != null)
                    m_PopupBar.ColorScheme = ((HVTTButton)objCtrl).ColorScheme;
				else
					m_PopupBar.ColorScheme=new ColorScheme(this.Style);
				m_PopupBar.SetBarState(HVTTBarState.Popup);
		...
		public virtual void ClosePopup()
		{
			...
			{
				if(PopupClose!=null)
					PopupClose(this,new EventArgs());
				ownerMenu.InvokePopupClose(this,new EventArgs());
			}
			...
			if(PopupFinalized!=null)
				PopupFinalized(this,new EventArgs());
		}

		/// <summary>
		/// Indicates whether sub-items are shown on popup Bar or popup menu.
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Appearance"),System.ComponentModel.Description("Indicates whether sub-items are shown on popup Bar or popup menu."),System.ComponentModel.DefaultValue(ePopupType.Menu)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Appearance"),System.ComponentModel.Description("Indicates the font that will be used on the popup window."),System.ComponentModel.DefaultValue(null)]
		...
		private System.Drawing.Point GetDisplayLocation()
		{
			...
			if(this.Parent==null && !(this.ContainerControl is HVTTButton))
				return p;

			if(m_PopupSide==PopupSide.Right)
			{
				p.X=m_Rect.Right;
				p.Y=m_Rect.Top;
			}
			else if(m_PopupSide==PopupSide.Left)
			{
				System.Drawing.Size ps=this.PopupSize;
				p.X=m_Rect.Left-ps.Width;
				p.Y=m_Rect.Top;
			}
			else if(m_PopupSide==PopupSide.Top)
			{
				System.Drawing.Size ps=this.PopupSize;
			...
			else if(m_PopupSide==PopupSide.Bottom)
			{
				System.Drawing.Size ps=this.PopupSize;
				p.Y=m_Rect.Bottom;
				if(ps.Width>m_Rect.Width)
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Appearance"),System.ComponentModel.Description("Indicates when menu items are displayed when MenuVisiblity is set to VisibleIfRecentlyUsed and RecentlyUsed is true."),System.ComponentModel.DefaultValue(HVTTPersonalizedMenus.Disabled)]
		public virtual HVTTPersonalizedMenus PersonalizedMenus
		{
			get
			{
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Behavior"),System.ComponentModel.Description("Indicates Animation type for Popups."),System.ComponentModel.DefaultValue(HVTTPopupAnimation.ManagerControlled)]
		public virtual HVTTPopupAnimation PopupAnimation
		{
			get
			{
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Layout"),System.ComponentModel.Description("Specifies the inital width for the Bar that hosts pop-up items. Applies to PopupType.Toolbar only."),System.ComponentModel.DefaultValue(200)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(false),System.ComponentModel.Description("Gets the control that is displaying the context menu.")]
