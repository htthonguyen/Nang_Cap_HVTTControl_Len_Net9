...
namespace HVTT.UI.Window.Forms.TextMarkup
{
    internal class ExpandElement : MarkupElement
    {
        ...
        public override void Render(MarkupDrawContext d)
        {
            ...
            if (d.ContextObject is ButtonItem)
            {
                if (m_Direction == eExpandDirection.Default)
                {
                    ...
                    if (item.IsOnMenu)
                    {
                        ...
                        if (item.PopupSide == PopupSide.Default && d.RightToLeft || item.PopupSide == PopupSide.Left)
                            direction = eExpandDirection.Left;
                    }
                    else if (item.PopupSide == PopupSide.Default)
                        direction = eExpandDirection.Bottom;
                    else if (item.PopupSide == PopupSide.Left)
                        direction = eExpandDirection.Left;
                    else if (item.PopupSide == PopupSide.Right)
                        direction = eExpandDirection.Right;
                    else if (item.PopupSide == PopupSide.Bottom)
                        direction = eExpandDirection.Bottom;
                    else if (item.PopupSide == PopupSide.Top)
            ...
            using (SolidBrush brush = new SolidBrush(Color.FromArgb(96, Color.White)))
            ...
            if (direction == eExpandDirection.PopupDropDown)
            {
                ...
                using (Pen pen = new Pen(Color.FromArgb(96, Color.White), 1))
