namespace HVTT.UI.Window.Forms
{
    ...
    public class CustomizeItem:PopupItem
    {
		...
		public override void Paint(ItemPaintArgs pa)
		{
			...
			else
			{
				if(this.IsThemed && !this.IsOnMenu)
					PaintThemed(pa);
                else if (m_Style == HVTTControlStyle.Office2003 || this.Style == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(this.Style))
				{
					PaintOffice2003(pa);
					return;
				}
		...
		private void PaintOffice2003(ItemPaintArgs pa)
		{
            ...
            if (this.Orientation == HVTTOrientation.Vertical)
            {
		...
		/*private bool IsOnPopUp()
		{
			...
			if(objTlb!=null && objTlb.BarState==HVTTBarState.Popup)
				return true;
			return false;
		}*/
		...
		private void ShowHideClick(object sender, System.EventArgs e)
		{
			...
			objBtn.Visible=!objBtn.Visible;
            objBtn.UserCustomized = true;
			objBtn.GlobalItem=bGlobal;
			((BaseItem)sender).Visible=!((BaseItem)sender).Visible;
			
			if(objBtn.ContainerControl is HVTTMarkStatus)
				((HVTTMarkStatus)objBtn.ContainerControl).RecalcLayout();
			...
			if(owner!=null)
			{
				...
				owner.InvokeEndUserCustomize(objBtn,new EndUserCustomizeEventArgs(EndUserCustomizeAction.ItemVisibilityChanged));
			}
		}
		...
		public override void Refresh()
		{
			if(this.SuspendLayout)
				return;
            if ((this.Style == HVTTControlStyle.Office2003 || this.Style == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(this.Style)) && !BaseItem.IsOnPopup(this))
			{
		...
		private void AddCustomizeItems()
		{
			...
			foreach(BaseItem objItem in objParent.SubItems)
			{
				if(!objItem.SystemItem && objItem.CanCustomize)
				{
					...
					{
						((ButtonItem)objTmp).HotTrackingStyle=HotTrackingStyle.Default;
			...
			if(owner!=null && owner.ShowResetButton)
			{
				...
				objBtn.Orientation=HVTTOrientation.Horizontal;
		...
		/// </summary>
        [Browsable(true), HVTTBrowsable(true), DefaultValue(true), Category("Behavior"), Description("Determines whether the item is visible or hidden.")]
        public override bool Visible
		{
			get
			{
				...
				if(objTlb!=null)
				{
					if(objTlb.BarState==HVTTBarState.Docked || objTlb.BarState==HVTTBarState.Popup)
						return base.Visible;
					else
						return false;
				}
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Behavior"),Description("Indicates whether Customize menu item is visible."),DefaultValue(true)]
        ...
        /// </summary>
        [Browsable(false), HVTTBrowsable(false), Category("Behavior"), DefaultValue(false), Description("Indicates whether the item will auto-collapse (fold) when clicked.")]
        ...
        /// </summary>
        [Browsable(false), HVTTBrowsable(false), DefaultValue(false), Category("Behavior"), Description("Indicates whether item can be customized by user.")]
        ...
        /// </summary>
        [Browsable(false), HVTTBrowsable(false), DefaultValue(false), Category("Behavior"), Description("Indicates whether certain global properties are propagated to all items with the same name when changed.")]
