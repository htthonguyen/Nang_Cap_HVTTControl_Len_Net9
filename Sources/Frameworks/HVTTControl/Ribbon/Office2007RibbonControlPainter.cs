...
namespace HVTT.UI.Window.Forms
{
    internal class Office2007RibbonControlPainter : RibbonControlPainter, Rendering.IOffice2007Painter
    {
        ...
        public override void PaintBackground(RibbonControlRendererEventArgs e)
        {
            ...
            if (rc.Expanded && rc.RibbonStrip.HasVisibleTabs)
            {
                ...
                using (GraphicsPath path = GetBottomLinePath(bottomBounds, cornerSize))
                {
                    ...
                    if (g.Clip != null) oldRegion = g.Clip.Clone() as Region;
        ...
        public HVTT.UI.Window.Forms.Rendering.Office2007ColorTable ColorTable
        {
            get
            {
                return m_ColorTable;
        ...
        public override void PaintCaptionText(RibbonControlRendererEventArgs e)
        {
            ...
            if (!rs.QuickToolbarBounds.IsEmpty)
                ExcludeEdgeRect(ref captionRect, rs.QuickToolbarBounds);
            else
            {
                BaseItem sb = e.RibbonControl.GetStartButton();
                if(sb!=null && sb.Visible && sb.Displayed)
                    ExcludeEdgeRect(ref captionRect, sb.Bounds);
                else
                    ExcludeEdgeRect(ref captionRect, new Rectangle(0,0,22,22)); // The system button in top-left corner
            }

            if(!rs.SystemCaptionItemBounds.IsEmpty)
                ExcludeEdgeRect(ref captionRect, rs.SystemCaptionItemBounds);
                ...
                foreach (RibbonTabItemGroup group in rs.TabGroups)
                {
                    foreach (Rectangle r in group.DisplayPositions)
                    {
                        if (rects.Count > 0)
                        {
                            ...
                            for(int irc = 0; irc<arrCopy.Length; irc++)
                            {
                                if (arrCopy[irc].IntersectsWith(r))
                                {
                                    ...
                                    Rectangle[] splitRects = ExcludeRectangle(arrCopy[irc], r);
                        ...
                        else
                        {
                            ...
                            {
                                Rectangle[] splitRects = ExcludeRectangle(captionRect, r);
        ...
        public static void PaintTextOnGlass(Graphics g, string text, Font font, Rectangle rect, ThemeTextFormat tf)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    dttOpts.iGlowSize = 16;
                    dttOpts.crText = new Themes.COLORREF(SystemColors.ControlText);
                    dttOpts.dwFlags = (int)Themes.DTT_VALIDBITS.DTT_COMPOSITED | (int)Themes.DTT_VALIDBITS.DTT_GLOWSIZE | (int)Themes.DTT_VALIDBITS.DTT_TEXTCOLOR;
        ...
        private void PaintGlassText(Graphics g, string text, Font font, Rectangle captionRect, bool isMaximized)
        {
            ...
            //    {
            //        WinApi.BITMAPINFO info = new WinApi.BITMAPINFO();
            //        info.biWidth = captionRect.Width;
            //        info.biHeight = -captionRect.Height;
            //        info.biPlanes = 1;
            //        info.biBitCount = 32;
            //        info.biSize = Marshal.SizeOf(info);
            //        IntPtr dib = WinApi.CreateDIBSection(hdc, info, 0, 0, IntPtr.Zero, 0);
            ...
            //        Themes.RECT bounds = new Themes.RECT(new Rectangle(0, 0, captionRect.Width, captionRect.Height));
            //        System.Windows.Forms.VisualStyles.VisualStyleRenderer themeRenderer = new System.Windows.Forms.VisualStyles.VisualStyleRenderer(System.Windows.Forms.VisualStyles.VisualStyleElement.Window.Caption.Active);
            //        Themes.DTTOPTS dttOpts = new Themes.DTTOPTS();
            //        dttOpts.iGlowSize = 16;
            //        dttOpts.crText = new Themes.COLORREF(SystemColors.ControlText);
            ...
            //        const int SRCCOPY = 0x00CC0020;
            //        WinApi.BitBlt(hdc, captionRect.Left, captionRect.Top, captionRect.Width, captionRect.Height, memdc, 0, 0, SRCCOPY);
        ...
        private void ExcludeEdgeRect(ref Rectangle captionRect, Rectangle exclude)
        {
        ...
        private Rectangle[] ExcludeRectangle(Rectangle r1, Rectangle exclude)
        {
            if (r1.X >= exclude.X && exclude.Right < r1.X) // Left aligned
        ...
        public override void PaintQuickAccessToolbarBackground(RibbonControlRendererEventArgs e)
        {
            ...
            {
                BaseItem startButton = ribbon.GetStartButton();
                if (startButton != null)
                {
