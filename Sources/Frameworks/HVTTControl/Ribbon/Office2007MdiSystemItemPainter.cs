...
namespace HVTT.UI.Window.Forms
{
    ...
    internal class Office2007MdiSystemItemPainter : MdiSystemItemPainter, IOffice2007Painter
    {
        ...
        public override void Paint(MdiSystemItemRendererEventArgs e)
        {
            ...
            if (mdi.Orientation == HVTTOrientation.Horizontal)
                r.Offset(0, (mdi.DisplayRectangle.Height - r.Height) / 2);
            else
                r.Offset((mdi.WidthInternal - r.Width) / 2, 0);

            this.PaintButton(g, mdi, SystemButton.Minimize, r, GetColorTable(mdi, SystemButton.Minimize));

            if (mdi.Orientation == HVTTOrientation.Horizontal)
                r.Offset(r.Width, 0);
            else
                r.Offset(0, r.Height);
            this.PaintButton(g, mdi, SystemButton.Restore, r, GetColorTable(mdi, SystemButton.Restore));
            ...
            this.PaintButton(g, mdi, SystemButton.Close, r, GetColorTable(mdi, SystemButton.Close));
        }
        ...
        private void PaintButton(Graphics g, MDISystemItem mdi , SystemButton button, Rectangle r, Office2007ButtonItemStateColorTable ct)
        {
            Region oldClip = g.Clip;
            g.SetClip(r);

            Office2007ButtonItemPainter.PaintBackground(g, ct, r, m_CornerSize);
            r.Inflate(-1, -1);
            r.Offset(1, 0);
            using (Bitmap bmp = mdi.GetButtonBitmap(g, button, r))
            {
