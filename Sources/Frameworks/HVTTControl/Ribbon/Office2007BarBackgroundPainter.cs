...
namespace HVTT.UI.Window.Forms
{
    internal class Office2007BarBackgroundPainter : BarBackgroundPainter, IOffice2007Painter
    {
        ...
        public override void PaintDockedBackground(ToolbarRendererEventArgs e)
        {
            ...
            HVTTMarkStatus bar = e.Bar;
            ItemPaintArgs pa = e.ItemPaintArgs;
            Rectangle r = e.Bounds;
            ColorScheme cs = m_ColorTable.LegacyColors;
            if (bar.LayoutType == HVTTLayoutType.DockContainer || bar.LayoutType == HVTTLayoutType.TaskList)
            {
            ...
            else if (bar.GrabHandleStyle != HVTTGrabHandleStyle.ResizeHandle && bar.BarType != eBarType.StatusBar)
            {
                // Docked state
                if (bar.ItemsContainer.m_BackgroundColor.IsEmpty && bar.BackColor != Color.Transparent)
                {
                    if (bar.IsThemed)
                    {
                        ...
                        theme.DrawBackground(g, ThemeRebarParts.Background, ThemeRHVTTBarStates.Normal, rb);
                    }
                    else
                    {
                ...
                if (!bar.IsThemed && bar.LayoutType == HVTTLayoutType.Toolbar && bar.BackColor != Color.Transparent && pa!=null)
                {
        ...
        private bool IsGradientBackground(HVTTMarkStatus bar)
        {
            if (bar.Style == HVTTControlStyle.VS2005 && bar.LayoutType == HVTTLayoutType.DockContainer)
                return false;
            return true;
        }
        #endregion
