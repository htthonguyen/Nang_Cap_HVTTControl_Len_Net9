...
namespace HVTT.UI.Window.Forms
{
	...
	{
		public static ItemContainer CreateItemContainer(IDesignerServices designer, BaseItem parent, HVTTOrientation containerOrientation)
		{
			...
			DesignerTransaction trans=dh.CreateTransaction("New HVTTCONTROLS Item Container");
			try
			{
		...
		public static void SetDefaults(RibbonTabItemGroup group)
		{
			...
			TypeDescriptor.GetProperties(group.Style)["Border"].SetValue(group.Style,StyleBorderType.Solid);
			TypeDescriptor.GetProperties(group.Style)["BorderColor"].SetValue(group.Style,ColorScheme.GetColor("9A3A3B"));
			TypeDescriptor.GetProperties(group.Style)["CornerType"].SetValue(group.Style,CornerType.Square);
			...
			TypeDescriptor.GetProperties(group.Style)["TextAlignment"].SetValue(group.Style,StyleTextAlignment.Center);
			TypeDescriptor.GetProperties(group.Style)["TextLineAlignment"].SetValue(group.Style,StyleTextAlignment.Near);
		}
	}
}
