...
namespace HVTT.UI.Window.Forms
{
    ...
    internal class Office2007RibbonTabGroupPainter : RibbonTabGroupPainter, IOffice2007Painter
    {
        ...
        public override void PaintTabGroup(RibbonTabGroupRendererEventArgs e)
        {
            ...
            ElementStyleDisplayInfo info = new ElementStyleDisplayInfo(style, e.Graphics, e.Bounds);
            ElementStyleDisplay.PaintText(info, e.RibbonTabItemGroup.GroupTitle, e.GroupFont, false, e.RibbonTabItemGroup.Style.TextFormat | eTextFormat.VerticalCenter);
        ...
        private void PaintTabGroupBackground(Graphics g, Office2007RibbonTabGroupColorTable colorTable, Rectangle bounds, Rectangle groupBounds, bool glassEnabled)
        {
            if (colorTable == null)
                return;
            ...
            DisplayHelp.FillRectangle(g, rFill, colorTable.Background.Start, colorTable.Background.End, 90, new float[] { 0f, .10f, .5f }, new float[] { 0f, (glassEnabled?.4f:.80f), 1f });

            // Draw highlight
            if (!colorTable.BackgroundHighlight.IsEmpty && r.Width > 0 && r.Height > 0)
            {
                Rectangle ellipse = new Rectangle(r.X - r.Width, r.Y, r.Width * 3, (int)(r.Height * 2.5f));
                ...
                blend.Positions = new float[] { .0f, .80f, 1f };
                brush.Blend = blend;
                path.Dispose();
                //rFill.Height++;
