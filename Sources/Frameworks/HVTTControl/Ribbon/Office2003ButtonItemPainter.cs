...
namespace HVTT.UI.Window.Forms
{
	...
	internal class Office2003ButtonItemPainter:ButtonItemPainter
    {
            ...
            try
            {
                ...
                if (!pa.IsOnMenu && !bIsOnMenuBar && bThemed)
                {
                    if (pa.ContainerControl is HVTTButton)
                        ThemedHVTTButtonPainter.PaintButton(button, pa);
                    else
                ...
                if (button.HotTrackingStyle != HotTrackingStyle.None)
                {
                ...
                if (image != null && button.ButtonStyle != HVTTButtonStyle.TextOnlyAlways)
		...
		public override void PaintButtonImage(ButtonItem button, ItemPaintArgs pa, CompositeImage image, Rectangle imagebounds)
		{
			...
			else if(!button.IsMouseOver && button.HotTrackingStyle==HotTrackingStyle.Color)
			{
...
//			}
		...
		public override Rectangle GetImageRectangle(ButtonItem button, ItemPaintArgs pa, CompositeImage image)
		{
			...
			if(image!=null)
			{
                ...
                {
                    if (!isOnMenu && (button.ImagePosition == ImagePosition.Top || button.ImagePosition == ImagePosition.Bottom))
		...
		public override Rectangle GetCheckRectangle(ButtonItem button, ItemPaintArgs pa, CompositeImage image)
		{
			...
			else if(button.HotTrackingStyle==HotTrackingStyle.Image && image!=null)
			{
		...
		public override void PaintButtonCheck(ButtonItem button, ItemPaintArgs pa, CompositeImage image, Rectangle r)
		{
			...
			if((image==null || button.ButtonStyle==HVTTButtonStyle.TextOnlyAlways) && isOnMenu)
			{
				// Draw checker...
		...
		public override Rectangle GetMouseOverRectangle(ButtonItem button, ItemPaintArgs pa, CompositeImage image)
		{
			...
			if(button.HotTrackingStyle==HotTrackingStyle.None || button.HotTrackingStyle==HotTrackingStyle.Color)
				return Rectangle.Empty;

			if(button.HotTrackingStyle==HotTrackingStyle.Image && image!=null)
			{
				r=GetImageRectangle(button,pa,image);
				r.Inflate(2,2);
				return r;
		...
		public override eTextFormat GetStringFormat(ButtonItem button, ItemPaintArgs pa, CompositeImage image)
		{
			...
			if(!isOnMenu)
			{
				if(pa.ContainerControl is RibbonStrip && (image==null || button.ImagePosition==ImagePosition.Top || button.ImagePosition==ImagePosition.Bottom))
					stringFormat |= eTextFormat.HorizontalCenter;
                else if (pa.ContainerControl is HVTTButton)
                {
                    HVTTButton HVTTButton = pa.ContainerControl as HVTTButton;
                    if(HVTTButton.TextAlignment== eButtonTextAlignment.Center)
                        stringFormat |= eTextFormat.HorizontalCenter;
                    else if(HVTTButton.TextAlignment== eButtonTextAlignment.Left)
                        stringFormat |= eTextFormat.Left;
                    else if (HVTTButton.TextAlignment == eButtonTextAlignment.Right && (image==null || button.ImagePosition == ImagePosition.Top || button.ImagePosition == ImagePosition.Bottom))
                        stringFormat |= eTextFormat.Right;
                }
                else if (pa.IsOnMenuBar || (pa.ContainerControl is HVTTMarkStatus || pa.ContainerControl is HVTTButton || button.Orientation == HVTTOrientation.Vertical) && image == null || button.ImagePosition == ImagePosition.Top || button.ImagePosition == ImagePosition.Bottom)
                    stringFormat |= eTextFormat.HorizontalCenter;
                if(pa.ContainerControl is RibbonBar && image!=null && button.ImagePosition==ImagePosition.Top)
        ...
        protected virtual bool IsTextCentered(ButtonItem button, ItemPaintArgs pa, CompositeImage image)
        {
            ...
            if (!isOnMenu)
            {
                ...
                if (cc is RibbonStrip && (image == null || button.ImagePosition == ImagePosition.Top || button.ImagePosition == ImagePosition.Bottom) || button.Name.StartsWith("sysgallery"))
                    return true;
                else if (cc is HVTTButton)
                {
                    HVTTButton HVTTButton = cc as HVTTButton;
                    if (HVTTButton.TextAlignment == eButtonTextAlignment.Center)
                        return true;
                }
                else if (pa.IsOnMenuBar || cc is HVTTMarkStatus && image == null || button.ImagePosition == ImagePosition.Top || button.ImagePosition == ImagePosition.Bottom)
                    return true;
            }
        ...
        protected virtual Rectangle GetTextRectangle(ButtonItem button, ItemPaintArgs pa, eTextFormat stringFormat, CompositeImage image)
        {
            ...
            // Draw menu item text
            if (isOnMenu || button.ButtonStyle != HVTTButtonStyle.Default || image == null || (!isOnMenu && (button.ImagePosition == ImagePosition.Top || button.ImagePosition == ImagePosition.Bottom)))
            {
                if (isOnMenu)
                {
                    if (rightToLeft)
                ...
                else
                {
                    ...
                    if (button.ImagePosition == ImagePosition.Top || button.ImagePosition == ImagePosition.Bottom)
                    {
                        if (button.Orientation != HVTTOrientation.Vertical)
                        {
                            textRect = new Rectangle(0, textRect.Y, itemRect.Width, textRect.Height);
                        }
                    }
                }
                ...
                if (button.Orientation == HVTTOrientation.Vertical && !isOnMenu)
                {
			...
			if(isOnMenu || button.ButtonStyle!=HVTTButtonStyle.Default || image==null || (!isOnMenu && (button.ImagePosition==ImagePosition.Top || button.ImagePosition==ImagePosition.Bottom)))
			{
				...
				else
				{
                    if (button.TextMarkupBody == null)
                    {
                        ...
                        if (pa.GlassEnabled && button.Parent is CaptionItemContainer)
                        {
                            if (!pa.CachedPaint)
                                Office2007RibbonControlPainter.PaintTextOnGlass(g, button.Text, font, rect, TextDrawing.GetTextFormat(stringFormat));
                        }
                        else
                    ...
                    else
                    {
                        ...
                        //Rectangle mr = new Rectangle(rect.X, rect.Y + (rect.Height - button.TextMarkupBody.Bounds.Height) / 2 - (isOnMenu ? 2 : 0), button.TextMarkupBody.Bounds.Width, button.TextMarkupBody.Bounds.Height);
                        if (IsTextCentered(button, pa, image)) //!isOnMenu && !(pa.ContainerControl is ItemPanel))
                            mr.Offset((rect.Width - mr.Width) / 2, 0);
                        button.TextMarkupBody.Bounds = mr;
                        button.TextMarkupBody.Render(d);
                    }

					if(!button.DesignMode && button.Focused && !isOnMenu && !isOnMenuBar)
					{
						Rectangle r=itemRect;
                        r.Inflate(-2, -2);
		...
		public override void PaintExpandButton(ButtonItem button, ItemPaintArgs pa)
		{
            ...
            using (SolidBrush textBrush = new SolidBrush(textColor))
            {
                ...
                if ((button.SubItems.Count > 0 || button.PopupType == ePopupType.Container) && button.ShowSubItems)
                {
                    ...
                    {
                        if (IsItemEnabled(button, pa) && ((mouseOver || button.Checked) && !button.Expanded && button.HotTrackingStyle != HotTrackingStyle.None && button.HotTrackingStyle != HotTrackingStyle.Image) && !button.AutoExpandOnClick)
                        {
		...
		public override void PaintButtonBackground(ButtonItem button, ItemPaintArgs pa, CompositeImage image)
		{
			...
			if(IsItemEnabled(button, pa) || button.DesignMode)
			{
				...
				{					
					...
					Point[] p;
					if(button.Orientation==HVTTOrientation.Horizontal && button.PopupSide==PopupSide.Default)
						p=new Point[4];
					else
						p=new Point[5];
					p[0].X=itemRect.Left;
					...
					p[1].X=itemRect.Left;
					p[1].Y=itemRect.Top;
                    if (button.Orientation == HVTTOrientation.Horizontal /*&& !pa.Colors.ItemExpandedShadow.IsEmpty*/)
                    {
                        if (!pa.Colors.ItemExpandedShadow.IsEmpty)
					...
					if(button.Orientation==HVTTOrientation.Vertical || button.PopupSide!=PopupSide.Default)
					{
						p[4].X=itemRect.Left;
					...
					// Draw the shadow
					if(!pa.Colors.ItemExpandedShadow.IsEmpty && button.Orientation==HVTTOrientation.Horizontal)
					{
