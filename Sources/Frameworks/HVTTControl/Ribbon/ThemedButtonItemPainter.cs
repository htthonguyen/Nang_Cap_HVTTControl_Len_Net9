...
namespace HVTT.UI.Window.Forms
{
	...
	public class ThemedButtonItemPainter
	{
		public static void PaintButton(ButtonItem button, ItemPaintArgs pa)
		{
			...
			{
				if(state==ThemeToolbarStates.Normal && button.HotTrackingStyle==HotTrackingStyle.Color)
				{
			...
			if(bSplitButton)
			{
				...
				if(button.HotTrackingStyle!=HotTrackingStyle.None && button.HotTrackingStyle!=HotTrackingStyle.Image && ButtonItemPainter.IsItemEnabled(button, pa))
				{
		...
		private static eTextFormat GetStringFormat(ButtonItem button, ItemPaintArgs pa, CompositeImage image)
		{
			...
			if(!isOnMenu)
			{
				if(button.ContainerControl is RibbonStrip && (image==null || button.ImagePosition==ImagePosition.Top || button.ImagePosition==ImagePosition.Bottom))
					stringFormat |= eTextFormat.HorizontalCenter;
				else if(pa.IsOnMenuBar || pa.ContainerControl is HVTTMarkStatus && image==null || button.ImagePosition==ImagePosition.Top || button.ImagePosition==ImagePosition.Bottom)
					stringFormat |= eTextFormat.HorizontalCenter;
			}
			return stringFormat;
		}
