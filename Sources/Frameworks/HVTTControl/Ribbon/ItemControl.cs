...
namespace HVTT.UI.Window.Forms
{
        ...
        IKeyTipsRenderer, IRenderingSupport, IAccessibilitySupport, IRibbonCustomize, IBarImageSize, IKeyTipsControl
	{
		...
		/// </summary>
		[Description("Occurs after HVTTCONTROLS definition is loaded.")]
		...
		public ItemControl()
		{
            ...
            //try
            //{
            //    m_MoveCursor=new Cursor(typeof(HVTTManager),"DRAGMOVE.CUR");
            //    //m_CopyCursor=new Cursor(typeof(HVTTManager),"DRAGCOPY.CUR");
            //    m_NACursor=new Cursor(typeof(HVTTManager),"DRAGNONE.CUR");
            //}
            //catch(Exception)
            //{
				m_MoveCursor=null;
		...
		protected override void WndProc(ref Message m)
		{
            if (m.Msg == NativeFunctions.WM_SETFOCUS)
            {
                ...
                if (objCtrl != null && !(objCtrl is HVTT.UI.Window.Forms.HVTTMarkStatus) && !(objCtrl is MenuPanel) && !(objCtrl is Controls.HVTTMarkTextBox) && !(objCtrl is Controls.HVTTComboBox))
                {
        ...
        protected RibbonControl GetRibbonControl()
        {
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Layout"),Description("Indicates whether control height is set automatically."),DefaultValue(false)]
		...
		internal BaseItem GetBaseItemContainer()
		{
		...
		protected bool IsThemed
		{
			get
			{
				if(m_ThemeAware && m_BaseItemContainer.Style!=HVTTControlStyle.Office2000 && BarFunctions.ThemedOS && Themes.ThemesActive)
					return true;
				return false;
			}
		}
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(false),Category("Appearance"),Description("Specifies whether SideBar is drawn using Themes when running on OS that supports themes like Windows XP.")]
        ...
        {
            if (m_BaseItemContainer.Style == HVTTControlStyle.Office2007)
            {
        ...
        internal bool IsFadeEnabled
        {
            get
            {
                if (this.DesignMode || (m_BaseItemContainer.Style!=HVTTControlStyle.Office2007) || m_FadeEffect && NativeFunctions.IsTerminalSession())
                    return false;
                return m_FadeEffect;
            }
        }
