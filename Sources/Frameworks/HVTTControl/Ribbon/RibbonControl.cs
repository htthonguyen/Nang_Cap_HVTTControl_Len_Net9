...
namespace HVTT.UI.Window.Forms
{
	...
	/// </summary>
	[ToolboxBitmap(typeof(RibbonControl),"Ribbon.RibbonControl.ico"),ToolboxItem(true),Designer(typeof(Design.RibbonControlDesigner)),System.Runtime.InteropServices.ComVisible(false)]
	public class RibbonControl:System.Windows.Forms.ContainerControl
    {
        ...
        public event RibbonCustomizeToolItemClickHandler ToolItemClick;
        ...
        /// the custom dialog you want used instead of the HVTTCONTROLS system customization dialog.
        /// </summary>
		...
		public RibbonControl()
        {
            ...
            m_RibbonStrip.LocalizeString += new HVTTManager.LocalizeStringEventHandler(RibbonStripLocalizeString);
        ...
        private int Search(String sName)
        {
            SubItemsCollection sb = GetQatSubItemsCollection();
            if (sb != null)
            {
        ...
        {
            if (sender is ButtonItem && ((ButtonItem)sender).Checked)
            {
                AutoSyncSize();
        ...
        private void RibbonStripItemClick(object sender, System.EventArgs e)
        {
            if (sender is ButtonItem)
            {
                ...
                int i = Search(bt.Name);
                if (i >= 0)
                {
                    if (ToolItemClick != null)
                    {
                        RibbonCustomizeToolItemClickArgs rt = new RibbonCustomizeToolItemClickArgs(bt.Name);
                        ToolItemClick(sender, rt);
                    }
                }
            }
        ...
        /// </summary>
        [Browsable(true), HVTTBrowsable(true), Category("Layout"), Description("Indicates whether control height is set automatically."), DefaultValue(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Visible)]
        ...
        /// </summary>
        [Browsable(true), DefaultValue(""), Editor(typeof(Design.TextMarkupUIEditor), typeof(System.Drawing.Design.UITypeEditor)), EditorBrowsable(EditorBrowsableState.Always), Category("Appearance"), Description("Indicates text displayed on Ribbon Title instead of the Form.Text property.")]
		...
		/// </summary>
		[Editor(typeof(RibbonTabItemGroupCollectionEditor), typeof(System.Drawing.Design.UITypeEditor)),DesignerSerializationVisibility(DesignerSerializationVisibility.Content),Category("Tab Groups"),Browsable(true),HVTTBrowsable(true)]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(10),Category("Tab Groups"),Description("Indicates height in pixels of tab group line that is displayed above the RibbonTabItem objects that have group assigned.")]
		...
		/// </summary>
        [Browsable(true), HVTTBrowsable(true), DefaultValue(false), Category("Tab Groups"), Description("Indicates whether tab group line that is displayed above the RibbonTabItem objects that have group assigned is visible.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Tab Groups"),DefaultValue(null)]
        ...
        private SubItemsCollection GetQatSubItemsCollection()
        {
            if (m_QatPositionedBelow)
            {
                if (m_QatSubItemsCollection == null)
                {
                    ...
                    BaseItem startButton = GetStartButton();
                    if (startButton != null)
		...
		public HVTTControlStyle Style
		{
			...
			set 
			{
				m_RibbonStrip.Style=value;
				foreach(Control c in this.Controls)
				{
                    if (c is RibbonPanel)
                    {
                        ((RibbonPanel)c).ColorSchemeStyle = value;
                        ...
                        foreach (Control r in c.Controls)
                        {
                            if (r is RibbonBar)
                            {
                                ...
                                TypeDescriptor.GetProperties(rb)["Style"].SetValue(rb, value);
                            }
                        }
                    }
				}
        ...
        private Rendering.eOffice2007ColorScheme m_DelayedColorTableChange = HVTT.UI.Window.Forms.Rendering.eOffice2007ColorScheme.Blue;
        ...
        /// </summary>
        [Browsable(true), Category("Appearance"), DefaultValue(Rendering.eOffice2007ColorScheme.Blue), Description("Indicates the Office 2007 Renderer global Color Table.")]
        ...
        internal void OnChildItemClick(BaseItem item)
        {
            ...
            this.CloseRibbonMenu(item, EventSource.Mouse);
        }
        ...
        {
            if (m_PopupMode)
                this.CloseRibbonMenu(this, EventSource.Code);
        ...
        public void PopupRibbon(object source, EventSource eventSource)
        {
            ...
            m_PopupMode = true;
            m_PopupRibbonPanel = this.SelectedRibbonTabItem.Panel;
            m_PopupRibbonPanel.SuspendLayout();
            this.Controls.Remove(m_PopupRibbonPanel);
            m_PopupRibbonPanel.Visible = false;
            m_PopupRibbonPanel.Dock = DockStyle.None;
            f.Controls.Add(m_PopupRibbonPanel);
            Point p = m_RibbonStrip.PointToScreen(new Point(0, m_RibbonStrip.Height));
            p = f.PointToClient(p);
            m_PopupRibbonPanel.Font = this.Font;
            m_PopupRibbonPanel.Bounds = new Rectangle(p.X, p.Y - DefaultBottomDockPadding - 1, this.Width, GetPopupRibbonPanelHeight(m_PopupRibbonPanel));
            m_PopupRibbonPanel.SetPopupMode(true, this);
            m_PopupRibbonPanel.ResumeLayout();
            m_PopupRibbonPanel.Visible = true;
            m_PopupRibbonPanel.BringToFront();
        ...
        {
            CloseRibbonMenu(null, EventSource.Code);
        }

        /// <summary>
        /// Closes the Ribbon Menu if one is currently displayed.
        /// </summary>
        public void CloseRibbonMenu(object source, EventSource eventSource)
        {
            ...
            if (!this.Expanded)
            {
                if (m_MenuTabsEnabled)
                {
                    if (rt.Panel == m_PopupRibbonPanel)
                        CloseRibbonMenu(rt, EventSource.Mouse);
                    else
                        PopupRibbon(rt, EventSource.Mouse);
                }
                else
                    this.Expanded = true;
            }
        ...
        internal void RibbonTabItemDoubleClick(RibbonTabItem ribbonTabItem)
        {
            ...
            {
                this.CloseRibbonMenu(ribbonTabItem, EventSource.Code);
                this.Expanded = true;
            }
            else
                this.Expanded = false;
        ...
        /// </summary>
        [DefaultValue(true), Browsable(true), HVTTBrowsable(true), Category("Layout"), Description("Indicates whether control is collapsed when RibbonTabItem is double clicked and expanded when RibbonTabItem is clicked.")]
        ...
        private void SyncMdiChildSystemItem()
        {
            if (m_MdiSystemItemVisible && m_ActiveMdiChild != null &&
                m_ActiveMdiChild.WindowState == FormWindowState.Maximized && m_ActiveMdiChild.Visible)
            {
        ...
        internal virtual void ShowCustomizeContextMenu(object o, bool ribbonStrip)
        {
            ...
            cont.Style = HVTTControlStyle.Office2007;
        ...
        private ArrayList GetQatItems(bool includeCustomizeItem)
        {
            ...
            {
                if (IsSystemItem(item) || item is ButtonItem && ((ButtonItem)item).HotTrackingStyle == HotTrackingStyle.Image && this.QuickToolbarItems.IndexOf(item) == 0)
                {
        ...
        private BaseItem GetQatItemCopy(object o)
        {
            ...
            if (o is ButtonItem)
            {
                ...
                button.ButtonStyle = HVTTButtonStyle.Default;
        ...
        internal BaseItem GetStartButton()
        {
			return m_RibbonStrip.GetStartButton();
        }
        ...
        /// </summary>
        [Browsable(true), HVTTBrowsable(true), NotifyParentPropertyAttribute(true), Category("Localization"), Description("Gets system text used by the component.."), DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
