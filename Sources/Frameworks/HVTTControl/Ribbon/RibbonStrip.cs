...
namespace HVTT.UI.Window.Forms
{
	...
	public class RibbonStrip:ItemControl
	{
		...
		public RibbonStrip()
		{
            ...
            m_StripContainer.Style = HVTTControlStyle.Office2003;
            ...
            //m_ItemContainer.Style=HVTTControlStyle.Office2003;
			this.ColorScheme.Style=HVTTControlStyle.Office2003;
            ...
            //m_CaptionContainer.SetOwner(this);
            
            //m_CaptionContainer.Style = HVTTControlStyle.Office2003;
        ...
        internal override bool IsGlassEnabled
        {
            get
            {
                if (!m_CanSupportGlass) return false;
                return base.IsGlassEnabled;
            }
        }
        ...
        internal Rectangle QuickToolbarBounds
        {
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(14),Category("Tab Groups"),Description("Indicates height in pixels of tab group line that is displayed above the RibbonTabItem objects that have group assigned.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(false),Category("Tab Groups"),Description("Indicates height in pixels of tab group line that is displayed above the RibbonTabItem objects that have group assigned.")]
					...
					if(tab.Group!=currentGroup)
					{
						if(tab.Visible)
						{
							...
							if(currentGroup!=null)
							{
                                ...
                                groupRect = new Rectangle(tab.DisplayRectangle.X, y, tab.DisplayRectangle.Width, tab.DisplayRectangle.Y - 4); //m_TabGroupHeight);
		...
		private void PaintGroup(ItemPaintArgs pa, RibbonTabItemGroup group, Rectangle rect, Rectangle groupBounds)
		{
            ...
            {
                RibbonTabGroupRendererEventArgs e = new RibbonTabGroupRendererEventArgs(pa.Graphics, group, rect, groupBounds, this.GetDefaultGroupFont(), pa);
                Rendering.BaseRenderer renderer = GetRenderer();
        ...
        private int GetCaptionHeight()
        {
            if (m_CaptionHeight == 0)
            {
                ...
                if (this.IsGlassEnabled) captionHeight += 2;
        ...
        {
            return GetCaptionHeight() + m_TabGroupHeight;
        }
        ...
        protected override bool OnMouseWheel(IntPtr hWnd, IntPtr wParam, IntPtr lParam)
        {
            ...
            if (r.Contains(mousePos) && !this.ShowKeyTips && expanded)
            {
        ...
        //{
        //    base.OnMouseHover(e);
        //    if (m_CaptionVisible) m_CaptionContainer.InternalMouseHover();
        //}

        //protected override void OnMouseLeave(EventArgs e)
        //{
        //    if (m_CaptionVisible) m_CaptionContainer.InternalMouseLeave();
        //    base.OnMouseLeave(e);
        //}
		...
		internal BaseItem GetStartButton()
		{
			...
			if (cont.SubItems.Count > 0 && cont.SubItems[0] is ButtonItem && ((ButtonItem)cont.SubItems[0]).HotTrackingStyle == HotTrackingStyle.Image)
				return cont.SubItems[0];
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            if (m_CaptionVisible)
            {
                if (e.Button == MouseButtons.Left && m_MouseDownOnCaption && HitTestCaption(new Point(e.X, e.Y)))
				{
					...
					{
                        PopupItem popup = GetStartButton() as PopupItem;
        ...
        internal void ClearMDIChildSystemItems(bool bRecalcLayout)
        {
            ...
            try
            {
                if (this.Items.Contains("HVTTCONTROLSsysiconitem"))
                {
                    this.Items.Remove("HVTTCONTROLSsysiconitem");
                    recalc = true;
                }
                if (this.Items.Contains("HVTTCONTROLSsysmenuitem"))
                {
                    this.Items.Remove("HVTTCONTROLSsysmenuitem");
                    recalc = true;
                }
        ...
        internal void ShowMDIChildSystemItems(System.Windows.Forms.Form objMdiChild, bool bRecalcLayout)
        {
            ...
            MDISystemItem mdi = new MDISystemItem("HVTTCONTROLSsysmenuitem");
