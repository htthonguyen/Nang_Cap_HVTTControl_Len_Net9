...
namespace HVTT.UI.Window.Forms
{
    ...
    /// </summary>
	[ToolboxBitmap(typeof(HVTTManager),"HVTT.ico"), Designer(typeof(Design.HVTTManagerDesigner)),ToolboxItem(true),System.Runtime.InteropServices.ComVisible(false), DefaultEvent("ItemClick")]
	public class HVTTManager : Component,
        ...
        ICustomSerialization
	{
		...
		/// <summary>
		/// Occurs after HVTTCONTROLS definition is loaded.
		/// </summary>
		[System.ComponentModel.Description("Occurs after HVTTCONTROLS definition is loaded."),Category("General")]
		public event EventHandler DefinitionLoaded;

		/// <summary>
		/// Occurs when users wants to reset the HVTTCONTROLS to default state.
		/// </summary>
		[System.ComponentModel.Description("Occurs when users wants to reset the HVTTCONTROLS to default state."),Category("General")]
		...
		/// <summary>
		/// Use this event if you want to serialize the hosted control state directly into the HVTTCONTROLS definition file.
		/// </summary>
		public event ControlContainerItem.ControlContainerSerializationEventHandler ContainerControlSerialize;
		/// <summary>
		/// Use this event if you want to deserialize the hosted control state directly from the HVTTCONTROLS definition file.
		/// </summary>
		...
		private HVTTStreamer m_BarStreamer=null;
		...
		private HVTTMenuDropShadow m_MenuDropShadow=HVTTMenuDropShadow.SystemDefault;
		...
		private HVTTPopupAnimation m_PopupAnimation=HVTTPopupAnimation.SystemDefault;
		...
		private HVTTControlStyle m_Style=HVTTControlStyle.Office2003;

		private bool m_DefinitionCodeSerialize=false;

		private HVTTMarkStatus m_FocusedBar=null;
		...
		/// <summary>
		/// Creates new instance of HVTTManager.
		/// </summary>
		public HVTTManager():this(null) {}
		/// <summary>
		/// Creates new instance of HVTTManager.
		/// </summary>
		/// <param name="cont">Container.</param>
		public HVTTManager(IContainer cont)
		{
        ...
        internal HVTTManager(IContainer cont, bool dnbe)
        {
            InitializeComponent(cont);
        }
        ...
        /// </summary>
        [DefaultValue(null), Description("Indicates Context menu bar associated with the HVTTManager which is used as part of Global Items feature."), Category("Data")]
		...
		/// <summary>
		/// Gets or sets the form HVTTManager is attached to.
		/// </summary>
		[System.ComponentModel.Browsable(true),System.ComponentModel.Category("Data"),System.ComponentModel.Description("Gets or sets the form HVTTManager is attached to.")]
		...
		/// <summary>
		/// Gets or sets the user control HVTTManager is parented to when on user control and providing popups only.
		/// </summary>
		...
		internal void InvokeDockTabClosing(HVTTMarkStatus bar, DockTabClosingEventArgs e)
		{
		...
		public HVTT.UI.Window.Forms.HVTTMarkStatus GetItemBar(BaseItem item)
		{
			while(item.Parent!=null)
				item=item.Parent;
			HVTTMarkStatus bar=item.ContainerControl as HVTTMarkStatus;
			return bar;
		}
...
//			{
//                // Try to get to the MDI form and register with that HVTTManager
//				if(m_ParentForm.MdiParent!=null)
//				{
//					HVTTManager mdiManager=null;
...
// TODO: Menu Merging Implementation
//		internal void RegisterMdiChildManager(HVTTManager mdiChildManager)
//		{
...
//		}
//
//		internal void UnregisterMdiChildManager(HVTTManager mdiChildManager)
//		{
//			if(m_MdiChildManagers==null)
//				return;
...
//							{
//								if(MessageBox.Show("Definition file is not included in existing projects. "+m_DefinitionName+" but it is found at: "+path+"\n\nDo you want HVTTCONTROLS to add it to the project?","HVTTCONTROLS Definition Load",MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
//									DesignTimeDte.AddFileToProject(path+m_DefinitionName);
//							}
						}
						else
						{
							MessageBox.Show("HVTTCONTROLS definition file not found: "+path+m_DefinitionName,"HVTTCONTROLS Designer",MessageBoxButtons.OK,MessageBoxIcon.Warning);
                            m_DefinitionName="";							
						}
					}
				}
		...
		void IOwner.OnApplicationDeactivate()
		{
			...
			if(m_WereVisible.Count==0)
			{
				if(m_Bars!=null)
				{
					...
					foreach(HVTTMarkStatus bar in m_Bars)
					{
					...
					foreach(HVTTMarkStatus bar in m_Bars)
					{
						...
						if(bar.BarState==HVTTBarState.Floating && bar.Visible && bar.HideFloatingInactive)
						{
							//bar.Visible=false;
							bar.HideBar();
			...
			HVTTMarkStatus menuBar=this.GetMenuBar();
...
//			{
//				foreach(HVTTManager m in ms_Managers)
//				{
//					if(m!=this)
//						m.App_LostFocus();
//				}
...
//		}
		...
		void IOwner.OnApplicationActivate()
		{
			...
			try
			{
				foreach(HVTTMarkStatus bar in m_WereVisible)
				{
					bar.ShowBar();
				}
			}
...
//			}
			...
			// Closing the form which contains dockable window which in turn contains the UserControl with HVTTCONTROLS and 
			// when HVTTCONTROLS is used to provide context menu for that UserControl
...
//			}
			...
			//foreach(HVTT.UI.Window.Forms.Bar  bar in m_Bars)
			//	bar.Dispose();
			if(m_Bars!=null)
			{
				//m_Bars.Clear();
		...
		/// <summary>
		/// Indicates whether HVTTCONTROLS provides docking hints for easy docking of bars.
		/// </summary>
		[Obsolete(), Browsable(false),DefaultValue(true), EditorBrowsable(EditorBrowsableState.Never), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),  Category("Docking"),Description("Indicates whether HVTTCONTROLS provides docking hints for easy docking of bars.")]
		...
		/// <summary>
		/// Use to remove bar from HVTTCONTROLS control. Bar will be undocked if it is docked and
		/// removed from all internal collections.
		/// </summary>
		/// <param name="bar">Bar to remove.</param>
		public void RemoveBar(HVTTMarkStatus bar)
		{
			if(!m_Bars.Contains(bar))
				throw new System.ArgumentException("Bar not part of Bars collections.");
			m_Bars.Remove(bar);
			if(m_TopDockSite!=null && m_TopDockSite.Contains(bar))
				m_TopDockSite.RemoveBar(bar);
			else if(m_BottomDockSite!=null && m_BottomDockSite.Contains(bar))
				m_BottomDockSite.RemoveBar(bar);
			else if(m_LeftDockSite!=null && m_LeftDockSite.Contains(bar))
				m_LeftDockSite.RemoveBar(bar);
			else if(m_RightDockSite!=null && m_RightDockSite.Contains(bar))
				m_RightDockSite.RemoveBar(bar);
			((IOwnerBarSupport)this).RemoveShortcutsFromBar(bar);
		}

		void IOwnerBarSupport.AddShortcutsFromBar(HVTTMarkStatus bar)
		{
		...
		void IOwnerBarSupport.RemoveShortcutsFromBar(HVTTMarkStatus bar)
		{
			IOwner owner=this as IOwner;
			if(bar.Items==null)
				return;
		...
		//[Editor(typeof(HVTTCONTROLSEditor), typeof(System.Drawing.Design.UITypeEditor)),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		/// <summary>
		/// Gets the collection of the Bar objects associated with HVTTManager.
		/// </summary>
		...
		public HVTT.UI.Window.Forms.Bars Bars
		{
			get
			{
				return m_Bars;
		...
		[Browsable(false),HVTTBrowsable(false),EditorBrowsable(EditorBrowsableState.Never)]
		...
		/// However, under certain circumstances you might need HVTTCONTROLS to load definition right away so
		/// you can access bars and items. We recommend moving the code to Form Load event and leaving the loading process for HVTTCONTROLS definition as is.
		/// </summary>
		...
		/// <summary>
		/// Gets the collection of all popup menus managed by HVTTManager.
		/// </summary>
		[Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Content), Obsolete("ContextMenus on HVTTManager is removed. Use ContextMenuBar control instead.")]
		...
		DockSiteInfo IOwnerBarSupport.GetDockInfo(IDockInfo pDock, int x, int y)
		{
			...
			if(((HVTTMarkStatus)pDock).LayoutType==HVTTLayoutType.DockContainer)
			{
		...
		private bool CanDockToBar(IDockInfo barDockInfo, HVTTMarkStatus referenceBar)
		{
		...
		private bool DockingHintHandler(ref DockSiteInfo dockInfo, IDockInfo barDockInfo, int x, int y)
		{
			...
			HVTTMarkStatus barMouseOver=null;
			foreach(HVTTMarkStatus bar in this.Bars)
			{
				if(bar.Visible && (bar!=barDockInfo || this.DesignMode || bar.DockSide==eDockSide.Document))
				{
					Point p=bar.PointToClient(new Point(x,y));
					if(bar.LayoutType==HVTTLayoutType.DockContainer && bar.ClientRectangle.Contains(p) && bar.DockSide!=eDockSide.None && CanDockToBar(barDockInfo, bar))
					{
						barMouseOver=bar;
						break;
					}
...
//							}
		...
		private void SetupDockingHintWindows(HVTTMarkStatus barMouseOver, IDockInfo barDockInfo)
		{
			...
			if(!this.DesignMode)
			{
				...
				if(barDockInfo.CanDockLeft)
				{
					if(m_DockingHintLeft==null)
					{
						m_DockingHintLeft=new DockingHint(eDockingHintSide.Left);
				...
				if(barDockInfo.CanDockRight)
				{
					if(m_DockingHintRight==null)
					{
						m_DockingHintRight=new DockingHint(eDockingHintSide.Right);
				...
				if(barDockInfo.CanDockTop)
				{
					if(m_DockingHintTop==null)
					{
						m_DockingHintTop=new DockingHint(eDockingHintSide.Top);
				...
				if(barDockInfo.CanDockBottom)
				{
					if(m_DockingHintBottom==null)
					{
						m_DockingHintBottom=new DockingHint(eDockingHintSide.Bottom);
			...
			{
				if(barMouseOver!=null)
					m_DockingHintMiddle=new DockingHint(hintSides,true);
				else
                    m_DockingHintMiddle=new DockingHint(hintSides,true);
				bShowMidHint=true;
			}
		...
		/// <summary>
		/// Gets or sets the collection of shortcut keys that are automatically dispatched to the control that has focus even if they are handled and used by one of the items. This gives you fine control over which shortcuts are passed through the system and which ones are marked as handled by HVTTCONTROLS.
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Behavior"),System.ComponentModel.Description("Indicates list of shortcut keys that are automatically dispatched to the control that has focus even if they are handled and used by one of the items."),System.ComponentModel.Editor(typeof(Design.ShortcutsDesigner),typeof(System.Drawing.Design.UITypeEditor)),System.ComponentModel.TypeConverter(typeof(Design.ShortcutsConverter)),System.ComponentModel.DesignerSerializationVisibility(System.ComponentModel.DesignerSerializationVisibility.Content)]
...
//		}
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),DefaultValue(HVTTMenuDropShadow.SystemDefault),System.ComponentModel.Category("Behavior"),System.ComponentModel.Description("Specifes whether drop shadow is displayed for Menus and pop-up Bars. OfficeXP Style only.")]
		public HVTTMenuDropShadow MenuDropShadow
		{
			get
			{
		...
		/// <summary>
		/// Gets or sets whether hooks are used for internal HVTTCONTROLS system functionality. Using hooks is recommended only if HVTTCONTROLS is used in hybrid environments like Visual Studio designers or IE.
		/// </summary>
		[System.ComponentModel.Browsable(true),DefaultValue(false),System.ComponentModel.Category("Behavior"),System.ComponentModel.Description("Gets or sets whether hooks are used for internal HVTTCONTROLS system functionality. Using hooks is recommended only if HVTTCONTROLS is used in hybrid environments like Visual Studio designers or IE.")]
		public bool UseHook
		{
			get
			{
		...
		bool IOwnerMenuSupport.ShowPopupShadow
		{
			get
			{
				if(m_MenuDropShadow==HVTTMenuDropShadow.Show)
					return true;
				else if(m_MenuDropShadow==HVTTMenuDropShadow.Hide)
					return false;
				// W2K and gang
		...
		/// <summary>
		/// Loads HVTTCONTROLS definition from file.
		/// </summary>
		/// <param name="FileName">File that contains HVTTCONTROLS defintion.</param>
		...
		{
            System.Xml.XmlElement xmlHVTTCONTROLS=xmlDoc.FirstChild as System.Xml.XmlElement;

            // Do not load old uncompatible definitions
            if (xmlHVTTCONTROLS != null && (!xmlHVTTCONTROLS.HasAttribute(DocumentSerializationXml.Version) ||
               XmlConvert.ToInt32(xmlHVTTCONTROLS.GetAttribute(DocumentSerializationXml.Version)) < 6))
                return;
			...
			try
			{
				...
				if(xmlHVTTCONTROLS.Name!="HVTTCONTROLS")
					throw new System.InvalidOperationException("Invalid file format (HVTTCONTROLS).");

//				if(xmlHVTTCONTROLS.HasAttribute("zorder"))
//					RestoreDockSiteZOrder(xmlHVTTCONTROLS.GetAttribute("zorder"));
			
				// Load user defined settings
				if(xmlHVTTCONTROLS.HasAttribute("fullmenus"))
					m_AlwaysShowFullMenus=System.Xml.XmlConvert.ToBoolean(xmlHVTTCONTROLS.GetAttribute("fullmenus"));
				else
					m_AlwaysShowFullMenus=false;
				if(xmlHVTTCONTROLS.HasAttribute("fullmenushover"))
					m_ShowFullMenusOnHover=System.Xml.XmlConvert.ToBoolean(xmlHVTTCONTROLS.GetAttribute("fullmenushover"));
				else
					m_ShowFullMenusOnHover=true;
				if(xmlHVTTCONTROLS.HasAttribute("tooltips"))
					m_ShowToolTips=System.Xml.XmlConvert.ToBoolean(xmlHVTTCONTROLS.GetAttribute("tooltips"));
				else
					m_ShowToolTips=true;
				if(xmlHVTTCONTROLS.HasAttribute("scintooltip"))
					m_ShowShortcutKeysInToolTips=System.Xml.XmlConvert.ToBoolean(xmlHVTTCONTROLS.GetAttribute("scintooltip"));
				else
					m_ShowShortcutKeysInToolTips=false;
				if(xmlHVTTCONTROLS.HasAttribute("animation"))
				{
					if((HVTTPopupAnimation)System.Xml.XmlConvert.ToInt32(xmlHVTTCONTROLS.GetAttribute("animation"))!=HVTTPopupAnimation.SystemDefault)
						m_PopupAnimation=(HVTTPopupAnimation)System.Xml.XmlConvert.ToInt32(xmlHVTTCONTROLS.GetAttribute("animation"));
				}
				else
					m_PopupAnimation=HVTTPopupAnimation.SystemDefault;
				...
				foreach(System.Xml.XmlElement xmlElem in xmlHVTTCONTROLS.ChildNodes)
				{
					...
					else if(xmlElem.Name=="bars")
					{
						...
						{
							HVTT.UI.Window.Forms.HVTTMarkStatus bar=new HVTT.UI.Window.Forms.HVTTMarkStatus();
							bar.Visible=false;
		...
		/// <summary>
		/// Gets or sets the HVTTCONTROLS definition string.
		/// </summary>
		[Browsable(false),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		...
		/// <summary>
		/// Gets or sets the HVTTCONTROLS layout string.
		/// </summary>
		[Browsable(false),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		...
		/// <summary>
		/// Saves current HVTTCONTROLS definition and state to the file.
		/// </summary>
		...
		internal void SaveDefinition(System.Xml.XmlDocument xmlDoc)
		{
			...
			System.Xml.XmlElement xmlHVTTCONTROLS=xmlDoc.CreateElement("HVTTCONTROLS");
            xmlHVTTCONTROLS.SetAttribute(DocumentSerializationXml.Version, "6");
			xmlDoc.AppendChild(xmlHVTTCONTROLS);

			// Save user settings
			xmlHVTTCONTROLS.SetAttribute("fullmenus",System.Xml.XmlConvert.ToString(m_AlwaysShowFullMenus));
			xmlHVTTCONTROLS.SetAttribute("fullmenushover",System.Xml.XmlConvert.ToString(m_ShowFullMenusOnHover));
			xmlHVTTCONTROLS.SetAttribute("tooltips",System.Xml.XmlConvert.ToString(m_ShowToolTips));
			xmlHVTTCONTROLS.SetAttribute("scintooltip",System.Xml.XmlConvert.ToString(m_ShowShortcutKeysInToolTips));
			xmlHVTTCONTROLS.SetAttribute("animation",System.Xml.XmlConvert.ToString((int)m_PopupAnimation));

			System.Xml.XmlElement xmlItems=xmlDoc.CreateElement("items");
			xmlHVTTCONTROLS.AppendChild(xmlItems);
			...
			xmlHVTTCONTROLS.AppendChild(xmlBars);
			
			// Serialize docked bars
			if(m_TopDockSite!=null)
                SerializeDockSite(m_TopDockSite, xmlHVTTCONTROLS);
			if(m_BottomDockSite!=null)
                SerializeDockSite(m_BottomDockSite, xmlHVTTCONTROLS);
			if(m_LeftDockSite!=null)
                SerializeDockSite(m_LeftDockSite, xmlHVTTCONTROLS);
			if(m_RightDockSite!=null)
                SerializeDockSite(m_RightDockSite, xmlHVTTCONTROLS);
			if(m_IncludeDockDocumentsInDefinition && m_FillDockSite!=null)
                SerializeDockSite(m_FillDockSite,xmlHVTTCONTROLS);
			...
			foreach(HVTT.UI.Window.Forms.HVTTMarkStatus bar in m_Bars)
			{
			...
			xmlHVTTCONTROLS.AppendChild(xmlPopups);
		...
		/// <summary>
		/// Loads HVTTCONTROLS layout from file.
		/// </summary>
		/// <param name="FileName">File that contains HVTTCONTROLS defintion.</param>
		...
		internal void LoadLayout(System.Xml.XmlDocument xmlDoc)
		{
			...
			try
			{
				if(xmlHVTTCONTROLS.Name!="HVTTCONTROLSlayout")
					throw new System.InvalidOperationException("Invalid file format (HVTTCONTROLSlayout expected).");
                ...
                HVTTMarkStatus[] bars = new HVTTMarkStatus[m_Bars.Count];
                m_Bars.CopyTo(bars);
        ...
        /// <summary>
        /// Gets whether HVTTManager is loading layout.
        /// </summary>
        [Browsable(false)]
		...
		/// <summary>
		/// Saves current HVTTCONTROLS layout to the file.
		/// </summary>
		/// <param name="FileName">File name.</param>
		...
		{
			// Save first items from categories
			System.Xml.XmlElement xmlHVTTCONTROLS=xmlDoc.CreateElement("HVTTCONTROLSlayout");
            xmlHVTTCONTROLS.SetAttribute(DocumentSerializationXml.Version, "6");
			xmlDoc.AppendChild(xmlHVTTCONTROLS);
			xmlHVTTCONTROLS.SetAttribute("zorder",GetDockSitesZOrder());
			
			// Serialize docked bars
			if(m_TopDockSite!=null)
                SerializeDockSiteLayout(m_TopDockSite, xmlHVTTCONTROLS);
			if(m_BottomDockSite!=null)
                SerializeDockSiteLayout(m_BottomDockSite, xmlHVTTCONTROLS);
			if(m_LeftDockSite!=null)
                SerializeDockSiteLayout(m_LeftDockSite, xmlHVTTCONTROLS);
			if(m_RightDockSite!=null)
                SerializeDockSiteLayout(m_RightDockSite, xmlHVTTCONTROLS);
			if(m_FillDockSite!=null)
                SerializeDockSiteLayout(m_FillDockSite, xmlHVTTCONTROLS);
		...
		private void SerializeDockSite(DockSite site, System.Xml.XmlElement xmlBars)
		{
			...
			else
			{
				...
				{
					HVTT.UI.Window.Forms.HVTTMarkStatus bar=ctrl as HVTT.UI.Window.Forms.HVTTMarkStatus;
					if(bar!=null)
					{
        ...
        //[Browsable(false),EditorBrowsable(EditorBrowsableState.Never)]
        //public bool SaveDesignDefinition()
        //{
        //    bool ret=false;
        //    try
        //    {
        //        string path=DesignTimeDte.GetActiveDocumentPath(this.Site);
        //        if(this.DefinitionName!="" && !System.IO.File.Exists(path+this.DefinitionName))
        //            path=DesignTimeDte.GetDefinitionPath(this.DefinitionName, this.Site);
        //        if(path!=null && path!="")
        //        {
        //            if(!path.EndsWith("\\"))
        //                path+="\\";
        ...
        //        else
        //            path="";
        //        string defFileName=this.DefinitionName;

        //        if(defFileName=="")
        //        {
        //            // First time deal, add the HVTTCONTROLS definition to the project
        //            if(m_ParentUserControl!=null)
        //                defFileName=m_ParentUserControl.Site.Name+"."+this.Site.Name+".xml";
        //            else if(m_TopDockSite!=null && m_TopDockSite.Parent!=null && m_TopDockSite.Parent is UserControl)
        //                defFileName=m_TopDockSite.Parent.Site.Name+"."+this.Site.Name+".xml";
        //            else if(this.ParentForm!=null)
        //                defFileName=this.ParentForm.Site.Name+"."+this.Site.Name+".xml";
        //        }
        //        // Save the definition
        //        if(defFileName!="" && path!="")
        //        {
        //            if(System.IO.File.Exists(path+defFileName) && (System.IO.File.GetAttributes(path+defFileName) & System.IO.FileAttributes.ReadOnly)==System.IO.FileAttributes.ReadOnly)
        //            {
        //                DesignTimeDte.CheckOutFile(defFileName, this.Site);
        //                if((System.IO.File.GetAttributes(path+defFileName) & System.IO.FileAttributes.ReadOnly)==System.IO.FileAttributes.ReadOnly)
        //                {
        //                    if(MessageBox.Show("HVTTCONTROLS definition file is read-only. This could be due to the Source Safe integration. Would you like HVTTCONTROLS to save definition file? \n (You should manually check out the '"+defFileName+"' file and make sure that you do not overwrite the version that is currently on the disk.","HVTTCONTROLS Designer",MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
        //                    {
        //                        System.IO.File.SetAttributes(path+defFileName,System.IO.FileAttributes.Normal);
        //                    }
        //                }
        //            }
        //            if(System.IO.File.Exists(path+defFileName))
        //                System.IO.File.Copy(path+defFileName,path+defFileName+"_dnb.bak",true);
        //            this.SaveDefinition(path+defFileName);
        //        }
        //        if(this.DefinitionName=="")
        //        {
        //            if(!DesignTimeDte.AddFileToProject(path+defFileName, this.Site))
        //                return false;
        //            // VS.NET 2005 B2 workaround. Including file in project sets file length to 0
        //            this.SaveDefinition(path+defFileName);
        //        }
        //        this.DefinitionName=defFileName;
        //        ret=true;
        //        // Set Dirty flag in designer
        //        System.ComponentModel.Design.IComponentChangeService cc=(System.ComponentModel.Design.IComponentChangeService)GetService(typeof(System.ComponentModel.Design.IComponentChangeService));
        //        if(cc!=null)
        //        {
        //            try
        //            {
        //                cc.OnComponentChanging(this,(TypeDescriptor.GetProperties(this)["DefinitionName"]));
        //                cc.OnComponentChanged(this,(TypeDescriptor.GetProperties(this)["DefinitionName"]),null,this.DefinitionName);
        //            }
        //            catch{}
        //        }
        //    }
        //    catch(Exception e)
        //    {
        //        MessageBox.Show("Could not save design-time definition file due to the error: "+e.ToString()+"\nStream will be used to save definition.","HVTTCONTROLS Designer",MessageBoxButtons.OK,MessageBoxIcon.Error);
        //        this.DefinitionName="";
        //    }
        //    return ret;
        //}
		...
		/// Invokes the HVTTCONTROLS Customize dialog.
		/// </summary>
		public void Customize(Design.HVTTManagerDesigner designerservices)
		{
		...
		/// </summary>
		/// <param name="ItemName">Item name to look for.</param>
		/// <param name="FullSearch">Specifies that search will be performed through all HVTTCONTROLS collections.</param>
		/// <returns></returns>
		...
		void IOwner.OnParentPositionChanging()
		{
			if(m_Bars!=null)
			{
				HVTTMarkStatus[] arr=new HVTTMarkStatus[m_Bars.Count];
				m_Bars.CopyTo(arr);
				foreach(HVTTMarkStatus bar in arr)
				{
					if(!bar.IsDisposed && bar.IsHandleCreated && bar.BarState==HVTTBarState.Floating)
					{
		...
		/// <summary>
		/// Registers popup item with HVTTCONTROLS. Use this function carefully. The registration is required only if Popup item is created completely from code and it is not added to any HVTTManager collection.
		/// </summary>
        ...
        /// <summary>
		/// Gets or sets the toolbar Top dock site used by HVTTManager.
		/// </summary>
        [Browsable(false), DefaultValue(null)]
        ...
        /// <summary>
        /// Gets or sets the toolbar Bottom dock site used by HVTTManager.
        /// </summary>
        [Browsable(false), DefaultValue(null)]
        ...
        /// <summary>
        /// Gets or sets the toolbar Left dock site used by HVTTManager.
        /// </summary>
        [Browsable(false), DefaultValue(null)]
        ...
        /// <summary>
        /// Gets or sets the toolbar Right dock site used by HVTTManager.
        /// </summary>
        [Browsable(false), DefaultValue(null)]
		...
		/// <summary>
		/// Gets or sets the Top dock site used by HVTTManager.
		/// </summary>
		[Browsable(false)]
		...
		/// <summary>
		/// Gets or sets the Bottom dock site used by HVTTManager.
		/// </summary>
		[Browsable(false)]
		...
		/// <summary>
		/// Gets or sets the Left dock site used by HVTTManager.
		/// </summary>
		[Browsable(false)]
		...
		/// <summary>
		/// Gets or sets the Right dock site used by HVTTManager.
		/// </summary>
		[Browsable(false)]
		...
		/// <summary>
		/// Gets or sets the Fill dock site used by HVTTManager. Fill dock site is most commonly used as dock site
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(false),DefaultValue(false),Category("Appearance"),Description("Specifies whether new bars created are drawn using Themes when running on OS that supports themes like Windows XP.")]
		...
		private void SetupParentUserControl()
		{
			...
			// The whole designer was reporting exception when UserControl with HVTTCONTROLS is opened in design-mode, switched to code, code changed, switched back to design mode.
		...
		/// <summary>
		/// Sets the style of all items in HVTTCONTROLS Manager.
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Color Scheme"),System.ComponentModel.Description("Specifies the default visual style of the Bars.")]
		...
		/// that this ColorScheme object will be used on all bars managed by this instance of HVTTManager.
		/// </summary>
		...
		/// <summary>
		/// Gets or sets whether ColorScheme object on HVTTManager is used as a default ColorScheme for all bars managed by HVTTManager.
		/// Default value is false which indicates that ColorScheme on each Bar is used.
		/// When set to true each bar will use the ColorScheme settings from HVTTManager and it will ignore any
		/// setting on the Bar.ColorScheme object.
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Color Scheme"),Description("Indicates whether ColorScheme object on HVTTManager is used as a default ColorScheme for all bars managed by HVTTManager."),DefaultValue(false)]
		...
		public bool AlwaysDisplayKeyAccelerators
		{
			...
			set
			{
				...
				HVTTMarkStatus bar=this.GetMenuBar();
				if(bar!=null)
					bar.Refresh();
			}
		}
		...
		/// <summary>
		/// Gets or sets whether HVTTCONTROLS ignores the F10 key which when pressed sets the focus to menu bar
		/// </summary>
		[System.ComponentModel.Browsable(true),DefaultValue(false),System.ComponentModel.Category("Run-time Behavior"),System.ComponentModel.Description("Indicates whether HVTTCONTROLS ignores the F10 key which when pressed sets the focus to menu bar.")]		
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),DefaultValue(HVTTPopupAnimation.SystemDefault),System.ComponentModel.Category("Run-time Behavior"),System.ComponentModel.Description("Specifies the pop-up animation style.")]
        ...
        /// <summary>
        /// Informs the HVTTManager that Mdi Child for has been activated. Calling this method is needed only under special
		...
		private void OnMdiChildActivate(object sender,System.EventArgs e)
		{
			...
			{
				HVTT.UI.Window.Forms.HVTTMarkStatus bar=GetMenuBar();
				if(bar!=null)
					bar.ClearMDIChildSystemItems(true);
			}
		}
		...
		private HVTT.UI.Window.Forms.HVTTMarkStatus GetMenuBar()
		{
			if(m_Bars==null)
				return null;
			try
		...
		internal HVTTMarkStatus FocusedBar
		{
		...
		bool IMessageHandlerClient.OnSysKeyDown(IntPtr hWnd, IntPtr wParam, IntPtr lParam)
		{
			...
			if((System.Windows.Forms.Control.ModifierKeys & Keys.Alt)==Keys.Alt && wParam.ToInt32()>0x1B && m_Bars!=null)
			{
				...
				if(key!=0)
				{
					foreach(HVTTMarkStatus b in m_Bars)
					{
		...
		bool IMessageHandlerClient.OnSysKeyUp(IntPtr hWnd, IntPtr wParam, IntPtr lParam)
		{
			...
			if(wParam.ToInt32()==18 || wParam.ToInt32()==121)
			{
				...
				foreach(HVTTMarkStatus b in this.Bars)
				{
		...
		bool IMessageHandlerClient.OnKeyDown(IntPtr hWnd, IntPtr wParam, IntPtr lParam)
		{
			...
			if(this.FocusedBar!=null && !designMode)
			{
				...
				if(ctrl2!=null)
				{
					while(ctrl2.Parent!=null)
						ctrl2=ctrl2.Parent;
					if((ctrl2 is MenuPanel || ctrl2 is HVTTMarkStatus || ctrl2 is PopupContainer || ctrl2 is PopupContainerControl) && ctrl2.Handle!=hWnd)
						bPassToMenu=false;
				}
...
//			return false;
			...
			if(m_RegisteredPopups.Count>0)
			{
				...
				foreach(BaseItem item in m_RegisteredPopups)
				{
					...
					else if(c!=null)
					{
						...
						if(topParent is HVTTMarkStatus || topParent is PopupContainerControl || topParent is PopupContainer)
							c=topParent;
					}
		...
		void IOwnerBarSupport.BarContextMenu(Control bar, MouseEventArgs e)
		{
			...
			ButtonItem btn;
			HVTTControlStyle style=HVTTControlStyle.OfficeXP;
			if(m_Bars.Count>0)
				style=m_Bars[0].Style;
			m_ContextMenu.Style=style;
			foreach(HVTTMarkStatus b in m_Bars)
			{
                if (b.LayoutType == HVTTLayoutType.DockContainer && b.Items.Count == 0) continue;
				// TODO: Menu Merge implementation
				//if(b.CanHide && (!m_ParentForm.IsMdiChild || !b.MergeEnabled ))
				if(b.CanHide && (b.LayoutType==HVTTLayoutType.DockContainer && b.CanCustomize || b.LayoutType!=HVTTLayoutType.DockContainer) && b.Text!="")
				{
					btn=new ButtonItem();
					btn.Text=b.Text;
		...
		private void CustomizeItemClick(object sender, System.EventArgs e)
		{
			...
			{
				HVTTMarkStatus bar=objItem.Tag as HVTTMarkStatus;
				if(bar.AutoHide)
					bar.AutoHide=false;
                if (!bar.Visible && bar.LayoutType == HVTTLayoutType.DockContainer && bar.VisibleItemCount == 0 && bar.Items.Count > 0)
		...
		/// <summary>
		/// Resets all usage data collected by HVTTCONTROLS in relation to the Personalized menus.
		/// </summary>
		public void ResetUsageData()
		{
			foreach(HVTTMarkStatus bar in m_Bars)
			{
				foreach(BaseItem item in bar.Items)
				{
					...
					if(ipm!=null && ipm.MenuVisibility==HVTTMenuVisibility.VisibleIfRecentlyUsed)
		...
		void IOwnerBarSupport.InvokeBarDock(HVTTMarkStatus bar,EventArgs e)
		{
			if(BarDock!=null)
				BarDock(bar,e);
		}

		void IOwnerBarSupport.InvokeBarUndock(HVTTMarkStatus bar,EventArgs e)
		{
			if(BarUndock!=null)
				BarUndock(bar,e);
		}
...
//		}
		...
		void IOwnerBarSupport.InvokeDockTabChange(HVTTMarkStatus bar,DockTabChangeEventArgs e)
		{
			if(DockTabChange!=null)
				DockTabChange(bar,e);
		}

		void IOwnerBarSupport.InvokeBarClosing(HVTTMarkStatus bar,BarClosingEventArgs e)
		{
			if(BarClosing!=null)
				BarClosing(bar,e);
		}

		void IOwnerBarSupport.InvokeBarTearOff(HVTTMarkStatus bar, EventArgs e)
		{
			if(BarTearOff!=null)
                BarTearOff(bar,e);
		}

		void IOwnerBarSupport.InvokeAutoHideChanged(HVTTMarkStatus bar,EventArgs e)
		{
			if(AutoHideChanged!=null)
				AutoHideChanged(bar,e);
		}

		void IOwnerBarSupport.InvokeAutoHideDisplay(HVTTMarkStatus bar,AutoHideDisplayEventArgs e)
		{
...
//        }
		...
		/// <summary>
		/// HVTTCONTROLS Design-Time editor
		/// </summary>
//		private class HVTTCONTROLSEditor:System.Drawing.Design.UITypeEditor
//		{
...
//					{
//						Bars b=null;
//						HVTTManager ctrl=context.Instance as HVTTManager;
//						if(ctrl!=null)
//							b=ctrl.Bars;
...
//		}
	...
	internal class ParentMsgHandler:NativeWindow
	{
		...
		//private HVTTManager m_BarControl;
		...
		protected override void WndProc(ref Message m)
		{
			...
			else if(m.Msg==NativeFunctions.WM_ACTIVATEAPP)
			{
				...
				{
					HVTTManager.RelayApplicationActivate(true);
					//ApplicationActivate();
				}
				else
				{
					HVTTManager.RelayApplicationActivate(false);
			...
			else if(m.Msg==NativeFunctions.WM_ACTIVATE)
			{
				if((m.WParam.ToInt32() & 0xFF)==NativeFunctions.WA_INACTIVE)
				{
					...
					{
						if(manager is HVTTManager && ((HVTTManager)manager).IsCustomizing)
						{
							customizing=true;
							break;
						}
					...
					if(ctrl!=null && !customizing)
					{
						while(ctrl.Parent!=null)
							ctrl=ctrl.Parent;
						if(!(ctrl is HVTT.UI.Window.Forms.HVTTMarkStatus || ctrl is HVTT.UI.Window.Forms.MenuPanel || ctrl is HVTT.UI.Window.Forms.PopupContainerControl || ctrl is HVTT.UI.Window.Forms.PopupContainer))
						{
							if(m_Managers.Count>0)
							{
								IOwner[] managers;
	...
	public class EndUserCustomizeEventArgs:EventArgs
	{
		...
		/// </summary>
		public readonly EndUserCustomizeAction Action;
		/// <summary>
		/// Default constructor.
		/// </summary>
		/// <param name="action">Indicates action user executed.</param>
		public EndUserCustomizeEventArgs(EndUserCustomizeAction action)
		{
			this.Action=action;
		}
	}
