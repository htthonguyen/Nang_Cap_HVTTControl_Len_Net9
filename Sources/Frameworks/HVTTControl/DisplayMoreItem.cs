...
namespace HVTT.UI.Window.Forms
{
    ...
    public class DisplayMoreItem:PopupItem
    {
		...
		private HVTTOrientation m_OldOrientation=HVTTOrientation.Horizontal;
        ...
        public DisplayMoreItem()
        {
			...
			// HVTTManager owner is not known and LocalizeString event cannot be invoked
		...
		private void PaintThemed(ItemPaintArgs pa)
		{
			...
			ThemeRHVTTBarStates state=ThemeRHVTTBarStates.ChevronNormal;
			if(m_Expanded)
				state=ThemeRHVTTBarStates.ChevronPressed;
			else if(m_MouseOver)
				state=ThemeRHVTTBarStates.ChevronHot;
		...
		protected virtual void AddItems()
		{
			...
			foreach(BaseItem objItem in m_SubItems)
			{
				...
				objItem.SetOrientation(HVTTOrientation.Horizontal);
        ...
        {
            if (this.PopupControl is HVTTMarkStatus)
            {
                ((HVTTMarkStatus)this.PopupControl).ParentItem = null;
            }
