...
namespace HVTT.UI.Window.Forms
{
	...
	/// </summary>
	[ToolboxItem(true),Designer(typeof(Design.TabStripDesigner)),ComVisible(false)]
	public class TabStrip : Control
	{
		...
		private HVTTMarkStatus m_DragBar=null;
...
//		private Color m_HotBorderDarkColor=Color.Empty;
		...
		private TabLayoutType m_TabLayoutType=TabLayoutType.FitContainer;
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			{
				if(m_NeedRecalcSize)
                    this.RecalcSize(e.Graphics, GetTabClientArea(this.DisplayRectangle, (this.TabLayoutType == TabLayoutType.MultilineWithNavigationBox), false));
...
//				}
		...
		private void PaintOneNote(Graphics g)
		{
			...
			if(selected!=null && selected.PredefinedColor!=TabItemColor.Default && !selected.BackColor2.IsEmpty)
			...
			get
			{
                if (m_Style == eTabStripStyle.SimulatedTheme || m_Style == eTabStripStyle.Office2007Document || m_Style == eTabStripStyle.Office2007Dock)
                {
                    ...
                    if (m_Alignment == eTabStripAlignment.Top || m_Alignment == eTabStripAlignment.Bottom)
                        ...
                        return (m_TabItemsBounds.Width + 1);
                }
		...
		public void RecalcSize()
		{
			...
			try
			{
                RecalcSize(g, GetTabClientArea(this.DisplayRectangle, (this.TabLayoutType == TabLayoutType.MultilineWithNavigationBox), false));
			}
			finally
			{
				g.Dispose();
		...
		private void RecalcSizeContentManager(Graphics g, Rectangle dispRect)
		{
			...
			if(m_Style!=eTabStripStyle.RoundHeader)
			{
				if(m_TabLayoutType==TabLayoutType.FitContainer)
				{
					m.FitContainerOversize=true;
                    m.OversizeDistribute = true;
					m.MultiLine=false;
				}
				else if(m_TabLayoutType==TabLayoutType.FixedWithNavigationBox)
				{
					m.FitContainerOversize=false;
					m.MultiLine=false;
				}
				else if(m_TabLayoutType==TabLayoutType.MultilineNoNavigationBox || m_TabLayoutType==TabLayoutType.MultilineWithNavigationBox)
				{
					m.FitContainerOversize=false;
					m.MultiLine=true;
				}
		...
		private void RecalcSizeLegacy(Graphics g, Rectangle dispRect)
		{
			...
			if(iWidth>iMaxWidth && m_TabLayoutType==TabLayoutType.FitContainer && !this.IsMultiLine)
			{
				if(iDisplayed>0)
				{
					if(m_Alignment==eTabStripAlignment.Left || m_Alignment==eTabStripAlignment.Right)
            ...
            else if (this.IsRightToLeft && m_TabLayoutType != TabLayoutType.FitContainer &&
		...
		internal TabItem DesignTimeSelection
		{
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Behavior"),Description("Indicates selected tab.")]
		public TabItem SelectedTab
		{
			...
			set
			{
				SelectTab(value,EventSource.Code);
			}
		}

		private void SelectTab(TabItem value, EventSource eventSource)
		{
			...
			{
				TabStripTabChangingEventArgs eventData=new TabStripTabChangingEventArgs(m_SelectedTab,value,EventSource.Code);
			...
			{
				TabStripTabChangedEventArgs eventData=new TabStripTabChangedEventArgs(oldTab,m_SelectedTab,EventSource.Code);
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(false),Category("Behavior"),Description("Indicates whether tabs are scrolled continuously while mouse is pressed over the scroll tab button.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(300),Category("Behavior"),Description("Indicates auto-repeat interval for the tab scrolling while mouse button is kept pressed over the scroll tab button.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Close Button"),Description("Indicates whether the Close button that closes the active tab is visible.")]
        ...
        /// </summary>
        [Browsable(true), DefaultValue(false), HVTTBrowsable(true), Category("Close Button"), Description("Indicates whether close button is visible on each tab instead of in system box.")]
        ...
        /// </summary>
        [Browsable(true), DefaultValue(true), HVTTBrowsable(true), Category("Close Button"), Description("Indicates whether close button on tabs when visible is displayed for every tab state.")]
        ...
        /// </summary>
        [Browsable(true), DefaultValue(eTabCloseButtonPosition.Left), HVTTBrowsable(true), Category("Close Button"), Description("Indicates position of the close button displayed on each tab.")]
		...
		/// </summary>
		[Editor(typeof(TabStripTabsEditor), typeof(UITypeEditor)),DesignerSerializationVisibility(DesignerSerializationVisibility.Content),Category("Data"),Description("Returns the collection of Tabs.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Appearance"),DefaultValue(null)]
		...
		/// </summary>
		[Browsable(true),DefaultValue(TabLayoutType.FitContainer),Category("Appearance"),Description("Indicates the type of the tab layout.")]
		...
		/// </summary>
		[Browsable(false),HVTTBrowsable(false),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),Obsolete("Please use TabLayoutType property instead.",true)]
		public bool VariableTabWidth
		{
			...
			{
				if(value)
					m_TabLayoutType=TabLayoutType.FitContainer;
				else
                    m_TabLayoutType=TabLayoutType.FixedWithNavigationBox;
			}
		}
		...
		/// </summary>
		[Browsable(false),HVTTBrowsable(false),DefaultValue(0),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		...
		internal TabSystemBox _TabSystemBox
		{
		...
		protected override bool ProcessCmdKey(ref Message msg,Keys keyData)
		{
			...
			{
				if(SelectPreviousTab(EventSource.Keyboard))
					return true;
			}
			else if(keyData==Keys.Right)
			{
				if(SelectNextTab(EventSource.Keyboard))
					return true;
			}
		...
		{
			return SelectPreviousTab(EventSource.Code);
		}
		private bool SelectPreviousTab(EventSource eventSource)
		{
		...
		{
			return SelectNextTab(EventSource.Code);
		}
		private bool SelectNextTab(EventSource eventSource)
		{
		...
		{
			get {return (m_TabLayoutType==TabLayoutType.FixedWithNavigationBox || m_TabLayoutType==TabLayoutType.MultilineWithNavigationBox);}
		}

		internal bool IsMultiLine
		{
			get {return (m_TabLayoutType==TabLayoutType.MultilineNoNavigationBox || m_TabLayoutType==TabLayoutType.MultilineWithNavigationBox);}
		}
				...
				foreach(TabItem tab in m_Tabs)
				{
					if((tab.DisplayRectangle.Contains(new Point(e.X,e.Y)) || rSelected.Contains(e.X,e.Y)) && tab.Visible)
					{
						...
						if(m_SelectedTab!=newSelected)
						{
							...
							if(SelectedTabChanging!=null)
							{
								TabStripTabChangingEventArgs eventData=new TabStripTabChangingEventArgs(m_SelectedTab,newSelected,EventSource.Mouse);
								SelectedTabChanging(this,eventData);
								if(eventData.Cancel)
									break;
							}
							...
							if(SelectedTabChanged!=null)
							{
								TabStripTabChangedEventArgs eventData=new TabStripTabChangedEventArgs(oldTab,m_SelectedTab,EventSource.Mouse);
								SelectedTabChanged(this,eventData);
							}
						}
                        else if (m_SelectedTab != null && m_SelectedTab.AttachedItem is DockContainerItem && m_SelectedTab.AttachedItem.ContainerControl is HVTTMarkStatus && ((HVTTMarkStatus)m_SelectedTab.AttachedItem.ContainerControl).DockSide == eDockSide.Document &&
		...
		public void SetTabIndex(String TabName,Int32 index)
		{
			TabItem tabItem = m_Tabs[TabName];
			if (tabItem == null)
				return;
			//m_MovingTab = true;
		...
		internal void InternalOnMouseMove(MouseEventArgs e)
		{
			...
			if(this.HasNavigationBox && !m_TabDrag)
			{
			...
			if(e.Button==MouseButtons.Left)
			{
				...
				if(m_TabDrag)
				{
					...
					else
					{
						...
						if(!r.Contains(e.X,e.Y))
						{
							...
							if(this.Parent is HVTTMarkStatus && ((HVTTMarkStatus)this.Parent).CanTearOffTabs && (visibleTabCount>1 || visibleTabCount==1 && ((HVTTMarkStatus)this.Parent).GrabHandleStyle==HVTTGrabHandleStyle.None))
							{
								m_TabDrag=false;
								m_IsDragging=true; // Very important to set this flag before calling StartTabDrag

								m_DragBar=((HVTTMarkStatus)this.Parent).StartTabDrag();
								if(m_DragBar!=null)
								{	
									this.Capture=true;
                                    this.Refresh();
				...
				else if((Math.Abs(e.X)>=4 || Math.Abs(e.Y)>=4) && m_Tabs.Count>0 && !this.IsMultiLine)
				{
					m_TabDrag=true;
				}
			}
		...
		internal void InternalOnMouseUp(MouseEventArgs e)
		{
            ...
            if (this.SelectedTab != null && this.SelectedTab == tabAt && !this.SelectedTab.CloseButtonBounds.IsEmpty && this.SelectedTab.CloseButtonBounds.Contains(e.X, e.Y))
            {
		...
		internal void InternalOnMouseLeave(EventArgs e)
		{
			HideToolTip();
			if(this.HasNavigationBox)
		...
		internal void InternalTabsCleared()
		{
			...
			if(m_SelectedTab!=null)
			{
				if(SelectedTabChanged!=null)
				{
					TabStripTabChangedEventArgs eventData=new TabStripTabChangedEventArgs(m_SelectedTab,null,EventSource.Code);
					SelectedTabChanged(this,eventData);
				}
				m_SelectedTab=null;
			}
		...
		{
			if(tab==null || !m_Tabs.Contains(tab) || m_TabLayoutType==TabLayoutType.FitContainer || this.IsMultiLine || !BarFunctions.IsHandleValid(this))
				return;
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Style"),DefaultValue(null),Description("Gets or sets the selected tab Font")]
		...
		/// </summary>
		[DefaultValue(eTabStripAlignment.Bottom),Browsable(true),HVTTBrowsable(true),Category("Appearance"),Description("Indicates the tab alignment within the Tab-Strip control.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Behavior"),Description("Specifes whether end-user can reorder the tabs.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Behavior"),Description("Specifes whether only selected tab is displaying it's text."),DefaultValue(false)]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Appearance"),Description("Specifes the TabStrip style."),DefaultValue(eTabStripStyle.Flat)]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(false),Category("Appearance"),Description("Specifies whether tab is drawn using Themes when running on OS that supports themes like Windows XP.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Style"),Description("Gets or sets Tab Color Scheme."),DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
		...
		protected override bool ProcessMnemonic(char charCode)
		{
			...
			foreach(TabItem tab in this.Tabs)
			{
				...
				if(text.IndexOf(s)>=0)
				{
					this.SelectTab(tab,EventSource.Keyboard);
					return true;
				}
			}
		...
		internal TabColors GetTabColors(TabItem tab)
		{
			...
			{
                if (tab.PredefinedColor != TabItemColor.Default)
				{
		...
		internal void ScrollForward()
		{
			...
			if(m_Alignment==eTabStripAlignment.Top || m_Alignment==eTabStripAlignment.Bottom)
			{
				if(iEnd>m_Tabs[m_Tabs.Count-1].DisplayRectangle.Right+m_ScrollOffset)
        ...
        private void CloseSelectedTab()
        {
            ...
            else if (this.SelectedTab != null && this.SelectedTab.AttachedItem != null)
            {
                ...
                {
                    HVTTMarkStatus bar = this.SelectedTab.AttachedItem.ContainerControl as HVTTMarkStatus;
                    if (bar != null)
                        bar.CloseDockTab((DockContainerItem)this.SelectedTab.AttachedItem, EventSource.Mouse);
                    else
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Description("Indicates whether the tab scrolling is animanted."),Category("Behavior"),DefaultValue(true)]
		public bool Animate
		{
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Description("Indicates the maximum number of characters that will be used as Tab text from Mdi Child caption."),Category("Mdi Support"),DefaultValue(32)]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Description("Indicates whether the Mdi Child Icon is displayed on Tab."),Category("Mdi Support"),DefaultValue(true)]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Description("Indicates whether the Tab-strip is automatically hidden when there are not Mdi Child forms open."),Category("Mdi Support"),DefaultValue(true)]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Description("Indicates whether flicker associated with switching maximized Mdi child forms is attempted to eliminate."),Category("Mdi Support"),DefaultValue(true)]
		...
		private string GetFormText(Form form)
		{
			if(form==null)
				return "";
			if(form.Text.Length>m_MaxMdiCaptionLength)
			{
				int i=m_MaxMdiCaptionLength/2;
				return form.Text.Substring(0,i)+"..."+form.Text.Substring(form.Text.Length-(m_MaxMdiCaptionLength-i));
			}
			return form.Text;
		}
...
//					item.Image=img;
	...
	public class TabStripTabChangedEventArgs : EventArgs 
	{
		...
		/// </summary>
		public readonly EventSource EventSource=EventSource.Code;
		...
		public TabStripTabChangedEventArgs(TabItem oldtab,TabItem newtab,EventSource source) 
		{
			this.OldTab=oldtab;
			this.NewTab=newtab;
	...
	public class TabStripTabChangingEventArgs : EventArgs 
	{
		...
		/// <param name="oldtab">Currently selected tab.</param>
		/// <param name="newtab">New selection.</param>
		public TabStripTabChangingEventArgs(TabItem oldtab,TabItem newtab,EventSource source) 
		{
			this.OldTab=oldtab;
			this.NewTab=newtab;
			this.EventSource=source;
	...
	{
		public TabStripTabsEditor(Type type):base(type)
		{
		}
	...
	internal class TabSystemBox
	{
		...
		public void Paint(Graphics g)
		{
			...
			Color colorDark=g.GetNearestColor(ControlPaint.Light(m_Parent.ColorScheme.TabItemSelectedText));
		...
		private void DrawBackgroundHot(Graphics g, Rectangle r)
		{
			...
			if((m_Parent.Style==eTabStripStyle.Office2003 || m_Parent.Style==eTabStripStyle.VS2005 || m_Parent.Style==eTabStripStyle.VS2005Document || m_Parent.Style==eTabStripStyle.VS2005Dock) && back.IsEmpty && back2.IsEmpty)
			{
				...
				if(m_Parent.Style==eTabStripStyle.Office2003)
					cs=new ColorScheme(HVTTControlStyle.Office2003);
				else
                    cs=new ColorScheme(HVTTControlStyle.VS2005);
