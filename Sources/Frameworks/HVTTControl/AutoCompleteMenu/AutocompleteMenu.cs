...
namespace HVTT.UI.Window.Forms.HVTTAutoCompleteMenu
{
    ...
    public class AutocompleteMenu : Component, IExtenderProvider
    {
        ...
        [DefaultValue(false)]
        public Boolean SearchAllText { get; set; }

        public Boolean ShowPopupOnClick { get; set; }
        ...
        public List<AutocompleteItem> AutocompleteItems
        {
            get
            {
                ...
                return (sourceItems as List<AutocompleteItem>);
            }
        }
        ...
        private void SetAutoSelectBox(Control control)
        {
            if (ShowPopupOnClick)
            {
                HVTT.UI.Window.Forms.Controls.HVTTTextBox txt = control as HVTT.UI.Window.Forms.Controls.HVTTTextBox;
                if (txt != null)
                {
                    txt.HasFocus += txt_HasFocus;
                    txt.Click += txt_Click;
                }
            }
        }
        ...
        private void txt_HasFocus(object sender, EventArgs e)
        {
            TargetControlWrapper = FindWrapper(sender as Control);
            if (TargetControlWrapper == null)
                return;
        ...
        private void ShowMenu()
        {
            if (!Host.Visible)
            {
                ...
                if (TargetControlWrapper.TargetControl != null)
                {
                    var f = TargetControlWrapper.TargetControl.FindForm();
                    if (f != null 
                        && (
                            f.ActiveControl is TextBox
                            || f.ActiveControl is RichTextBox
                        )
                        && f.ActiveControl != targetControlWrapper.TargetControl)
                    {
                        args.Cancel = true;
                    }
                }
        ...
        private Range GetFragment(string searchPattern)
        {
            ...
            {
                // return new Range(tb);
                var result1 = new Range(tb);
                result1.Start = 0;
                result1.End = tb.Text.Length;
                return result1;
            }
        ...
        public void UpdateItem(MenuColumn Key,List<MenuColumn> NewColumns,String TextSearch)
        {
            if (sourceItems == null || sourceItems.Count() == 0)
                return;

            var lst = sourceItems.Where(x => x.MenuColumns.Where(y => y.ColumnName == Key.ColumnName && y.ColumnValue == Key.ColumnValue).Count() > 0);
            if (lst != null && lst.Count() > 0)
            {
                lst.FirstOrDefault().MenuColumns = NewColumns;
                lst.FirstOrDefault().Text = TextSearch;
            }            
        }
        ...
        {
            string newText = "";
            if (item.KeyText != null && item.KeyText.Trim() != "")
                newText = item.KeyText;
            else
                newText = item.GetTextForReplace();
