...
namespace HVTT.UI.Window.Forms
{
	...
	/// </summary>
	[ToolboxItem(true),DefaultEvent("ButtonClick"),System.Runtime.InteropServices.ComVisible(false),Designer(typeof(Design.BubbleBarDesigner))]
	public class BubbleBar:Control, ISupportInitialize, IMessageHandlerClient
	{
		...
		private HVTTOrientation m_Orientation=HVTTOrientation.Horizontal;
		...
		private BubbleButtonAlignment m_ButtonAlignment=BubbleButtonAlignment.Bottom;
...
//			m_MouseOverTabColors.BorderColor=SystemColors.Highlight;
		...
		/// </summary>
		[Editor(typeof(BubbleBarTabCollectionEditor), typeof(System.Drawing.Design.UITypeEditor)),DesignerSerializationVisibility(DesignerSerializationVisibility.Content),System.ComponentModel.Category("Tabs"),Description("Returns the collection of Tabs.")]
		...
		public BubbleButtonAlignment Alignment
		{
			get
			{
		...
		private Rectangle GetOverlayBounds()
		{
			...
			if(m_ButtonAlignment==BubbleButtonAlignment.Top || m_ButtonAlignment==BubbleButtonAlignment.Bottom)
			{
				...
				r.Height+=increase;
				if(m_ButtonAlignment==BubbleButtonAlignment.Bottom)
					r.Y-=increase;
			}
			else
			{
		...
		private void MouseDownMessage(MouseEventArgs e)
		{
			...
			else if(m_TabsBounds.Contains(e.X,e.Y))
			{
				...
				if(tab!=null && tab!=m_SelectedTab)
					SetSelectedTab(tab,EventSource.Mouse,true);
			}
		}
		...
		private void MouseUpMessage(MouseEventArgs e)
		{
			if(m_MouseDownButton!=null)
			{
                ...
                if (dispRect.Contains(e.X, e.Y) || magRect.Contains(e.X, e.Y))
				    button.InvokeClick(EventSource.Mouse,e.Button);
		...
		private Rectangle GetButtonDisplayArea()
		{
			...
			else if(m_ButtonAlignment==BubbleButtonAlignment.Top)
			{
				r.Height-=m_ButtonMargin;
				r.Y+=m_ButtonMargin;
			}
...
//			else if(m_ButtonAlignment==BubbleButtonAlignment.Left)
//			{
//				m_ContentManager.ContentAlignment=eContentAlignment.Center;
//				m_ContentManager.ContentVerticalAlignment=eContentVerticalAlignment.Bottom;
//				m_ContentManager.ContentOrientation=eContentOrientation.Horizontal;
//			}
		...
		internal void OnTabRemoved(BubbleBarTab tab)
		{
			...
			{
				SelectNextVisible(tab,EventSource.Code, false);
			}
			if(this.DesignMode)
			{
				this.RecalcLayout();
		...
		private void SelectNextVisible(BubbleBarTab reference, EventSource source, bool bCanCancel)
		{
		...
		private void SetSelectedTab(BubbleBarTab tab, EventSource source, bool bCanCancel)
		{
			if(TabChanging!=null)
			{
		...
		internal void SetMouseOverTab(BubbleBarTab tab)
		{
		...
		internal bool DragInProgress
		{
			get {return m_DragInProgress;}
		}
		internal void StartDrag(BubbleBarTab tab)
		{
			if(m_DragInProgress)
				return;
		...
		internal void StartDrag(BubbleButton button)
		{
			if(m_DragInProgress)
				return;
		...
		internal void DragMouseMove(Point mousePosition)
		{
			if(m_DragTab!=null)
			{
				BubbleBarTab tab=GetTabAt(mousePosition);
		...
		internal void DragMouseUp(Point mousePosition)
		{
			if(m_DragTab!=null)
			{
				BubbleBarTab tab=GetTabAt(mousePosition);
			...
			if(m_HasShortcuts)
			{
				foreach(BubbleBarTab t in this.Tabs)
				{
					foreach(BubbleButton b in t.Buttons)
					{
						...
						{
							b.InvokeClick(EventSource.Keyboard, MouseButtons.None);
							return true;
						}
					}
				}
	...
	public class BubbleBarTabChangingEventArgs:EventArgs
	{
		...
		/// </summary>
		public EventSource Source=EventSource.Code;
		/// <summary>
