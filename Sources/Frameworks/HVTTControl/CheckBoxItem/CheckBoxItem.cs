...
namespace HVTT.UI.Window.Forms
{
    ...
    /// </summary>
    [ToolboxItem(false), DesignTimeVisible(false), DefaultEvent("Click"), Designer(typeof(Design.CheckBoxItemDesigner))]
    public class CheckBoxItem : BaseItem
    {
                ...
                try
                {
                    ...
                    if (m_CheckBoxPosition == eCheckBoxPosition.Left || m_CheckBoxPosition == eCheckBoxPosition.Right)
                    {
                        m_Rect = new Rectangle(m_Rect.X, m_Rect.Y,
                            textSize.Width + checkTextSpacing + m_CheckSignSize.Width,
                            Math.Max(m_CheckSignSize.Height, textSize.Height) + verticalPadding * 2);
                    }
                    else
                    {
                        m_Rect = new Rectangle(m_Rect.X, m_Rect.Y,
                            Math.Max(textSize.Width, m_CheckSignSize.Width) + verticalPadding * 2,
                            textSize.Height + checkTextSpacing + m_CheckSignSize.Height);
                    }
                }
                finally
                {
        ...
        /// </summary>
        [Browsable(true), Editor(typeof(Design.TextMarkupUIEditor), typeof(System.Drawing.Design.UITypeEditor)), Category("Appearance"), Description("The text contained in the item."), Localizable(true), DefaultValue("")]
        ...
        /// </summary>
        [Browsable(true), RefreshProperties(RefreshProperties.All), Category("Appearance"), Description("Indicates whether item is checked or not."), DefaultValue(false)]
        public virtual bool Checked
        {
            ...
            set
            {
                if (m_Checked != value)
                {
                    ...
                    SetChecked(value, EventSource.Code);
                }
            }
        }
        ...
        public override void RaiseClick(EventSource source)
        {
            if (CanRaiseClick && !(this.CheckBoxStyle == eCheckBoxStyle.RadioButton && this.Checked))
            {
                if (this.ThreeState)
                {
        ...
        public virtual void SetChecked(bool newValue, EventSource source)
        {
        ...
        /// <param name="newValue">New value for Checked property</param>
        /// <param name="source">Source of the change.</param>
        public virtual void SetChecked(CheckState newValue, EventSource source)
        {
            CheckBoxChangeEventArgs e = new CheckBoxChangeEventArgs(null, this, source);
            InvokeCheckedChanging(e);
            if (e.Cancel)
        ...
        /// </summary>
        protected virtual void OnCheckedChanged(EventSource source)
        {
        ...
        /// </summary>
        [Browsable(true), Category("Behavior"), DefaultValue(CheckState.Unchecked), RefreshProperties(RefreshProperties.All), Description("Specifies the state of a control, such as a check box, that can be checked, unchecked, or set to an indeterminate state")]
    ...
    public class CheckBoxChangeEventArgs : EventArgs
    {
        ...
        /// Default constructor.
        /// </summary>
        public CheckBoxChangeEventArgs(CheckBoxItem oldchecked, CheckBoxItem newchecked, EventSource eventSource)
        {
