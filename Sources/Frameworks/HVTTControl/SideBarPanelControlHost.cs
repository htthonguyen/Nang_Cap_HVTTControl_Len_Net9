...
namespace HVTT.UI.Window.Forms
{
	...
	public class SideBarPanelControlHost:Control,IThemeCache
	{
		...
		public SideBarPanelControlHost(SideBarPanelItem parentPanel)
		{
			...
			if(parentPanel.Appearance==SideBarAppearance.Flat)
			{
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			{
				((IThemeCache)this).ThemeRebar.DrawBackground(e.Graphics,ThemeRebarParts.Background,ThemeRHVTTBarStates.Normal,this.DisplayRectangle);
			}
			else
			{
                if (!m_ParentPanel.BackgroundStyle.BackColor1.IsEmpty)
                {
                    if (m_ParentPanel.Appearance == SideBarAppearance.Flat)
                    {
                        BorderSide oldBorder = m_ParentPanel.BackgroundStyle.BorderSide;
                        m_ParentPanel.BackgroundStyle.BorderSide = ~(oldBorder & BorderSide.Top) & oldBorder;
                ...
                else
                {
                    ...
                    if (bar != null && bar.Style == HVTTControlStyle.Office2007 && bar.BackColor == SystemColors.Control)
                    {
		...
		private void InternalMouseHover()
		{
			...
			if(m_HotSubItem!=null)
			{
				...
				{
					BaseItem objItem=m_ParentPanel.ExpandedItem();
					if(objItem!=null && m_HotSubItem!=objItem && (m_ParentPanel.IsOnMenu || m_ParentPanel.ContainerControl is HVTTMarkStatus))
						objItem.Expanded=false;
				}
		...
		private void InternalMouseMove(System.Windows.Forms.MouseEventArgs objArg)
		{
			...
			if(m_ParentPanel.IsContainer && !m_ParentPanel.DesignMode)
			{
				...
				if(objNew!=m_HotSubItem)
				{
					...
					{
						m_HotSubItem.InternalMouseLeave();
						if(objNew!=null && m_HotSubItem.Expanded && (m_ParentPanel.IsOnMenu || m_ParentPanel.ContainerControl is HVTTMarkStatus))
