...
{
    public class HVTTCheckedBoxComboBox : HVTTComboBox {
        /// <summary>
        /// Internal class to represent the dropdown list of the HVTTCheckedBoxComboBox
        /// </summary>
        internal class Dropdown : Form {
            ...
            /// </summary>
            internal class HVTTCheckedBoxEventArgs : EventArgs {
                ...
                public HVTTCheckedBoxEventArgs(EventArgs e, bool assignValues) : base() {
                    this.e = e;
            ...
            /// <summary>
            /// A custom CheckedListBox being shown within the dropdown form representing the dropdown list of the HVTTCheckedBoxComboBox.
            /// </summary>
            internal class CustomCheckedListBox : CheckedListBox {
                ...
                protected override void OnKeyDown(KeyEventArgs e) {
                    if (e.KeyCode == Keys.Enter) {
                        // Enact selection.
                        ((HVTTCheckedBoxComboBox.Dropdown) Parent).OnDeactivate(new HVTTCheckedBoxEventArgs(null, true));
                        e.Handled = true;

                    } else if (e.KeyCode == Keys.Escape) {
                        // Cancel selection.
                        ((HVTTCheckedBoxComboBox.Dropdown) Parent).OnDeactivate(new HVTTCheckedBoxEventArgs(null, false));
                        e.Handled = true;
            ...
            private HVTTCheckedBoxComboBox ccbParent;

            // Keeps track of whether checked item(s) changed, hence the value of the HVTTCheckedBoxComboBox as a whole changed.
            ...
            public Dropdown(HVTTCheckedBoxComboBox ccbParent) {
            ...
            ///       HVTTCheckedBoxComboBox (after the dropdown has closed) to determine any actual value changes.
            /// </summary>
            ...
            public void CloseDropdown(bool enactChanges) {
                ...
                dropdownClosed = true;
                // Set the focus to our parent HVTTCheckedBoxComboBox and hide the dropdown check list.
                ccbParent.Focus();
                this.Hide();
                // Notify HVTTCheckedBoxComboBox that its dropdown is closed. (NOTE: it does not matter which parameters we pass to
                ...
                ccbParent.OnDropDownClosed(new HVTTCheckedBoxEventArgs(null, false));
            }
            ...
            protected override void OnDeactivate(EventArgs e) {
                ...
                base.OnDeactivate(e);
                HVTTCheckedBoxEventArgs ce = e as HVTTCheckedBoxEventArgs;
                if (ce != null) {
        ...
        List<HVTTCheckedBoxItem> _mDataSource = new List<HVTTCheckedBoxItem>();
        List<HVTTCheckedBoxItem> _mSelectedItems = new List<HVTTCheckedBoxItem>();

        // The valueSeparator character(s) between the ticked elements as they appear in the 
        // text portion of the HVTTCheckedBoxComboBox.
        ...
        public new List<HVTTCheckedBoxItem> Items {
            get { return _mDataSource; }
        }
        public new List<HVTTCheckedBoxItem> DataSource
        {
            get;set;
        }
        public void ApplyToDataSources()
        {
            _mSelectedItems = new List<HVTTCheckedBoxItem>();
            dropdown.List.DisplayMember = "Text";
            dropdown.List.ValueMember = "Value";
            foreach (var x in _mDataSource)
            {
                dropdown.List.Items.Add(x);
                if (x.Checked)
                    dropdown.List.SetItemChecked(dropdown.List.Items.Count - 1, true);
            }
            this.Text = GetCheckedItemString();
        ...
        public List<HVTTCheckedBoxItem> SelectedItems
        {
        ...
        public HVTTCheckedBoxComboBox() : base() {
            ...
            this.dropdown.List.ItemCheck += List_ItemCheck;
        ...
        {
            StringBuilder sb = new StringBuilder();
            foreach (var x in _mSelectedItems)
                sb.Append(x.Text).Append(ValueSeparator);
            return sb.ToString();
        }
        ...
        private void List_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            ...
            if (_mDataSource.Count <= e.Index || e.Index < 0)
                return;

            if (e.CurrentValue == CheckState.Checked)
            {
                var x = _mDataSource[e.Index];
                if (e.NewValue == CheckState.Unchecked)
                {
                    _mSelectedItems.Remove(x);
                    this.Text = GetCheckedItemString();
                }
                else
                {
                    var lst = (from y in _mSelectedItems
                               where y.Value == x.Value
                               select y).ToList();
                    if (lst != null && lst.Count > 0)
                        return;
                    _mSelectedItems.Add(x);
                    this.Text = GetCheckedItemString();
                }
            }
        ...
        protected override void OnDropDownClosed(EventArgs e) {
            ...
            if (e is Dropdown.HVTTCheckedBoxEventArgs) {
    ...
    } // end public class HVTTCheckedBoxComboBox
