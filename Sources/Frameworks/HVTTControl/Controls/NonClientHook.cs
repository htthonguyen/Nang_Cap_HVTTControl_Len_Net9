...
namespace HVTT.UI.Window.Forms
{
    ...
    internal class NonClientHook
    {
        ...
        // Removed unsafe by using Marshal.PtrToStructure
        private static IntPtr OnMouseHook(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode == HC_ACTION)
            {
                int msg = wParam.ToInt32();
                if (msg == (int)WinApi.WindowsMessages.WM_MOUSEMOVE || msg == (int)WinApi.WindowsMessages.WM_LBUTTONUP)
                {
                    try
                    {
                        MOUSEHOOKSTRUCT ws = Marshal.PtrToStructure<MOUSEHOOKSTRUCT>(lParam);
                        if (msg == (int)WinApi.WindowsMessages.WM_MOUSEMOVE)
                            PostMouseMove(ws.hwnd, ws.pt.ToPoint());
                        else if (msg == (int)WinApi.WindowsMessages.WM_LBUTTONUP)
                            PostMouseUp(ws.hwnd, ws.pt.ToPoint());
                    }
                    catch
                    {
                        // Swallow marshal exceptions – proceed to next hook
                    }
                }
            }
