...
namespace HVTT.UI.Window.Forms.Controls.HVTTSyntax
{
    public class SyntaxHighlighter : IDisposable
    {
        ...
        protected Regex SQLiteKeywordsRegex;
        protected Regex SQLiteNumberRegex;
        protected Regex SQLiteStatementsRegex;
        protected Regex SQLiteStringRegex;
        protected Regex SQLiteTypesRegex;
        protected Regex SQLiteVarRegex;
        protected Regex SQLiteTableRegex;
        ...
        {
            var tb = (sender as HVTTSyntaxTextBox);
        ...
        {
            /*
            HVTTSyntaxTextBox tb = sender as HVTTSyntaxTextBox;
        ...
        protected void RestoreBrackets(HVTTSyntaxTextBox tb, char[] oldBrackets)
        {
        ...
        protected char[] RememberBrackets(HVTTSyntaxTextBox tb)
        {
...
";
        ...
        protected void InitSQLiteRegex()
        {
            SQLiteStringRegex = new Regex(@"""""|''|"".*?[^\\]""|'.*?[^\\]'", RegexCompiledOption);
            SQLiteNumberRegex = new Regex(@"\b\d+[\.]?\d*([eE]\-?\d+)?\b", RegexCompiledOption);
            SQLiteVarRegex = new Regex(@"@[a-zA-Z_\d]*\b", RegexCompiledOption);
            SQLiteStatementsRegex = new Regex(@"\b(ALTER DATABASE|ALTER FUNCTION|ALTER INDEX|ALTER TABLE|ALTER TRIGGER|ALTER VIEW|CREATE DATABASE|CREATE FUNCTION|CREATE INDEX|CREATE PROCEDURE|CREATE TABLE|CREATE TRIGGER|CREATE VIEW|DELETE|DROP DATABASE|DROP FUNCTION|DROP INDEX|DROP PROCEDURE|DROP TABLE|DROP TRIGGER|DROP VIEW|REPLACE|FROM|INSERT|SELECT|TOP|TRUNCATE TABLE|UPDATE|WHERE|WITH|INTO|IN|SET)\b", RegexOptions.IgnoreCase | RegexCompiledOption);
            SQLiteKeywordsRegex = new Regex(@"\b(ABORT,ACTION,ADD,AFTER,ALL,ALTER,ANALYZE,AND,AS,ASC,ATTACH,AUTOINCREMENT,BEFORE,BEGIN,BETWEEN,BY,CASCADE,CASE,CAST,CHECK,COLLATE,COLUMN,COMMIT,CONFLICT,CONSTRAINT,CREATE,CROSS,CURRENT_DATE,CURRENT_TIME,CURRENT_TIMESTAMP,DATABASE,DEFAULT,DEFERRABLE,DEFERRED,DELETE,DESC,DETACH,DISTINCT,DROP,EACH,ELSE,END,ESCAPE,EXCEPT,EXCLUSIVE,EXISTS,EXPLAIN,FAIL,FOR,FOREIGN,FROM,FULL,GLOB,GROUP,HAVING,IF,IGNORE,IMMEDIATE,IN,INDEX,INDEXED,INITIALLY,INNER,INSERT,INSTEAD,INTERSECT,INTO,IS,ISNULL,JOIN,KEY,LIKE,LIMIT,MATCH,NATURAL,NO,NOT,NOTNULL,NULL,OF,OFFSET,ON,OR,ORDER,OUTER,PLAN,PRAGMA,PRIMARY,QUERY,RAISE,RECURSIVE,REFERENCES,REGEXP,REINDEX,RELEASE,RENAME,REPLACE,RESTRICT,RIGHT,ROLLBACK,ROW,SAVEPOINT,SELECT,SET,TABLE,TEMP,TEMPORARY,THEN,TO,TRANSACTION,TRIGGER,UNION,UNIQUE,UPDATE,USING,VACUUM,VALUES,VIEW,VIRTUAL,WHEN,WHERE,WITH,WITHOUT)\b", RegexOptions.IgnoreCase | RegexCompiledOption);
            SQLiteTypesRegex =
                new Regex(
                    @"\b(INTEGER,TEXT,NUMERIC,BLOB,REAL)\b",
                    RegexOptions.IgnoreCase | RegexCompiledOption);
        }
        ...
        /// <summary>
        /// Highlights SQLite code
        /// </summary>
        /// <param name="range"></param>
        public virtual void SQLiteSyntaxHighlight(Range range)
        {
            range.tb.CommentPrefix = "--";
            range.tb.LeftBracket = '(';
            range.tb.RightBracket = ')';
            ...
            range.ClearStyle(CommentStyle, StringStyle, NumberStyle, VariableStyle, StatementsStyle, KeywordStyle,
                             FunctionsStyle, TypesStyle);
            //
            if (SQLiteStringRegex == null)
                InitSQLiteRegex();
            //comment highlighting
          
            range.SetStyle(StringStyle, SQLiteStringRegex);
            //number highlighting
            range.SetStyle(NumberStyle, SQLiteNumberRegex);
            //types highlighting
            range.SetStyle(TypesStyle, SQLiteTypesRegex);
            //var highlighting
            range.SetStyle(VariableStyle, SQLiteVarRegex);
            //statements
            range.SetStyle(StatementsStyle, SQLiteStatementsRegex);
            //keywords
            range.SetStyle(KeywordStyle, SQLiteKeywordsRegex);
