namespace HVTT.UI.Window.Forms
{
    ...
    /// </summary>
	[ToolboxBitmap(typeof(HVTTMarkStatus),"Bar.ico"), ToolboxItem(true),Designer(typeof(BarDesigner)),System.Runtime.InteropServices.ComVisible(false), DefaultEvent("ItemClick")]
	public class HVTTMarkStatus:Control, IDockInfo, IBarImageSize, IOwner, IOwnerMenuSupport,
        ...
        ICustomSerialization, IRenderingSupport, IAccessibilitySupport, IOwnerLocalize
	{
		...
		public event HVTTManager.DockTabChangeEventHandler DockTabChange;
		...
		public event HVTTManager.BarClosingEventHandler Closing;
		...
		public event HVTTManager.AutoHideDisplayEventHandler AutoHideDisplay;

		/// <summary>
		/// Occurs when popup item is closing. Event is fired only when Bar is used independently of HVTTManager.
		/// </summary>
		[System.ComponentModel.Description("Occurs when popup item is closing."),Category("Item")]
		public event EventHandler PopupClose;

		/// <summary>
		/// Occurs when popup of type container is loading. Event is fired only when Bar is used independently of HVTTManager.
		/// </summary>
		...
		/// <summary>
		/// Occurs when popup of type container is unloading. Event is fired only when Bar is used independently of HVTTManager.
		/// </summary>
		...
		/// <summary>
		/// Occurs when popup item is about to open.  Event is fired only when Bar is used independently of HVTTManager.
		/// </summary>
		[System.ComponentModel.Description("Occurs when popup item is about to open."),Category("Item")]
		public event HVTTManager.PopupOpenEventHandler PopupOpen;

		/// <summary>
		/// Occurs just before popup window is shown. Event is fired only when Bar is used independently of HVTTManager.
		/// </summary>
        ...
        /// <summary>
        /// Occurs when HVTTCONTROLS is looking for translated text for one of the internal text that are
		...
		private HVTTBarState m_BarState;
		private HVTTGrabHandleStyle m_GrabHandleStyle;
		...
		private DockSiteInfo m_DragDockInfo;
		private Rectangle m_LastDragRect;
		private DockSiteInfo m_LastDockSiteInfo;
		...
		private HVTTPopupAnimation m_PopupAnimation=HVTTPopupAnimation.ManagerControlled;
		private PopupShadow m_DropShadow=null;
		private BarImageSize m_ImageSize=BarImageSize.Default;
...
//		internal int _SplitDockHeight=0;
		...
		private HVTTMarkStatus m_TempTabBar=null;
		...
		private BackgroundImagePosition m_BackgroundImagePosition=BackgroundImagePosition.Stretch;
		...
		private BarBaseControlDesigner m_BarDesigner=null;
		...
		public HVTTMarkStatus()
		{
			...
			//m_BarState=HVTTBarState.Popup;
			m_BarState=HVTTBarState.Docked;

			m_DockOffset=0;
			m_DockLine=0;
			m_GrabHandleStyle=HVTTGrabHandleStyle.None;
		...
		public HVTTMarkStatus(string BarCaption):this()
		{
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),DefaultValue(null),HVTTBrowsable(false),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		public object Owner
		{
			get
			{
		...
		protected override void Dispose(bool disposing)
		{
			...
			{
				HVTTManager.UnRegisterOwnerParentMsgHandler(this,null);
			...
			if(m_Owner is HVTTManager && ((HVTTManager)m_Owner).IgnoreLoadedControlDispose)
				this.Controls.Clear();
		...
		protected override CreateParams CreateParams
		{
			get
			{
				...
				if(PassiveBar)
					return p;
				if(m_BarState==HVTTBarState.Popup && (m_ParentItem==null || (m_ParentItem.Site == null || !m_ParentItem.Site.DesignMode)))
				{
					p.Style=unchecked((int)(WS_POPUP | WS_CLIPSIBLINGS | WS_CLIPCHILDREN));
					p.ExStyle=(int)(WS_EX_TOPMOST | WS_EX_TOOLWINDOW);
				}
				//else if(m_BarState==HVTTBarState.Floating)
				//{
		...
		protected override void OnLeave(EventArgs e)
		{
			...
			{
				m_HasFocus=false;
                if (m_GrabHandleStyle == HVTTGrabHandleStyle.Caption && (m_BarState == HVTTBarState.Docked || m_BarState == HVTTBarState.AutoHide))
                    this.Invalidate();

				if(m_BarState==HVTTBarState.AutoHide)
				{
        ...
        //        r = new Rectangle(0, 0, GrabHandleResizeWidth, this.Height);
        //    else
        //        r = new Rectangle(this.Width - GrabHandleResizeWidth, 0, GrabHandleResizeWidth, this.Height);
		...
		{
			if(m.Msg==WM_MOUSEACTIVATE && (m_BarState==HVTTBarState.Popup || m_BarState==HVTTBarState.Floating && m_ItemContainer.LayoutType==HVTTLayoutType.TaskList))// && m_BarState!=HVTTBarState.Docked)
			{
			...
			{
				if(!m_DockingInProgress && m_BarState==HVTTBarState.Floating)
			...
			else if(m.Msg==NativeFunctions.WM_SETFOCUS)
			{
				...
				if(objCtrl!=null && !(objCtrl is HVTT.UI.Window.Forms.HVTTMarkStatus) && !(objCtrl is MenuPanel) && !(objCtrl is Controls.HVTTMarkTextBox))
				{
        ...
        /// This method is used by internal HVTTCONTROLS implementation and you should not use it.
		/// </summary>
		...
		/// </summary>
		[Browsable(false),HVTTBrowsable(false),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			g.PageUnit=GraphicsUnit.Pixel;
				...
				if(m_ItemContainer.Style==HVTTControlStyle.Office2000)
					PaintOffice(g);
				else
					PaintDotNet(e);
		...
		protected override void OnResize(EventArgs e)
		{
			...
			if(!(this.Parent is DockSite || this.Parent is FloatingContainer) && m_BarState==HVTTBarState.Docked && !m_LayoutSuspended)
				this.RecalcLayout();
			ResizeDockTab();
		}
        ...
        private bool IsGradientStyle
        {
            get
            {
                return (this.Style == HVTTControlStyle.Office2003 || this.Style == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(this.Style));
            }
        }
        ...
        internal ItemPaintArgs GetItemPaintArgs(Graphics g)
        {
            ...
            if (m_BarState==HVTTBarState.Popup && m_ParentItem!=null && m_ParentItem.DesignMode)
            {
		...
		protected void PaintDotNet(PaintEventArgs e)
		{
			...
			Pen p;
			if(m_BarState==HVTTBarState.Popup)
				p=new Pen(pa.Colors.BarPopupBorder,1);
			else
				p=new Pen(pa.Colors.BarFloatingBorder);

            if (this.DisplayShadow && !this.AlphaShadow && m_ParentItem!=null && m_ParentItem.Style!=HVTTControlStyle.Office2007)
				SetupRegion();
			...
			if(m_BarState==HVTTBarState.Popup)
			{
                ...
                if(!re.Cancel)
                {
                    if (m_ParentItem != null && BarFunctions.IsOffice2007Style(m_ParentItem.Style) && GetRenderer() != null)
                    {
                    ...
                    else
                    {
                        ...
                        if (m_ParentItem != null && BarFunctions.IsOffice2007Style(m_ParentItem.Style))
                        ...
                        if (m_ParentItem != null && m_ParentItem.Style != HVTTControlStyle.Office2007 && m_ParentItem is ButtonItem && m_ParentItem.Displayed)
                        {
            ...
            else if (m_BarState == HVTTBarState.Floating)
            {
                ...
                if (this.DrawThemedCaption)
                {
                    ...
                    if (!re.Cancel)
                    {
                        ...
                        if (this.LayoutType == HVTTLayoutType.DockContainer && !m_HasFocus)
                ...
                else if (m_GrabHandleStyle == HVTTGrabHandleStyle.Caption && this.LayoutType == HVTTLayoutType.DockContainer && this.Style == HVTTControlStyle.Office2000)
                {
                ...
                else
                {
                    ...
                    if (!re.Cancel)
                    {
                        ...
                        if (this.GrabHandleStyle != HVTTGrabHandleStyle.CaptionTaskPane)
                        {
                            sm = g.SmoothingMode;
                            g.SmoothingMode = SmoothingMode.Default;
                            ...
                            {
                                if (this.LayoutType == HVTTLayoutType.Toolbar || m_HasFocus)
                    ...
                    if (r.Width > 0 && drawCaptionText)
                    {
                        ...
                        if (!re.Cancel)
                        {
                            ...
                            if (m_CaptionForeColor.IsEmpty)
                            {
                                ...
                                //else
                                TextDrawing.DrawString(g, this.Text, objFont, ((m_HasFocus || this.LayoutType == HVTTLayoutType.Toolbar) ? pa.Colors.BarCaptionText : pa.Colors.BarCaptionInactiveText), r, sf);
                            }
                            else
            ...
            else if (m_BarState == HVTTBarState.Docked && BarFunctions.IsOffice2007Style(this.Style) && this.GetRenderer() != null && (!this.IsThemed || this.MenuBar))
            {
            ...
            else
            {
                ...
                if (m_ItemContainer.m_BackgroundColor.IsEmpty && this.BackColor != Color.Transparent)
                {
                    if (this.IsThemed)
                    {
                        ...
                        theme.DrawBackground(g, ThemeRebarParts.Background, ThemeRHVTTBarStates.Normal, r);
                    }
                    else
                    {
                        sm = g.SmoothingMode;
                        ...
                        if (!re.Cancel)
                        {
                            ...
                            {
                                if (this.Style == HVTTControlStyle.VS2005 && this.LayoutType == HVTTLayoutType.DockContainer && !this.BackColor.IsEmpty)
                                {
                ...
                if (drawBorder)
                {
                    if (IsGradientStyle && !this.IsThemed && this.LayoutType == HVTTLayoutType.Toolbar && !this.MenuBar && this.BarType == eBarType.Toolbar && this.BackColor != Color.Transparent)
                    {
            ...
            if (m_BarType == eBarType.StatusBar && this.GrabHandleStyle == HVTTGrabHandleStyle.ResizeHandle)
            {
                PaintGrabHandle(pa);
            }
		...
		private bool GradientBackground
		{
			get
			{
                if (this.Style == HVTTControlStyle.VS2005 && this.LayoutType == HVTTLayoutType.DockContainer)
					return false;
				return true;
			}
		}
		...
		internal void PaintGrabHandle(ItemPaintArgs pa)
		{
			...
			// Draw grab handles
			if(m_GrabHandleStyle!=HVTTGrabHandleStyle.None)
			{
				Rectangle r=Rectangle.Empty;

                if (m_GrabHandleStyle != HVTTGrabHandleStyle.Caption && m_GrabHandleStyle != HVTTGrabHandleStyle.CaptionTaskPane && m_GrabHandleStyle != HVTTGrabHandleStyle.CaptionDotted && this.IsThemed && m_GrabHandleStyle != HVTTGrabHandleStyle.ResizeHandle)
				{
					ThemeRebar theme=this.ThemeRebar;
					ThemeRebarParts part=ThemeRebarParts.Gripper;

					if(m_ItemContainer.Orientation==HVTTOrientation.Vertical)
					{
                    ...
                    r = re.Bounds;
                    if(!re.Cancel)
					    theme.DrawBackground(g,part,ThemeRHVTTBarStates.Normal,r);
					return;
				}
				...
				{
					case HVTTGrabHandleStyle.Single:
					{
						if(m_ItemContainer.Orientation==HVTTOrientation.Horizontal && m_ItemContainer.LayoutType!=HVTTLayoutType.DockContainer)
						{
					...
					case HVTTGrabHandleStyle.Double:
					{
						if(m_ItemContainer.Orientation==HVTTOrientation.Horizontal && m_ItemContainer.LayoutType!=HVTTLayoutType.DockContainer)
						{
					...
					case HVTTGrabHandleStyle.DoubleThin:
					{
						if(m_ItemContainer.Orientation==HVTTOrientation.Horizontal && m_ItemContainer.LayoutType!=HVTTLayoutType.DockContainer)
						{
					...
					case HVTTGrabHandleStyle.DoubleFlat:
					{
					...
					case HVTTGrabHandleStyle.Dotted:
					{
                        re = new RenderBarEventArgs(this, g, eBarRenderPart.GrabHandle, m_GrabHandleRect);
                        OnPreRender(re);
                        if (!re.Cancel)
					...
					case HVTTGrabHandleStyle.Stripe:
					{
                        re = new RenderBarEventArgs(this, g, eBarRenderPart.GrabHandle, m_GrabHandleRect);
                        OnPreRender(re);
                        if (!re.Cancel)
					...
					case HVTTGrabHandleStyle.StripeFlat:
					{
                        re = new RenderBarEventArgs(this, g, eBarRenderPart.GrabHandle, m_GrabHandleRect);
                        OnPreRender(re);
                        if (!re.Cancel)
					...
					case HVTTGrabHandleStyle.CaptionTaskPane:
                    case HVTTGrabHandleStyle.CaptionDotted:
					{
                        ...
                        if (!re.Cancel)
                        {
                            if (m_BarState == HVTTBarState.Floating)
                            {
                            ...
                            else
                            {
                                ...
                                {
                                    if (m_HasFocus && m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionDotted)
                        ...
                        if(m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionDotted)
                            PaintCaptionText(pa, targetRect);
						break;
					}
					case HVTTGrabHandleStyle.Caption:
					{
						Rectangle targetRect=m_GrabHandleRect;
                        re = new RenderBarEventArgs(this, g, eBarRenderPart.Caption, m_GrabHandleRect);
                        OnPreRender(re);
                        if (!re.Cancel)
                        {
                            ...
                            if (m_CaptionBackColor.IsEmpty)
                            {
                                    ...
                                    else
                                    {
                                        if (m_HasFocus)
                                        {
                                            if (pa.Colors.BarCaptionBackground2.IsEmpty)
                                                g.FillRectangle(new SolidBrush(pa.Colors.BarCaptionBackground), targetRect);
                                            else
                                            {
                                        ...
                                        else
                                        {
                                            if (pa.Colors.BarCaptionInactiveBackground2.IsEmpty)
                                                g.FillRectangle(new SolidBrush(pa.Colors.BarCaptionInactiveBackground), targetRect);
                                            else
                                            {
                                            ...
                                            if (this.Style != HVTTControlStyle.VS2005)
                                            {
                                ...
                                g.FillRectangle(new SolidBrush(m_CaptionBackColor), targetRect);
                        }
					...
					case HVTTGrabHandleStyle.ResizeHandle:
					{
					...
					case HVTTGrabHandleStyle.Office2003:
					case HVTTGrabHandleStyle.Office2003SingleDot:
					{
                        re = new RenderBarEventArgs(this, g, eBarRenderPart.GrabHandle, m_GrabHandleRect);
                        OnPreRender(re);
                        if (!re.Cancel)
                        {
                            ...
                            if (m_GrabHandleStyle == HVTTGrabHandleStyle.Office2003SingleDot)
                            {
            ...
            if (m_CaptionForeColor.IsEmpty)
            {
                ...
                else
                {
                    ...
                    if (targetRect.Width > 0)
                    {
                        ...
                        else if ((this.Style == HVTTControlStyle.OfficeXP || this.Style == HVTTControlStyle.Office2003 || this.Style == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(this.Style)) && !m_HasFocus)
		...
		internal bool DrawThemedCaption
		{
			get
			{
                if (this.Style == HVTTControlStyle.Office2007)
                    return false;
				if(this.IsThemed || m_BarState==HVTTBarState.Floating && BarFunctions.ThemedOS && Themes.ThemesActive && this.LayoutType==HVTTLayoutType.DockContainer)
					return true;
				return false;
			}
		}
		...
		{
			bool ret=false;
			if(m_CanAutoHide && m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && m_BarState!=HVTTBarState.Floating)
			{
			...
			if(m_ItemContainer.SubItems.Count>0)
			{
				...
				else if(m_BarState==HVTTBarState.Floating)
				{
					...
					if(m_CaptionBackColor.IsEmpty)
						...
						g.FillRectangle(new SolidBrush(m_CaptionBackColor),4,4,this.ClientSize.Width-8,15);
			...
			else
			{
				...
				if(m_SystemButtons.MouseDownClose)
				{
					if(pa.Colors.ItemPressedBackground2.IsEmpty)
						g.FillRectangle(new SolidBrush(pa.Colors.ItemPressedBackground),m_SystemButtons.CloseButtonRect);
					else
					{
					...
					NativeFunctions.DrawRectangle(g,new Pen(pa.Colors.ItemPressedBorder),m_SystemButtons.CloseButtonRect);
				}
				else if(m_SystemButtons.MouseOverClose)
				{
					...
					if(!pa.Colors.ItemHotBackground2.IsEmpty)
					{
						...
						g.FillRectangle(new SolidBrush(pa.Colors.ItemHotBackground),m_SystemButtons.CloseButtonRect);
					NativeFunctions.DrawRectangle(g,new Pen(pa.Colors.ItemHotBorder),m_SystemButtons.CloseButtonRect);
				}

                if (m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionTaskPane || m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionDotted)
					pen=new Pen(pa.Colors.ItemText);
				else
                    pen = new Pen(((m_HasFocus || this.LayoutType == HVTTLayoutType.Toolbar) ? pa.Colors.BarCaptionText : pa.Colors.BarCaptionInactiveText));
                ...
                else if (!m_HasFocus && m_BarState != HVTTBarState.Floating && m_GrabHandleStyle != HVTTGrabHandleStyle.CaptionTaskPane && m_GrabHandleStyle != HVTTGrabHandleStyle.CaptionDotted)
			...
			else
			{
				if(m_CaptionMenu!=null && m_CaptionMenu.Expanded)
				{
					if(pa.Colors.ItemExpandedBackground2.IsEmpty)
						g.FillRectangle(new SolidBrush(pa.Colors.ItemExpandedBackground),rect);
					else
					{
					...
					NativeFunctions.DrawRectangle(g,new Pen(pa.Colors.ItemExpandedBorder),rect);
				}
				else if(m_SystemButtons.MouseDownCaption)
				{
					if(pa.Colors.ItemPressedBackground2.IsEmpty)
						g.FillRectangle(new SolidBrush(pa.Colors.ItemPressedBackground),rect);
					else
					{
					...
					NativeFunctions.DrawRectangle(g,new Pen(pa.Colors.ItemPressedBorder),rect);
				}
				else if(m_SystemButtons.MouseOverCaption)
				{
					...
					if(!pa.Colors.ItemHotBackground2.IsEmpty)
					{
						...
						g.FillRectangle(new SolidBrush(pa.Colors.ItemHotBackground),rect);
					NativeFunctions.DrawRectangle(g,new Pen(pa.Colors.ItemHotBorder),rect);
				}
			}
		...
		private void PaintCustomizeButton(ItemPaintArgs pa)
		{
			if(m_SystemButtons.CustomizeButtonRect.IsEmpty)
				return;
			Graphics g=pa.Graphics;
            Brush brush = new SolidBrush(((m_HasFocus || this.LayoutType == HVTTLayoutType.Toolbar) ? pa.Colors.BarCaptionText : pa.Colors.BarCaptionInactiveText));
			...
			if(!this.DrawThemedCaption)
			{
				...
				else
				{
					...
					if(!m_HasFocus && m_BarState!=HVTTBarState.Floating)
						brush=new SolidBrush(pa.Colors.BarCaptionInactiveText);

					if(m_SystemButtons.MouseDownCustomize)
					{
						if(pa.Colors.ItemPressedBackground2.IsEmpty)
							g.FillRectangle(new SolidBrush(pa.Colors.ItemPressedBackground),m_SystemButtons.CustomizeButtonRect);
						else
						{
						...
						NativeFunctions.DrawRectangle(g,new Pen(pa.Colors.ItemPressedBorder),m_SystemButtons.CustomizeButtonRect);
					}
					else if(m_SystemButtons.MouseOverCustomize)
					{
						...
						if(!pa.Colors.ItemHotBackground2.IsEmpty)
						{
							...
							g.FillRectangle(new SolidBrush(pa.Colors.ItemHotBackground),m_SystemButtons.CustomizeButtonRect);
						NativeFunctions.DrawRectangle(g,new Pen(pa.Colors.ItemHotBorder),m_SystemButtons.CustomizeButtonRect);
					}
				}
			}
            ...
            if (m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionTaskPane || m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionDotted)
                ...
                brush = new SolidBrush(((m_HasFocus || this.LayoutType == HVTTLayoutType.Toolbar) ? pa.Colors.BarCaptionText : pa.Colors.BarCaptionInactiveText));
		...
		private void PaintAutoHideButton(ItemPaintArgs pa)
		{
			...
			else
			{
				...
				{
					if(pa.Colors.ItemPressedBackground2.IsEmpty)
						g.FillRectangle(new SolidBrush(pa.Colors.ItemPressedBackground),m_SystemButtons.AutoHideButtonRect);
					else
					{
					...
					NativeFunctions.DrawRectangle(g,new Pen(pa.Colors.ItemPressedBorder),m_SystemButtons.AutoHideButtonRect);
				}
				else if(m_SystemButtons.MouseOverAutoHide)
				{
					if(!pa.Colors.ItemHotBackground2.IsEmpty)
					{
						...
						g.FillRectangle(new SolidBrush(pa.Colors.ItemHotBackground),m_SystemButtons.AutoHideButtonRect);
					NativeFunctions.DrawRectangle(g,new Pen(pa.Colors.ItemHotBorder),m_SystemButtons.AutoHideButtonRect);
				}
			}
			if(m_SystemButtons.MouseDownAutoHide && !this.AutoHide || this.AutoHide && !m_SystemButtons.MouseDownAutoHide)
			{
		...
		{
			if(m_SideBarImage.Picture==null || m_BarState!=HVTTBarState.Popup)
				return;
			...
			if(!m_SideBarImage.GradientColor1.IsEmpty && !m_SideBarImage.GradientColor2.IsEmpty)
			{
				...
				g.FillRectangle(new SolidBrush(m_SideBarImage.BackColor),m_SideBarRect);
		...
		public void RecalcLayout()
		{
			...
			if(m_BarState==HVTTBarState.Docked)
			{
				if(this.Parent!=null && !PassiveBar)
				{
					if(m_ItemContainer.LayoutType!=HVTTLayoutType.DockContainer)
		...
		internal void OnDockContainerWidthChanged(DockContainerItem item, int width)
		{
            ...
            else if (m_BarState == HVTTBarState.AutoHide)
            {
		...
		private void ChangeSplitSize(System.Drawing.Size size)
		{
			...
			{
				HVTTMarkStatus barLeft=null, barRight=null;
				
				HVTTMarkStatus previous=this.GetPreviousVisibleBar(this);
				HVTTMarkStatus next=this.GetNextVisibleBar(this);
				if(next!=null && next.LayoutType==HVTTLayoutType.DockContainer && next.DockLine==this.DockLine)
				{
					barLeft=this;
					barRight=next;
				}
				else if(previous!=null && previous.LayoutType==HVTTLayoutType.DockContainer && previous.DockLine==this.DockLine)
				{
					barLeft=previous;
					barRight=this;
					if(size.Width>0)
				...
				if(this.DockSide==eDockSide.Left || this.DockSide==eDockSide.Right)
				{
					...
					{
						System.Drawing.Size minLeftSize=barLeft.MinimumDockSize(HVTTOrientation.Horizontal);
						System.Drawing.Size minThisSize=barRight.MinimumDockSize(HVTTOrientation.Horizontal);
		...
		internal int SplitDockHeight
		{
			get
			{
				if(this.Parent!=null && m_SplitDockHeightPercent>0)
				{
					...
					Size sz=GetAdjustedFullSize(this.MinimumDockSize(HVTTOrientation.Horizontal));
					if(height<sz.Height)
						height=sz.Height;
					return height;
				}
		...
		public void RecalcSize()
		{
			...
			if(this.LayoutType==HVTTLayoutType.DockContainer && m_TabDockItems==null)
			...
			try
			{
				...
				else if(m_BarState==HVTTBarState.Floating || m_BarState==HVTTBarState.AutoHide)
				{
				...
				if(m_ItemContainer.Style==HVTTControlStyle.Office2000)
				{
					if(m_BarState==HVTTBarState.Docked && !(this.Parent is DockSite))
					{
						System.Drawing.Size size=RecalcSizeOffice();
						if(m_ItemContainer.Orientation==HVTTOrientation.Horizontal)
				...
				if(m_BarState==HVTTBarState.Docked)
				{
					...
					this.Region=null;
				if(m_BarState==HVTTBarState.Floating && !PassiveBar)
				{
			...
			if(this.LayoutType==HVTTLayoutType.DockContainer && m_TabDockItems!=null && this.DesignMode && this.Site!=null)
				RefreshDockTab(false);
		}

		private void RefreshOffice2003Region()
		{
            if (m_RoundCorners && IsGradientStyle && !this.MenuBar && this.LayoutType == HVTTLayoutType.Toolbar && !this.IsThemed && this.GrabHandleStyle != HVTTGrabHandleStyle.ResizeHandle && this.BackColor != Color.Transparent)
			{
		...
		private System.Drawing.Size RecalcSizeOnly(System.Drawing.Size objFrameSize, HVTTOrientation barOrientation, HVTTBarState barState, bool bWrapItems)
		{
			if(m_ItemContainer.SubItems.Count>0)
			{
				if(m_ItemContainer.Style==HVTTControlStyle.Office2000)
		...
		private System.Drawing.Size RecalcSizeDotNet(bool bCalculateOnly, System.Drawing.Size objFrameSize, HVTTOrientation barOrientation, HVTTBarState barState, bool bWrapItems)
		{
			...
			HVTTOrientation oldOrientation=m_ItemContainer.Orientation;
			HVTTBarState thisBarState=m_BarState;
			...
			if(bCalculateOnly)
			{
				...
				if(barState==HVTTBarState.Docked)
					m_ItemContainer.Stretch=m_DockStretch;
				else
				{
					if(m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer)
			...
			if(thisBarState==HVTTBarState.Docked)
			{
				if(m_DockedBorder!=eBorderType.None)
				{
					iTopMargin=3;
			...
			else if(thisBarState==HVTTBarState.Floating)
			{
			...
			if(m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && !m_AutoHideState && GetDockTabVisible())
			{
				if(m_ItemContainer.VisibleSubItems>1 || m_AlwaysDisplayDockTab && m_ItemContainer.VisibleSubItems==1 && m_BarState!=HVTTBarState.Floating)
				{
			...
			if(thisBarState==HVTTBarState.Popup && m_SideBarImage.Picture!=null)
				iLeft+=m_SideBarImage.Picture.Width;
			else if(thisBarState==HVTTBarState.Docked || this.BarState==HVTTBarState.AutoHide)
			{
				// Show grab handles if any selected
				if(m_GrabHandleStyle!=HVTTGrabHandleStyle.None)
				{
					if(m_GrabHandleStyle==HVTTGrabHandleStyle.ResizeHandle)
					{
					...
					{
						if(m_ItemContainer.Orientation==HVTTOrientation.Horizontal && m_ItemContainer.LayoutType==HVTTLayoutType.Toolbar)
						{

                            if (m_GrabHandleStyle == HVTTGrabHandleStyle.Caption)
						...
						else
						{
                            ...
                            else if (m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionTaskPane || m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionDotted)
			...
			if(!bCalculateOnly)
			{
				if(thisBarState==HVTTBarState.Popup)
			...
			if(m_ItemContainer.VisibleSubItems==0 || (m_ItemContainer.SubItems.Count==1 && m_ItemContainer.SubItems[0] is CustomizeItem && !m_ItemContainer.SubItems[0].Visible))
			{
				...
				if(this.LayoutType!=HVTTLayoutType.DockContainer && this.DockSide!=eDockSide.Document)
				{
			...
			if(IsGradientStyle && this.LayoutType==HVTTLayoutType.Toolbar && !this.MenuBar && !this.IsThemed)
				iTop++;
            ...
            if (!bCalculateOnly)
            {
                if (this.RightToLeft == RightToLeft.Yes)
                {
                    if (this.LayoutType == HVTTLayoutType.Toolbar && m_ItemContainer.WidthInternal < objFrameSize.Width - iLeft - iRightMargin - iRightTabMargin && this.Stretch)
                    {
			...
			if(!bCalculateOnly)
			{
				if((thisBarState==HVTTBarState.Docked || thisBarState==HVTTBarState.AutoHide) && m_GrabHandleStyle!=HVTTGrabHandleStyle.None && m_GrabHandleStyle!=HVTTGrabHandleStyle.ResizeHandle)
				{
					if(m_ItemContainer.Orientation==HVTTOrientation.Horizontal && m_ItemContainer.LayoutType==HVTTLayoutType.Toolbar)
					{
                        if (m_GrabHandleStyle == HVTTGrabHandleStyle.Caption)
				...
				{
					if(m_GrabHandleStyle==HVTTGrabHandleStyle.CaptionTaskPane)
					{
		...
		private System.Drawing.Size RecalcSizeOffice(bool bCalculateOnly, System.Drawing.Size objFrameSize, HVTTOrientation barOrientation, HVTTBarState barState, bool bWrapItems)
		{
			System.Drawing.Size thisSize=System.Drawing.Size.Empty;
			...
			if(!bCalculateOnly)
			{
				if((thisBarState==HVTTBarState.Docked || this.BarState==HVTTBarState.AutoHide) && m_GrabHandleStyle!=HVTTGrabHandleStyle.None)
				{
					if(m_ItemContainer.Orientation==HVTTOrientation.Horizontal && m_ItemContainer.LayoutType!=HVTTLayoutType.DockContainer)
					{
		...
		/// internally to optimize loading of hidden floating bars. You should not use this property in your code. It is for internal HVTTCONTROLS
        ...
        internal void DockDocumentManager(DockSiteInfo pDockInfo)
        {
		...
		internal void DockingHandler(DockSiteInfo pDockInfo,Point p)
		{
			...
			if(pDockInfo.objDockSite==null && (m_BarState!=HVTTBarState.Floating || m_Float==null))
			{
				...
				// Undock the window
				m_BarState=HVTTBarState.Floating;
				if(m_Float==null)
				{
				...
				this.DockOrientation=HVTTOrientation.Horizontal;
				if(m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && m_AlwaysDisplayDockTab)
					RefreshDockTab(true);
				this.RecalcSize();
				m_FloatingRect.Size=this.Size;

				m_Float.Location=p;
				if(m_ItemContainer.LayoutType!=HVTTLayoutType.Toolbar)
			...
			else
			{
				...
				if(m_Owner!=null)
				{
					...
					else
					{
						...
						if(!this.Visible && !m_BarDefinitionLoading)
							this.Visible=true;
						if(pDockInfo.objDockSite!=null && m_BarState!=HVTTBarState.Docked)
						{
							m_DockingInProgress=true;
							if(m_Float!=null && m_BarState==HVTTBarState.Floating)
							{
								if(owner.ParentForm!=null)
									owner.ParentForm.Activate();
								// Remember undocked size
...
//								ownerDockEvents.InvokeBarDock(this,new EventArgs());
				...
				if(m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && (m_TabDockItems==null || !m_TabDockItems.Visible))
				{
					int iVisible=m_ItemContainer.VisibleSubItems;
					if(!(m_BarState==HVTTBarState.Floating && iVisible<=1) && (m_AlwaysDisplayDockTab || iVisible>0) )
					{
						HVTTBarState oldBarState=m_BarState;
						m_BarState=HVTTBarState.Docked;
						RefreshDockTab(false);
		...
		internal HVTTMarkStatus TempTabBar
		{
		...
		/// <summary>
		/// Method is called by HVTTManager when bar is being moved and Escape key is pressed.
		/// </summary>
		...
		protected override void OnMouseMove(MouseEventArgs e)
		{
			...
			if(m_MoveWindow && m_BarState!=HVTTBarState.Popup && e.Button==System.Windows.Forms.MouseButtons.Left)
			{
				...
				if(pDockInfo.UseOutline && pDockInfo.objDockSite!=null) // && pDockInfo.TabDockContainer==null && m_TempTabBar==null)
				{
				...
				if(bPreview)
				{
					//if(m_BarState==HVTTBarState.Floating || this.LayoutType!=HVTTLayoutType.DockContainer || pDockInfo.TabDockContainer!=null)
			...
			else if(e.Button==System.Windows.Forms.MouseButtons.None)
			{
				if(m_BarState==HVTTBarState.Floating)
				{
					if(m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && ((e.X<=2 && e.Y<=2) || (e.X>=this.Width-4 && e.Y>=this.Height-4)))
						this.Cursor=System.Windows.Forms.Cursors.SizeNWSE;
					else if(m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && ((e.X>=this.Width-2 && e.Y<=2) || (e.X<=4 && e.Y>=this.Height-4)))
                ...
                //else if(m_BarState==HVTTBarState.Docked && m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && m_ItemContainer.Stretch)
                //{
                ...
                //}
				else if(m_BarState==HVTTBarState.AutoHide && m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && m_ItemContainer.Stretch)
				{
                ...
                else if (m_BarState == HVTTBarState.Docked && m_GrabHandleStyle == HVTTGrabHandleStyle.ResizeHandle)
                {
                ...
                if (m_ItemContainer.Style == HVTTControlStyle.OfficeXP || m_ItemContainer.Style == HVTTControlStyle.Office2003 || m_ItemContainer.Style == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(this.Style))
				{
			...
			else if(e.Button==System.Windows.Forms.MouseButtons.Left && m_SizeWindow!=0 && m_BarState==HVTTBarState.Floating)
			{
				...
				if(m_SizeWindow==SIZE_NWN || m_SizeWindow==SIZE_NWS)
				{
					...
					if(!oldSize.Equals(newSize) && newSize.Width>=minSize.Width && newSize.Height>=minSize.Height)
					{
						...
						RecalcSize();
						this.Update();
						// Updates the parent form, there was a very bad lagging painting effect when dockable window is resized and the HVTTCONTROLS controls where behind it.
						IOwner owner=m_Owner as IOwner;
				...
				else if(m_SizeWindow==SIZE_S)
				{
					if(e.Y>0)
					{
						if(m_ItemContainer.LayoutType==HVTTLayoutType.TaskList || m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer)
            ...
            else if (e.Button == System.Windows.Forms.MouseButtons.Left && m_SizeWindow != 0 && m_BarState == HVTTBarState.AutoHide || m_SizeWindow == SIZE_PARENTRESIZE && m_BarState == HVTTBarState.Docked) //(m_BarState == HVTTBarState.Docked || m_BarState == HVTTBarState.AutoHide))
			{
				System.Drawing.Size oldSize=this.ClientSize;
				System.Drawing.Size newSize=System.Drawing.Size.Empty;
				...
				else if(m_SizeWindow==SIZE_HSPLITRIGHT)
				{
                    ...
                    if (m_Owner is HVTTManager) minClientSize = ((HVTTManager)m_Owner).MinimumClientSize.Width;
				...
				else if(m_SizeWindow==SIZE_VSPLITBOTTOM)
				{
					...
					int mouseY=e.Y;
                    int minClientSize = 32;
                    if (m_Owner is HVTTManager) minClientSize = ((HVTTManager)m_Owner).MinimumClientSize.Height;
				...
				{
					HVTTMarkStatus barLeft=GetPreviousVisibleBar(this); //this.Parent.Controls[iIndex] as Bar;
					if(barLeft!=null && barLeft.DockLine==this.DockLine)
					{
						System.Drawing.Size minLeftSize=GetAdjustedFullSize(barLeft.MinimumDockSize(HVTTOrientation.Horizontal));
						System.Drawing.Size minThisSize=GetAdjustedFullSize(this.MinimumDockSize(HVTTOrientation.Horizontal));
                        int x = e.X;
						...
						if(barLeft.Width+x>=minLeftSize.Width && this.Width-x>=minThisSize.Width)
						{
							...
							if(!oldLeftBarSize.Equals(newBarLeftSize) && !oldSize.Equals(newSize))
							{
                                ...
                                {
                                    if (c != this && c != barLeft && c.Visible && c is HVTTMarkStatus)
                                    {
                                        HVTTMarkStatus b = c as HVTTMarkStatus;
            ...
            if (m_BarState==HVTTBarState.Popup && m_ParentItem != null && m_ParentItem.DesignMode && e.Button == System.Windows.Forms.MouseButtons.Left && (Math.Abs(e.X - m_MouseDownPt.X) >= 2 || Math.Abs(e.Y - m_MouseDownPt.Y) >= 2 || m_DragDropInProgress))
            {
		...
		private HVTTMarkStatus GetPreviousVisibleBar(HVTTMarkStatus startBar)
		{
			if(this.Parent==null)
				return null;
			...
			if(iIndex>=0)
			{
				for(int i=iIndex;i>=0;i--)
				{
					if(this.Parent.Controls[i].Visible)
						return this.Parent.Controls[i] as HVTTMarkStatus;
				}
			}
			return null;
		}

		private HVTTMarkStatus GetNextVisibleBar(HVTTMarkStatus startBar)
		{
			if(this.Parent==null)
				return null;
			int iIndex=this.Parent.Controls.IndexOf(this)+1;
		...
		{
			IOwner owner=m_Owner as IOwner;
			if(owner==null || this.Parent==null || (this.Parent.Parent==null && m_BarState!=HVTTBarState.AutoHide))
				return 0;
			...
			if(m_BarState==HVTTBarState.AutoHide && this.Visible)
				width-=this.Width;
			return width;
		}
		...
		internal void SyncLineMinWidth()
		{
			...
			{
				HVTTMarkStatus bar=ctrl as HVTTMarkStatus;
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
			...
			else if(e.Button==System.Windows.Forms.MouseButtons.Left && m_BarState==HVTTBarState.Floating)
			{
				if(m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && e.X<=4 && e.Y<=4)
					m_SizeWindow=SIZE_NWN;
				else if(m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && e.X>=this.Width-4 && e.Y>=this.Height-4)
					m_SizeWindow=SIZE_NWS;
				else if(m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && e.X>=this.Width-4 && e.Y<=4)
					m_SizeWindow=SIZE_NEN;
				else if(m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && e.X<=4 && e.Y>=this.Height-4)
            ...
            //else if(e.Button==System.Windows.Forms.MouseButtons.Left && m_BarState==HVTTBarState.Docked && m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && m_ItemContainer.Stretch)
            //{
            ...
            //}
			else if(e.Button==System.Windows.Forms.MouseButtons.Left && m_BarState==HVTTBarState.AutoHide && m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && m_ItemContainer.Stretch)
			{
			...
			else if(e.Button==MouseButtons.Right && this.CanCustomize && !m_ItemContainer.DesignMode && m_BarState!=HVTTBarState.Popup && !m_MoveWindow)
			{
				HVTTManager owner=m_Owner as HVTTManager;
				if(owner!=null)
				{
            ...
            if (e.Button == MouseButtons.Left && m_BarState == HVTTBarState.Docked && this.m_GrabHandleStyle == HVTTGrabHandleStyle.ResizeHandle
            ...
            else if (e.Button == MouseButtons.Left && m_BarState == HVTTBarState.Docked && this.m_GrabHandleStyle == HVTTGrabHandleStyle.ResizeHandle
		...
		private void EndDocking(bool revertLast, Point mousePos)
		{
			...
			if(m_TempTabBar!=null || m_DragDockInfo.TabDockContainer!=null)
			{
				...
				HVTTMarkStatus targetBar=m_TempTabBar;
				if(targetBar==null)
				...
				if(m_DragDockInfo.TabDockContainer!=this)
				{
					if(m_DockTabTearOffIndex==-1)
					{
                        ...
                        HVTTManager manager = m_Owner as HVTTManager;
                        if(manager!=null)
						{
                            m_MoveWindow = false;
                            targetBar = null;
			...
			if(m_DockTabTearOffIndex!=-1)
			{
				...
				else
				{
					...
					HVTTMarkStatus bar=TearOffDockContainerItem(dc,false);
		...
		internal HVTTMarkStatus StartTabDrag()
		{
			...
			if(this.VisibleItemCount==1 && this.GrabHandleStyle==HVTTGrabHandleStyle.None)
			{
				this.StartBarMove();
				return this;
			}
			...
			HVTTMarkStatus bar=TearOffDockContainerItem(item,true);
			bar.InitalFloatLocation=Point.Empty;
			bar.StartBarMove();
			return bar;
		}

		private HVTTMarkStatus TearOffDockContainerItem(DockContainerItem item, bool floatBar)
		{
            Form f = this.FindForm();
            if (f != null)
            {
			...
			this.RecalcLayout();
			HVTTMarkStatus bar=BarFunctions.CreateDuplicateDockBar(this);
			bar.Text=item.Text;
			Point initalFloatLocation=new Point(Control.MousePosition.X-32,Control.MousePosition.Y-8);
			bar.InitalFloatLocation=initalFloatLocation;
			...
			if(floatBar)
			{
				...
				bar.Location=initalFloatLocation;
			}
		...
		{
			CloseDockTab(dockTab,EventSource.Code);
		}
		...
		public void CloseDockTab(DockContainerItem dockTab, EventSource source)
		{
		...
		internal void InvokeDockTabClosing(DockTabClosingEventArgs e)
		{
			...
			if(this.Owner is HVTTManager)
			{
				((HVTTManager)this.Owner).InvokeDockTabClosing(this,e);
			}
		}
		...
		protected override void OnDoubleClick(EventArgs e)
		{
			...
			if(/*m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && */m_GrabHandleRect.Contains(this.PointToClient(Control.MousePosition)))
			{
				if(m_BarState==HVTTBarState.Floating && (this.CanDockBottom || this.CanDockTop || this.CanDockLeft || this.CanDockRight))
				{
					DockingHandler(m_LastDockSiteInfo,Point.Empty);
				}
				else if(m_BarState==HVTTBarState.Docked && this.CanUndock && this.Parent is DockSite)
				{
					Point p=Point.Empty;
		...
		protected override void OnVisibleChanged(EventArgs e)
		{
			...
			// to the Dock Site.
            if (m_BarState == HVTTBarState.Docked && !m_DockingInProgress && this.Visible)
            {
                this.RecalcLayout();
            }
		...
		protected override void OnParentChanged(EventArgs e)
		{
			...
			if(this.Parent!=null && !(this.Parent is FloatingContainer) && !(this.Parent is DockSite))
			{
				...
				{
					HVTTManager.RegisterOwnerParentMsgHandler(this,this.FindForm());
		...
		protected override void OnHandleCreated(EventArgs e)
		{
			...
			//if(!(this.Parent is DockSite) || this.Parent is DockSite && this.Parent.Dock==DockStyle.Fill)
				this.RecalcSize();
		}
		...
		private int ClientMarginLeft
		{
			get
			{
				...
				if(m_ParentItem!=null && m_ParentItem.Style!=HVTTControlStyle.Office2000)
					iMargin=1;
				else
					iMargin=3;
		...
		internal bool DisplayShadow
		{
			get
			{
				...
				if(ownersupport!=null)
				{
					if(m_ParentItem!=null && m_ParentItem.Style==HVTTControlStyle.Office2000)
					{
						if(ownersupport.MenuDropShadow==HVTTMenuDropShadow.Show)
							return true;
						else
							return false;
					}
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),DefaultValue(HVTTPopupAnimation.ManagerControlled)]
		public HVTTPopupAnimation PopupAnimation
		{
			get
			{
		...
		internal void Serialize(System.Xml.XmlElement xmlThisBar, bool bPropertiesOnly)
		{
			...
			if(m_AutoHideState)
			{
				xmlThisBar.SetAttribute("state",System.Xml.XmlConvert.ToString((int)HVTTBarState.Docked));
			...
			IOwnerBarSupport ownersupport=m_Owner as IOwnerBarSupport;
			if(m_BarState==HVTTBarState.Floating && !this.Visible && ownersupport!=null && ownersupport.WereVisible.Count>0 && ownersupport.WereVisible.Contains(this))
			...
			if(m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && this.SelectedDockTab>=0)
		...
		internal void Deserialize(System.Xml.XmlElement xmlThisBar, ItemSerializationContext context)
		{
				...
				foreach(System.Xml.XmlElement xmlElem in xmlThisBar.ChildNodes)
				{
					switch(xmlElem.Name)
					{
						...
						case "backimage":
						{
							...
							m_BackgroundImagePosition=(BackgroundImagePosition)System.Xml.XmlConvert.ToInt32(xmlElem.GetAttribute("pos"));
		...
		private void LoadCommonProperties(System.Xml.XmlElement xmlBar)
		{
			...
			if(xmlBar.HasAttribute("grabhandle"))
				m_GrabHandleStyle=(HVTTGrabHandleStyle)System.Xml.XmlConvert.ToInt32(xmlBar.GetAttribute("grabhandle"));
			...
			if(xmlBar.HasAttribute("imagesize"))
				m_ImageSize=(BarImageSize)System.Xml.XmlConvert.ToInt32(xmlBar.GetAttribute("imagesize"));
...
//				m_ItemContainer.ItemSpacing=0;
			...
			if(xmlBar.HasAttribute("layout"))
				m_ItemContainer.LayoutType=(HVTTLayoutType)System.Xml.XmlConvert.ToInt32(xmlBar.GetAttribute("layout"));
...
//				this.AutoSyncBarCaption=false;
            ...
            if (xmlBar.HasAttribute("style"))
            {
                ...
                if (sty == "5")
                    ...
                    this.Style = (HVTTControlStyle)System.Xml.XmlConvert.ToInt32(sty);
            }
...
//				m_SaveLayoutChanges=true;
		...
		/// <summary>
		/// Gets or sets whether layout changes are saved for this bar when HVTTManager.SaveLayout method is used to save layout for all bars. Default value is true which means that layout changes are saved.
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(true),Category("Behavior"),Description("Indicates whether layout changes are saved for this bar")]
		...
		internal void SerializeLayout(System.Xml.XmlElement xmlThisBar)
		{
			...
			foreach(BaseItem item in m_ItemContainer.SubItems)
			{
				if(item.OriginalBarName!="" || item.OriginalPosition>=0 || item.UserCustomized || this.CustomBar || m_TabsRearranged || this.LayoutType == HVTTLayoutType.DockContainer)
				{
					System.Xml.XmlElement xmlItem=xmlThisBar.OwnerDocument.CreateElement("item");
		...
		internal void DeserializeLayout(System.Xml.XmlElement xmlBar)
		{
			...
			try
			{
				...
				if(xmlBar.ChildNodes.Count>0 && xmlBar.ChildNodes[0].Name=="items")
				{
		...
		private void LoadLayoutItems(System.Xml.XmlElement xmlBar)
		{
			...
			HVTTLayoutType layoutType=HVTTLayoutType.Toolbar;
			HVTTManager manager=null;
			if(m_Owner!=null && m_Owner is HVTTManager)
				manager=m_Owner as HVTTManager;

			if(xmlBar.HasAttribute("layout"))
				layoutType=(HVTTLayoutType)System.Xml.XmlConvert.ToInt32(xmlBar.GetAttribute("layout"));
			
			if(layoutType==HVTTLayoutType.DockContainer)
						...
						for(int i=0;i<xmlElem.ChildNodes.Count;i++)
						{
							...
							if(layoutType==HVTTLayoutType.DockContainer)
							{
								...
								else if(manager!=null)
								{
									...
									{
										HVTTMarkStatus parentBar=item.ContainerControl as HVTTMarkStatus;
							...
							else
							{
								...
								else
								{
									...
									if(item!=null)
									{
										if(item.ContainerControl==null || originalBar=="")
											item=item.Copy();
										else
											item.Parent.SubItems.Remove(item);
                                        if (this.Items.Count > actualPos)
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Appearance"),DefaultValue(BackgroundImagePosition.Stretch),Description("Specifies background image position when container is larger than image.")]
		...
		/// </summary>
		[Browsable(true),DefaultValue((byte)255),HVTTBrowsable(true),Category("Appearance"),Description("Specifies the transparency of background image.")]
		...
		public override string Text
		{
			...
			set
			{
				...
				{
					base.Text=value;
                    if (this.Visible && (m_BarState == HVTTBarState.Floating ||
                        (m_BarState == HVTTBarState.Docked || m_BarState == HVTTBarState.AutoHide) && 
                        (m_GrabHandleStyle == HVTTGrabHandleStyle.Caption || m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionTaskPane || 
                        m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionDotted)))
					{
						this.Refresh();
					}
				}
		...
		protected internal bool IsThemed
		{
			get
			{
				if(m_ThemeAware && this.Style!=HVTTControlStyle.Office2000  && BarFunctions.ThemedOS && Themes.ThemesActive)
					return true;
				return false;
			}
		}
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.DefaultValue(false),System.ComponentModel.Category("Appearance"),System.ComponentModel.Description("Specifies whether SideBar is drawn using Themes when running on OS that supports themes like Windows XP.")]
		...
		internal void SetBarState(HVTTBarState state)
		{
			m_BarState=state;
			if(m_BarState!=HVTTBarState.Popup && m_DropShadow!=null)
			{
				m_DropShadow.Hide();
				m_DropShadow.Dispose();
				m_DropShadow=null;
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(false),DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Behavior"),System.ComponentModel.Description("Indicates whether the items that could not be displayed on the non-wrap Bar are displayed on popup menu or popup Bar."),DefaultValue(false)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Layout"),System.ComponentModel.Description("Indicates the spacing in pixels between the sub-items."),DefaultValue(0)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Layout"),System.ComponentModel.Description("Gets/Sets the padding in pixels. This represents the spacing between the top of the bar and the top of the item."),DefaultValue(1)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Layout"),System.ComponentModel.Description("Gets/Sets the padding in pixels. This represents the spacing between the bottom edge of the bar and the bottom of the item."),DefaultValue(1)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Layout"),System.ComponentModel.Description("Gets/Sets the padding in pixels. This represents the spacing between the left edge of the bar and the left position of the first item."),DefaultValue(1)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Layout"),System.ComponentModel.Description("Gets/Sets the padding in pixels. This represents the spacing between the right edge of the bar and the right side of the last item."),DefaultValue(1)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Behavior"),System.ComponentModel.Description("Indicates Bar layout type."),DefaultValue(false)]
		public bool MenuBar
		{
			get
			{
        ...
        /// </summary>
        [Browsable(true), HVTTBrowsable(true), DefaultValue(eBarType.Toolbar), Category("Appearance"), Description("Indicates visual type of the bar. The type specified here is used to determine the appearance of the bar.")]
		...
		/// </summary>
		[System.ComponentModel.Editor(typeof(ColorSchemeVSEditor), typeof(System.Drawing.Design.UITypeEditor)),System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Appearance"),System.ComponentModel.Description("Gets or sets Bar Color Scheme."),DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
		public HVTT.UI.Window.Forms.ColorScheme ColorScheme
		{
		...
		internal ColorScheme GetColorScheme()
		{
			...
			if(m_Owner is HVTTManager && ((HVTTManager)m_Owner).UseGlobalColorScheme)
				return ((HVTTManager)m_Owner).ColorScheme;
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(true),System.ComponentModel.Category("Appearance"),System.ComponentModel.Description("Indicates the background color of the caption (Title bar)."),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(true),System.ComponentModel.Category("Appearance"),System.ComponentModel.Description("Indicates the caption (Title bar) text color."),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		...
		internal void ClearMDIChildSystemItems(bool bRecalcLayout)
		{
			...
			try
			{
				if(m_ItemContainer.SubItems.Contains("HVTTCONTROLSsysiconitem"))
					m_ItemContainer.SubItems.Remove("HVTTCONTROLSsysiconitem");
				if(m_ItemContainer.SubItems.Contains("HVTTCONTROLSsysmenuitem"))
					m_ItemContainer.SubItems.Remove("HVTTCONTROLSsysmenuitem");
				if(bRecalcLayout)
					this.RecalcLayout();
			}
			catch(Exception)
		...
		internal void ShowMDIChildSystemItems(System.Windows.Forms.Form objMdiChild, bool bRecalcLayout)
		{
			...
			MDISystemItem mdi=new MDISystemItem("HVTTCONTROLSsysiconitem");
			...
			mdi=new MDISystemItem("HVTTCONTROLSsysmenuitem");
			if(!objMdiChild.ControlBox)
				mdi.CloseEnabled=false;
			if(!objMdiChild.MinimizeBox)
				mdi.MinimizeEnabled=false;
        ...
        /// </summary>
        [Browsable(true), HVTTBrowsable(false), Category("Appearance"), Description("Indicates whether toolbars with appropriate style appear with rounded corners."), DefaultValue(true)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Appearance"),System.ComponentModel.Description("Specifies the visual style of the Bar."),DefaultValue(HVTTControlStyle.OfficeXP)]
		public HVTTControlStyle Style
		{
			...
			set
			{
                ...
                if ((value == HVTTControlStyle.Office2003 || value == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(value)) && this.LayoutType == HVTTLayoutType.Toolbar && this.GrabHandleStyle != HVTTGrabHandleStyle.None && this.GrabHandleStyle != HVTTGrabHandleStyle.ResizeHandle)
					m_GrabHandleStyle=HVTTGrabHandleStyle.Office2003;
        ...
        //        }
        //        else if(m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && m_ItemContainer.SubItems.Count==1 && m_ItemContainer.SubItems[0] is DockContainerItem)
		...
		internal void HideBar()
		{
			...
			else
			{
				...
				if(m_Owner!=null && m_Owner is HVTTManager && ((HVTTManager)m_Owner).IsDisposed)
					return;
				this.RecalcLayout();
			}
		}
		...
		internal void ShowBar()
		{
			...
			else if(m_BarState==HVTTBarState.Popup)
			{
                // Design mode add
				...
				HVTTPopupAnimation animation=m_PopupAnimation;
				if(!BarFunctions.SupportsAnimation)
					animation=HVTTPopupAnimation.None;
				else
				{
					IOwnerMenuSupport ownersupport=m_Owner as IOwnerMenuSupport;
					if(animation==HVTTPopupAnimation.ManagerControlled)
					{
						if(ownersupport!=null)
							animation=ownersupport.PopupAnimation;
						if(animation==HVTTPopupAnimation.ManagerControlled)
							animation=HVTTPopupAnimation.SystemDefault;
					}

					if(animation==HVTTPopupAnimation.SystemDefault)
						animation=NativeFunctions.SystemMenuAnimation;
					else if(animation==HVTTPopupAnimation.Random)
					{
						Random r=new System.Random();
						int i=r.Next(2);
						animation=HVTTPopupAnimation.Fade;
						if(i==1)
							animation=HVTTPopupAnimation.Slide;
						else if(i==2)
							animation=HVTTPopupAnimation.Unfold;
					}
				}
				...
				if(animation==HVTTPopupAnimation.Fade && Environment.OSVersion.Version.Major>=5)
				{
				...
				else if(animation==HVTTPopupAnimation.Slide)
				...
				else if(animation==HVTTPopupAnimation.Unfold)
				...
				if(animation!=HVTTPopupAnimation.None && this.Controls.Count>0)
					this.Refresh();
                ...
                if (!m_DockingInProgress && !m_LayoutSuspended && this.Parent is DockSite && this.Parent.Dock !=DockStyle.Fill && ((DockSite)this.Parent).DocumentDockContainer != null)
                {
                    ...
                    if (!m_BarDefinitionLoading)
                    {
                        ...
                        if (dc != null && dc.Parent is DocumentDockContainer)
                        {
                            ...
                            if (p.Documents.Count > 1)
                            {
                                if (p.Orientation == HVTTOrientation.Horizontal && (ds.Dock == DockStyle.Top || ds.Dock == DockStyle.Bottom))
                                {
                                ...
                                else if (p.Orientation == HVTTOrientation.Vertical && (ds.Dock == DockStyle.Left || ds.Dock == DockStyle.Right))
                                {
                    ...
                    HVTTManager man = this.Owner as HVTTManager;
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Layout"),System.ComponentModel.Description("Indicates whether the items will be wrapped into next line when Bar is full. Applies only to Bars that are docked."),DefaultValue(false)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(true),System.ComponentModel.Category("Layout"),System.ComponentModel.Description("Indicates whether the items will be wrapped into next line when Bar is full. Applies only to Bars that are floating."),DefaultValue(true)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Appearance"),System.ComponentModel.Description("Indicates the grab handle style of the docked Bars."),DefaultValue(HVTTGrabHandleStyle.None)]
		public HVTTGrabHandleStyle GrabHandleStyle
		{
			get
			{
		...
		/// </summary>
		[Browsable(false),HVTTBrowsable(false)]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Behavior"),System.ComponentModel.Description("Determines whether bar can be hidden."),DefaultValue(false)]
		public bool CanHide
		{
			get
			{
			...
			set
			{
				if(m_CanHide==value)
					return;
				m_CanHide=value;
                if (m_BarState == HVTTBarState.Floating || m_BarState == HVTTBarState.Docked && (m_GrabHandleStyle == HVTTGrabHandleStyle.Caption || m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionTaskPane || m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionDotted))
					this.Refresh();
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Appearance"),System.ComponentModel.Description("Indicates border style when Bar is docked."),DefaultValue(eBorderType.None)]
		public eBorderType DockedBorderStyle
		{
			...
			set
			{
				...
				if(m_BarState==HVTTBarState.Docked && this.Visible)
					this.RecalcLayout();
			}
		}
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(true),System.ComponentModel.DefaultValue(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Indicates whether floating bar is hidden when application loses focus.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(false),Category("Docking"),Description("Indicates whether tab navigation buttons are shown for tabbed dockable bars.")]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Appearance"),System.ComponentModel.Description("Indicates the border line color when docked border is a single line.")]
		public Color SingleLineColor
		{
			...
			set
			{
				if(m_SingleLineColor!=value)
				{
					...
					if(m_BarState==HVTTBarState.Docked && this.Visible && m_DockedBorder==eBorderType.SingleLine)
						this.Refresh();
				}
			}
		}
		...
		/// </summary>
		[HVTTBrowsable(true), DefaultValue(true)]
		public new bool Visible
		{
			get
			{
				if(m_BarState==HVTTBarState.Floating && m_Float!=null)
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(true),System.ComponentModel.Category("Behavior"),System.ComponentModel.Description("Determines whether end-user can drag and drop items on the Bar."),DefaultValue(true)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(true),System.ComponentModel.Category("Design"),System.ComponentModel.Description("Bar name that can be used to identify Bar from code.")]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Behavior"),System.ComponentModel.Description("Specifies the Image size that will be used by items on this bar."),DefaultValue(BarImageSize.Default)]
		public BarImageSize ImageSize
		{
			get
			{
				return m_ImageSize;
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Layout"),System.ComponentModel.Description("Indicates Bar layout type."),DefaultValue(HVTTLayoutType.Toolbar)]
		public HVTTLayoutType LayoutType
		{
			get
			{
                if (m_ItemContainer == null)
                    return HVTTLayoutType.Toolbar;
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Behavior"),System.ComponentModel.Description("Indicates that buttons will be automatically resized to match the size of the largest button on the bar."),DefaultValue(false)]
        ...
        internal bool IsFadeEnabled
        {
            get
            {
                if (this.DesignMode || (m_ItemContainer.Style!=HVTTControlStyle.Office2007) || m_FadeEffect && NativeFunctions.IsTerminalSession() || this.IsThemed)
                    return false;
                return m_FadeEffect;
            }
        }
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Appearance"),System.ComponentModel.Description("Specifies background color of the Bar."),DesignerSerializationVisibility(DesignerSerializationVisibility.Visible)]
		...
		{
			if(this.LayoutType==HVTTLayoutType.DockContainer && this.Parent is DockSite && this.Parent.Dock==DockStyle.Fill)
				return false;
			return true;
		}
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(false),System.ComponentModel.Category("Behavior"),System.ComponentModel.Description("Indicates the auto-hide side of the parent form where bar is positioned.")]
		...
		/// </summary>
        [Browsable(true), DefaultValue(false), HVTTBrowsable(true), System.ComponentModel.Category("Auto-Hide"), System.ComponentModel.Description("Indicates whether Bar is in auto-hide state. Applies to non-document dockable bars only."), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		public virtual bool AutoHide
		{
			...
			set
			{
				...
				if(!m_CanAutoHide || this.LayoutType!=HVTTLayoutType.DockContainer || this.Parent!=null && this.Parent.Dock == DockStyle.Fill && value)
					return;
        ...
        /// HVTTCONTROLS and should not be set directly.
        /// </summary>
		...
		public BaseItem GetItem(string name)
		{
            ...
            if (m_GlobalParentComponent != null)
            {
                ...
                else if (m_GlobalParentComponent is HVTTManager)
                    return ((HVTTManager)m_GlobalParentComponent).GetItem(name);
            }
		...
		public ArrayList GetItems(string ItemName)
		{
			...
			if (m_GlobalParentComponent != null)
            {
                if (m_GlobalParentComponent is RibbonControl)
                    ...
                    list.AddRange(((HVTTManager)m_GlobalParentComponent).GetItems(ItemName));
            }
            else
            {
		...
		public ArrayList GetItems(string ItemName, Type itemType)
		{
            ...
            if (m_GlobalParentComponent != null)
            {
                if (m_GlobalParentComponent is RibbonControl)
                    ...
                    list.AddRange(((HVTTManager)m_GlobalParentComponent).GetItems(ItemName, itemType));
            }
            else
            {
        ...
        public ArrayList GetItems(string ItemName, Type itemType, bool useGlobalName)
        {
            ...
            if (m_GlobalParentComponent != null)
            {
                if (m_GlobalParentComponent is RibbonControl)
                    ...
                    list.AddRange(((HVTTManager)m_GlobalParentComponent).GetItems(ItemName, itemType, false, useGlobalName));
            }
            else
            {
		...
		private Control GetOwnerControl()
		{
			Control parentControl=null;
			IOwner owner=m_Owner as IOwner;
			if(owner is HVTTManager && ((HVTTManager)owner).TopDockSite!=null)
				parentControl=((HVTTManager)owner).TopDockSite.Parent;
			else if(owner!=null)
				parentControl=owner.ParentForm;
		...
		private void AutoHideStateChanged()
		{
			...
			if(m_AutoHideState)
			{
				...
				m_LastDockSiteInfo.DockLine=this.DockLine;
				m_LastDockSiteInfo.DockOffset=this.DockOffset;
				if(this.Parent!=null && m_BarState==HVTTBarState.Docked)
					m_LastDockSiteInfo.DockSide=this.Parent.Dock;
				else
					m_LastDockSiteInfo.DockSide=DockStyle.Left;
				if(this.Parent!=null && this.Parent is DockSite)
				...
				// Undock the window
				m_BarState=HVTTBarState.AutoHide;
				ResetActiveControl();
                        ...
                        if (ds.DocumentDockContainer != null)
                        {
                            ...
                            if (ds.Dock != DockStyle.Fill)
                            {
                                ...
                                if (dbr != null && dbr.Parent is DocumentDockContainer)
                                {
                                    ...
                                    if (ddc.Orientation == HVTTOrientation.Horizontal && (ds.Dock == DockStyle.Top || ds.Dock == DockStyle.Bottom) ||
                                        ddc.Orientation == HVTTOrientation.Vertical && (ds.Dock == DockStyle.Left || ds.Dock == DockStyle.Right) && ddc.Documents.Count>1)
                                    {
                                        for (int i = 0; i < ddc.Documents.Count; i++)
                                        {
                                            if (ddc.Documents[i] is DocumentBarContainer && ddc.Documents[i].Visible && ddc.Documents[i] != dbr)
                                            {
                                                ...
                                                if (i < ddc.Documents.IndexOf(dbr))
                                                    m_LastDockSiteInfo.MouseOverDockSide = (ddc.Orientation == HVTTOrientation.Horizontal?eDockSide.Left:eDockSide.Top);
                                                else
                                                    m_LastDockSiteInfo.MouseOverDockSide = (ddc.Orientation == HVTTOrientation.Horizontal ? eDockSide.Right : eDockSide.Bottom);
                                                break;
                                            }
                                        }
                                    }
...
//					}
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(100),Category("Auto-Hide"),Description("Specifies how long it takes to play the auto-hide animation, in milliseconds. Maximum value is 2000, 0 disables animation.")]
		...
		private bool ShowCustomizeMenuButton
		{
			get
			{
                ...
                if (m_ItemContainer.LayoutType == HVTTLayoutType.DockContainer && (m_GrabHandleStyle == HVTTGrabHandleStyle.Caption || m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionTaskPane || m_GrabHandleStyle == HVTTGrabHandleStyle.CaptionDotted) && m_CustomizeMenu != null)
					return true;
				if(m_ItemContainer.LayoutType!=HVTTLayoutType.DockContainer)
				{
		...
		private void UpdateDockTabSettings()
		{
			...
			if(m_TabNavigation)
				m_TabDockItems.TabLayoutType=TabLayoutType.FixedWithNavigationBox;
			else
				m_TabDockItems.TabLayoutType=TabLayoutType.FitContainer;

			m_TabDockItems.ShowFocusRectangle=false;

			if(m_TabDockItems._TabSystemBox!=null)
				m_TabDockItems._TabSystemBox.CloseVisible=this.CanHide;
        ...
        private void TabStripTabItemClose(object sender, TabStripActionEventArgs e)
        {
            ...
            if (ti != null)
            {
                ...
                if (di != null)
                    CloseDockTab(di, EventSource.Mouse);
            }
        }

		internal void SetDockTabStyle(HVTTControlStyle style)
		{
			if(m_TabDockItems==null)
				return;
			if(this.Style==HVTTControlStyle.Office2003)
			{
            ...
            else if (this.Style == HVTTControlStyle.VS2005)
			{
				IOwnerBarSupport ob=m_Owner as IOwnerBarSupport;
				if(this.DockSide==eDockSide.Document && ob!=null && ob.ApplyDocumentBarStyle)
        ...
        internal void RefreshDockTab(bool bFireEvents)
        {
            ...
            if (iVisibleItems > 1 || m_AlwaysDisplayDockTab && m_BarState != HVTTBarState.Floating)
            {
                if (m_TabDockItems == null)
                {
                    CreateDockTab();
		...
		internal void OnSubItemRemoved(BaseItem objItem)
		{
			...
			{
				if(this.LayoutType==HVTTLayoutType.DockContainer)
				{
		...
		{
			if(m_GrabHandleStyle==HVTTGrabHandleStyle.CaptionTaskPane && m_AutoCreateCaptionMenu)
				return false;
			return true;
		}
		...
		private void RefreshDockTabItems()
		{
			if(m_RefreshingDockTab)
				return;
			if(!m_TabDockItems.Visible && m_BarState!=HVTTBarState.AutoHide)
			{
				m_TabDockItems.Visible=GetDockTabVisible();
				if(this.IsThemed && m_BarState==HVTTBarState.Floating)
				{
			...
			try
			{
				...
				if(iCount>1 || iCount==1 && m_AlwaysDisplayDockTab && m_BarState!=HVTTBarState.Floating)
				{
					if(!m_TabDockItems.Visible && m_BarState!=HVTTBarState.AutoHide)
						m_TabDockItems.Visible=GetDockTabVisible();
                    if (m_TabDockItems.SelectedTab == null)
		...
		private void ResizeDockTab()
		{
			if(m_TabDockItems!=null && m_TabDockItems.Visible)
			{
				...
				switch(m_DockTabAlignment)
				{
					case eTabStripAlignment.Top:
					{
						if(m_GrabHandleStyle!=HVTTGrabHandleStyle.None && !m_GrabHandleRect.IsEmpty)
						{
					...
					case eTabStripAlignment.Right:
					{
						if(m_GrabHandleStyle!=HVTTGrabHandleStyle.None && !m_GrabHandleRect.IsEmpty && m_BarState==HVTTBarState.Docked)
						{
					...
					default:
					{
						...
						if(this.DockSide==eDockSide.Top && m_BarState==HVTTBarState.Docked)
						{
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(false)]
		...
		{
			if(m_AutoSyncBarCaption && this.LayoutType==HVTTLayoutType.DockContainer)
			{
		...
		public DockContainerItem SelectedDockContainerItem
		{
			get
			{
				if(m_ItemContainer.LayoutType!=HVTTLayoutType.DockContainer) return null;
				...
				if(this.VisibleItemCount>0)
				{
					foreach(BaseItem item in this.Items)
					{
						if(item.Visible) return item as DockContainerItem;
					}
				}
				return null;
			}
			...
			{
				if(m_ItemContainer.LayoutType!=HVTTLayoutType.DockContainer)
					throw new InvalidOperationException("Bar type is not dockable window. LayoutType must be set to HVTTLayoutType.DockContainer");
		...
		/// <summary>
		/// Gets or sets the tab (DockContainerItem) index for Bars with LayoutType set to HVTTLayoutType.DockContainer. Index corresponds to the index of the DockContainerItem in Bar.Items collection.
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Gets or sets the tab (DockContainerItem) index for Bars with LayoutType set to HVTTLayoutType.DockContainer."),DefaultValue(-1)]
		public int SelectedDockTab
		{
			get
			{
				if(m_ItemContainer.LayoutType!=HVTTLayoutType.DockContainer || m_TabDockItems==null)
					return -1;
				if(m_TabDockItems.SelectedTab!=null)
			...
			set
			{
				...
				if(m_ItemContainer.LayoutType!=HVTTLayoutType.DockContainer)
					throw new InvalidOperationException("SelectedDockTab property can be set only for LayoutType=HVTTLayoutType.DockContainer.");
                if (value < 0) value = 0;
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),Category("Appearance"),Description("Indicates Bar background image.")]
		...
		[Browsable(true),HVTTBrowsable(false),System.ComponentModel.EditorBrowsable(System.ComponentModel.EditorBrowsableState.Never)]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(true),Category("Behavior"),Description("Indicates whether caption button drop-down menu is automatically created")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(false),Category("Behavior"),Description("Indicates whether caption is automatically set to the active dock container item text.")]
		...
		/// <returns></returns>
		public System.Drawing.Size MinimumDockSize(HVTTOrientation dockOrientation)
		{
			if(m_ItemContainer==null) return new Size(32,32);
			if(m_ItemContainer.LayoutType==HVTTLayoutType.DockContainer && (m_ItemContainer.Stretch || m_DockStretch))
			{
		...
		internal int GetBarDockedSize(HVTTOrientation o)
		{
			const int TOTAL_BORDER_SIZE=6;
			if(m_ItemContainer==null)
				return 0;
			if(o==HVTTOrientation.Horizontal && m_LastDockSiteInfo.DockedHeight>0 && (m_LastDockSiteInfo.DockSide==DockStyle.Top || m_LastDockSiteInfo.DockSide==DockStyle.Bottom))
				return m_LastDockSiteInfo.DockedHeight;
			else if(o==HVTTOrientation.Vertical && m_LastDockSiteInfo.DockedWidth>0 && (m_LastDockSiteInfo.DockSide==DockStyle.Left || m_LastDockSiteInfo.DockSide==DockStyle.Right))
				return m_LastDockSiteInfo.DockedWidth;

			if(o==HVTTOrientation.Horizontal && m_ItemContainer.MinHeight>0 && m_ItemContainer.MinHeight+SystemInformation.ToolWindowCaptionHeight<this.GetFormClientHeight())
				return m_ItemContainer.MinHeight+SystemInformation.ToolWindowCaptionHeight;
			else if(o==HVTTOrientation.Vertical && m_ItemContainer.MinWidth>0 && m_ItemContainer.MinWidth+SystemInformation.ToolWindowCaptionHeight<this.GetFormClientWidth())
				return m_ItemContainer.MinWidth+SystemInformation.ToolWindowCaptionHeight;
			
			if(this.SelectedDockTab>=0 || this.VisibleItemCount>0 && this.LayoutType == HVTTLayoutType.DockContainer)
			{
                DockContainerItem item = null;
                if (this.SelectedDockTab >= 0)
				...
				if(item!=null)
				{
					if(o==HVTTOrientation.Horizontal)
					{
                        if (item.MinimumSize.Height > 0)
                        {
                            if (this.Height > item.MinimumSize.Height)
                                return this.Height;
					...
					else
					{
                        ...
                        {
                            //if(item.MinimumSize.Width>32)
                            //    return item.MinimumSize.Width + TOTAL_BORDER_SIZE;
                            //else
                            if (this.Width > item.MinimumSize.Width)
                                return this.Width;
		...
		/// <returns></returns>
		public System.Drawing.Size PreferredDockSize(HVTTOrientation dockOrientation)
		{
			// Return preffered size for this container
			IOwner owner=m_Owner as IOwner;
			if(dockOrientation==HVTTOrientation.Horizontal)
			{
				if(m_DockedSizeH.IsEmpty)
				{
					if(owner!=null && owner.ParentForm!=null)
						m_DockedSizeH=RecalcSizeOnly(owner.ParentForm.Size,HVTTOrientation.Horizontal,HVTTBarState.Docked,m_WrapItemsDock);
					else
						m_DockedSizeH=RecalcSizeOnly(System.Windows.Forms.Screen.FromControl(this).WorkingArea.Size,HVTTOrientation.Horizontal,HVTTBarState.Docked,m_WrapItemsDock);
				}
			...
			else
			{
				...
				if(m_DockedSizeV.IsEmpty)
				{
					...
					if(owner!=null && owner.ParentForm!=null)
						size=RecalcSizeOnly(owner.ParentForm.Size,HVTTOrientation.Horizontal,HVTTBarState.Docked,m_WrapItemsDock);
					else
						size=RecalcSizeOnly(System.Windows.Forms.Screen.FromControl(this).WorkingArea.Size,HVTTOrientation.Horizontal,HVTTBarState.Docked,m_WrapItemsDock);
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Specifies whether Bar can be undocked."),DefaultValue(true)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Specifes whether end-user can tear-off (deattach) the tabs on dockable window."),DefaultValue(true)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Specifes whether end-user can reorder the tabs on dockable window."),DefaultValue(true)]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(true),Category("Docking"),Description("Indicates whether bar or DockContainerItem that is torn-off this bar can be docked as tab to another bar.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Determines whether bar can be docked to the top edge of container.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(true),Category("Docking"),Description("Determines whether bar can be docked to the bottom edge of container.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Determines whether bar can be docked to the left edge of container.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),DefaultValue(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Determines whether bar can be docked to the right edge of container.")]
		...
		/// <summary>
		/// Specifes whether Bar can be docked as document. Default value is false. See HVTTManager.EnableDocumentDocking for more details.
		/// </summary>
		[Browsable(false),DefaultValue(false),HVTTBrowsable(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Specifes whether Bar can be docked as document.")]
		...
		//		/// </summary>
		//		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),System.ComponentModel.DefaultValue(false),System.ComponentModel.Category("Layout"),System.ComponentModel.Description("Specifies whether Bar is merged with MDI Parent Bars when on MDI Child form.")]
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Indicates the distance from the far left/top side of the docking site.."),DefaultValue(0)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Indicates the docking line."),DefaultValue(0)]
		public int DockLine
		{
			get
			{
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Gets or sets the dock tab alignment."),System.ComponentModel.DefaultValue(eTabStripAlignment.Bottom)]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Indicates whether tab that shows all dock containers on the bar is visible all the time."),System.ComponentModel.DefaultValue(false)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Indicates whether bar is locked to prevent docking below it. Applies to undockable bars only."),DefaultValue(false)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),DefaultValue(HVTTOrientation.Horizontal),HVTTBrowsable(false)]
		public HVTTOrientation DockOrientation
		{
			get
			{
		...
		public bool Docked
		{
			get
			{
				return (m_BarState==HVTTBarState.Docked);
			}
		}
		...
		/// </summary>
		[System.ComponentModel.Browsable(false),HVTTBrowsable(true),System.ComponentModel.Category("Docking"),System.ComponentModel.Description("Indicates the dock side for the Bar.")]
		public eDockSide DockSide
		{
			...
			set
			{
				...
				{
                    DockSite ds = null;
                    if (this.LayoutType == HVTTLayoutType.Toolbar)
                    {
                        ds = ownersupport.ToolbarLeftDockSite;
                        if (ds == null)
                            throw (new System.InvalidOperationException("HVTTManager.ToolbarLeftDockSite dock-site is not set."));
                    }
                    else
                    {
                        ds = ownersupport.LeftDockSite;
                        if (ds == null)
                            throw (new System.InvalidOperationException("HVTTManager.LeftDockSite dock-site is not set."));
                    }
				...
				else if(value==eDockSide.Right)
				{
                    ...
                    if (this.LayoutType == HVTTLayoutType.Toolbar)
                    {
                        ...
                        if (ds == null)
                            throw (new System.InvalidOperationException("HVTTManager.ToolbarRightDockSite dock-site is not set."));
                    }
                    else
                    {
                        ds = ownersupport.RightDockSite;
                        if (ds == null)
                            throw (new System.InvalidOperationException("HVTTManager.RightDockSite dock-site is not set."));
                    }
				...
				else if(value==eDockSide.Top)
				{
                    ...
                    if (this.LayoutType == HVTTLayoutType.Toolbar)
                    {
                        ...
                        if (ds == null)
                            throw (new System.InvalidOperationException("HVTTManager.ToolbarTopDockSite dock-site is not set."));
                    }
                    else
                    {
                        ds = ownersupport.TopDockSite;
                        if (ds == null)
                            throw (new System.InvalidOperationException("HVTTManager.TopDockSite dock-site is not set."));
                    }
				...
				else if(value==eDockSide.Bottom)
				{
                    ...
                    if (this.LayoutType == HVTTLayoutType.Toolbar)
                    {
                        ...
                        if (ds == null)
                            throw (new System.InvalidOperationException("HVTTManager.ToolbarBottomDockSite dock-site is not set."));
                    }
                    else
                    {
                        ds = ownersupport.BottomDockSite;
                        if (ds == null)
                            throw (new System.InvalidOperationException("HVTTManager.BottomDockSite dock-site is not set."));
                    }
		...
		/// </summary>
		[Browsable(false),HVTTBrowsable(false),Description("Indicates the inital floating location."),DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		...
		/// </summary>
		[System.ComponentModel.Browsable(true),HVTTBrowsable(true),DefaultValue(true),System.ComponentModel.Category("Run-time Behavior"),System.ComponentModel.Description("Indicates whether Tooltips are shown on Bar and it's sub-items.")]
        ...
        /// </summary>
        internal bool DesignerSelection
        {
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(false),DefaultValue(false),Category("Run-time Behavior"),Description("Indicates whether accelerator letters for menu or toolbar commands are underlined regardless of current Windows settings.")]
		...
		/// </summary>
		[Browsable(true),HVTTBrowsable(false),DefaultValue(null),Category("Data"),Description("ImageList for images used on Items. Images specified here will always be used on menu-items and are by default used on all Bars.")]
		...
		bool IMessageHandlerClient.OnKeyDown(IntPtr hWnd, IntPtr wParam, IntPtr lParam)
		{
			...
			if(m_RegisteredPopups.Count>0)
			{
				...
				if(popup.Parent==null || IsContextPopup(popup))
				{
...
//			return false;
		...
		{
			if(this.FindForm()==null || !this.FindForm().Focused || this.DesignMode)
				return false;
			...
			if(wParam.ToInt32()==18 || wParam.ToInt32()==121)
			{
				...
				{
					HVTT.UI.Window.Forms.HVTTMarkStatus bar=null;
					if(this.MenuBar)
						bar=this;
		...
		{
			HVTTMarkStatus m_Owner = null;
			public BarAccessibleObject(HVTTMarkStatus owner):base(owner)
			{
				m_Owner = owner;
			}
			...
			public override AccessibleStates State
			{
				get
				{
					...
					if(m_Owner.GrabHandleStyle!=HVTTGrabHandleStyle.None && m_Owner.GrabHandleStyle!=HVTTGrabHandleStyle.ResizeHandle)
					{
		...
		/// </summary>
		System.Drawing.Size IDockInfo.MinimumDockSize(HVTTOrientation dockOrientation)
		{
		...
		/// </summary>
		System.Drawing.Size IDockInfo.PreferredDockSize(HVTTOrientation dockOrientation)
		{
		...
		/// </summary>
		HVTTOrientation IDockInfo.DockOrientation
		{
        ...
        private void DesignTimeMouseUp(MouseEventArgs e)
        {
            ...
            if (m_DesignTimeProvider != null)
            {
                ...
                if (objParent != null)
                {
                    ...
                    if (ctrl is HVTTMarkStatus)
                        ((HVTTMarkStatus)ctrl).RecalcLayout();
            ...
            if (owner != null)
            {
                ...
                if (site != null)
                {
                    ...
                    if (e.Button == MouseButtons.Right)
                    {
                        ...
                        if (service1 != null)
                        {
                            service1.ShowContextMenu(System.Windows.Forms.Design.MenuCommands.SelectionMenu, Control.MousePosition.X, Control.MousePosition.Y);
                        }
                    }
                }
            }
	...
	internal class FloatingContainer:Form
	{
		...
		const int MA_NOACTIVATE = 3;
		//const int MA_NOACTIVATEANDEAT = 4;
		HVTTMarkStatus m_Bar;
		private Size m_OldSize=Size.Empty;
		public FloatingContainer(HVTTMarkStatus objBar) {
			m_Bar = objBar;
		...
		protected override void WndProc(ref Message m)
		{
			if(m.Msg==WM_MOUSEACTIVATE && m_Bar.LayoutType!=HVTTLayoutType.DockContainer)
			{
				m.Result=new System.IntPtr(MA_NOACTIVATE);
				return;
			}
		...
		{
			base.OnActivated(e);
			if(m_Bar!=null && m_Bar.LayoutType==HVTTLayoutType.DockContainer)
			{
    ...
    public class RenderBarEventArgs : EventArgs
    {
        ...
        /// </summary>
        public readonly HVTTMarkStatus Bar;
        ...
        /// </summary>
        public RenderBarEventArgs(HVTTMarkStatus bar, Graphics g, eBarRenderPart part, Rectangle bounds)
        {
            this.Bar = bar;
            this.Part = part;
            this.Bounds = bounds;
