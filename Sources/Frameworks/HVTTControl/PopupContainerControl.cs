	...
	public class PopupContainerControl:UserControl
	{
        ...
        private bool IsGradientStyle
        {
            get
            {
                if (m_Parent != null)
                {
                    return m_Parent.Style == HVTTControlStyle.OfficeXP || m_Parent.Style == HVTTControlStyle.Office2003 || m_Parent.Style == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(m_Parent.Style);
                }
                return true;
            }
        }
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			if(IsGradientStyle)
			{
				if(this.DisplayShadow && !this.AlphaShadow)
					SetupRegion();
				Pen p=null;
				if(m_Parent!=null && m_Parent.ContainerControl is HVTTMarkStatus)
					p=new Pen(((HVTTMarkStatus)m_Parent.ContainerControl).ColorScheme.ItemHotBorder,1);
				...
				if(m_Parent is ButtonItem && m_Parent.Displayed && m_Parent.Style!=HVTTControlStyle.Office2007)
				{
		...
		private bool DisplayShadow
		{
			get
			{
				...
				{
					if(m_Parent!=null && m_Parent.Style==HVTTControlStyle.Office2000)
					{
