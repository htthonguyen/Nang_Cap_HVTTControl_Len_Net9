...
namespace HVTT.UI.Window.Forms.Design
{
	...
	public class BubbleBarDesigner:System.Windows.Forms.Design.ControlDesigner
	{
        ...
        private void SetDesignTimeDefaults()
        {
            ...
            bar.ButtonBackAreaStyle.SetColorScheme(new ColorScheme(HVTTControlStyle.Office2003));
            ...
            bar.ButtonBackAreaStyle.BorderTop = StyleBorderType.Solid;
            bar.ButtonBackAreaStyle.BorderTopWidth = 1;
            bar.ButtonBackAreaStyle.BorderBottom = StyleBorderType.Solid;
            bar.ButtonBackAreaStyle.BorderBottomWidth = 1;
            bar.ButtonBackAreaStyle.BorderLeft = StyleBorderType.Solid;
            bar.ButtonBackAreaStyle.BorderLeftWidth = 1;
            bar.ButtonBackAreaStyle.BorderRight = StyleBorderType.Solid;
...
//			}
		...
		private BubbleBarTab CreateTab()
		{
			...
			tab.Text=tab.Name;
			TabItemColor color=TabItemColor.Blue;
			if(bar.Tabs.Count>0)
			{
				int tt=bar.Tabs.Count+1;
				Type t=typeof(TabItemColor);
				FieldInfo[] fi=t.GetFields(BindingFlags.Public | BindingFlags.Static);
				int count=fi.Length;
                while(tt>count)
					tt-=count;
				if(tt==0) tt++;
				color=(TabItemColor)Enum.Parse(typeof(TabItemColor),fi[tt].Name);
			}
