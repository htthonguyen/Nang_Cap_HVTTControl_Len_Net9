	...
	public class BarBaseControlDesigner:System.Windows.Forms.Design.ParentControlDesigner,IDesignerServices
	{
		...
		protected virtual void DesignTimeSelectionChanged(ISelectionService ss)
		{
			...
			if(ss.PrimarySelection!=this.Control)
			{
				...
				if(ss.PrimarySelection is BaseItem)
				{
                    ...
                    {
                        if ((!item.IsOnMenu || item.IsOnBar && ((HVTTMarkStatus)item.ContainerControl).BarState == HVTTBarState.Popup) && !(item.Expanded && item is PopupItem || item is GalleryContainer && ((GalleryContainer)item).PopupGalleryItem.Expanded))
                    ...
                    {
                        if(!item.IsOnMenu && !(item.IsOnBar && ((HVTTMarkStatus)item.ContainerControl).BarState == HVTTBarState.Popup))
                            this.GetIOwnerMenuSupport().ClosePopups();
                        return;
                    }
		...
		protected override void WndProc(ref Message m)
		{
			...
			switch(m.Msg)
			{
				...
				{
					if(OnMouseDown(ref m, m.Msg==NativeFunctions.WM_LBUTTONDOWN?MouseButtons.Left:MouseButtons.Right))
						return;
					break;
				}
                ...
                case NativeFunctions.WM_HSCROLL:
                case NativeFunctions.WM_VSCROLL:
                {
                    base.WndProc(ref m);
                    this.RecalcLayout();
                    return;
		...
		protected virtual void RecalcLayout()
		{
            ...
            else if (c is HVTTButton)
                ((HVTTButton)c).RecalcLayout();
		}
		...
		protected virtual void SetComponentSelected(bool selected)
		{
            ...
            else if (this.GetItemContainerControl() is HVTTMarkStatus)
            {
                ((HVTTMarkStatus)this.GetItemContainerControl()).DesignerSelection = selected;
            }
        ...
        protected override bool GetHitTest(Point pt)
        {
            ...
            {
                int res = NativeFunctions.SendMessage(c.Handle, 0x84, 0, NativeFunctions.MAKELPARAM(pt.X, pt.Y));
                if ((res == 7) || (res == 6))
                {
                    return true;
                }
		...
		protected virtual void CreateDocument(object sender, EventArgs e)
		{
            ...
            try
            {
                ...
                OnSubItemsChanged();
                this.RecalcLayout();
                if (this.Control is HVTTMarkStatus)
                {
		...
		protected virtual HVTTControlStyle InternalStyle
		{
			get {return HVTTControlStyle.Office2003;}
		}
		#endregion
