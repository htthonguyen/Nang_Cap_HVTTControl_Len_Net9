...
namespace HVTT.UI.Window.Forms.Design
{
	...
	public class SideBarDesigner:BarBaseControlDesigner
	{
		...
		private SideBarPanelItem CreateNewPanel()
		{
			...
			if(bar!=null && dh!=null)
			{
                try
                {
                    ...
                    if (item.Appearance == SideBarAppearance.Flat)
                        SideBar.ApplyColorScheme(item, bar.PredefinedColorScheme);
                    if (bar.Style == HVTTControlStyle.Office2007)
                        item.FontBold = true;
                    if (change != null)
			...
			if(bar!=null && dh!=null)
			{
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        if (bar.Appearance == SideBarAppearance.Flat)
                            item.ImagePosition = ImagePosition.Left;
                        else
                            item.ImagePosition = ImagePosition.Top;
                        item.ButtonStyle = HVTTButtonStyle.ImageAndText;
		...
		private void SetColorScheme(SideBarColorScheme scheme)
		{
			SideBar sidebar=this.Control as SideBar;
			if(sidebar!=null)
			{
				if(sidebar.Appearance!=SideBarAppearance.Flat)
                    TypeDescriptor.GetProperties(sidebar)["Appearance"].SetValue(sidebar, SideBarAppearance.Flat);
		...
		public SideBarAppearance Appearance
		{
			...
			set 
			{
                ...
                if (bar.Appearance != value)
                {
                    ...
                    {
                        if (value == SideBarAppearance.Flat)
                        {
                            SetColorScheme(bar.PredefinedColorScheme);
                        }
                        else if (value == SideBarAppearance.Traditional)
                        {
                            bar.ResetTraditional();
                            RaiseStyleChanged();
                        }
                        if (dh != null && !dh.Loading && value == SideBarAppearance.Flat && bar.Style == HVTTControlStyle.Office2007)
                        {
