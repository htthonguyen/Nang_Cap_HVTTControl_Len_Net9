...
namespace HVTT.UI.Window.Forms.Design
{
	...
	public class BaseItemDesigner:System.ComponentModel.Design.ComponentDesigner,IDesignerServices
    {
        ...
        {
            if (item is LabelItem && BarFunctions.IsOffice2007Style(item.Style))
            {
		...
		protected virtual void RecalcLayout()
		{
            ...
            if (item.Expanded && item is PopupItem && ((PopupItem)item).PopupControl != null)
            {
                ...
                if (control is MenuPanel)
                    ((MenuPanel)control).RecalcSize();
                else if (control is HVTTMarkStatus)
                    ((HVTTMarkStatus)control).RecalcLayout();
            }
		}
		...
		/// </summary>
		[DefaultValue(true),Browsable(true),HVTTBrowsable(true),Category("Layout"),Description("Gets or sets whether item is visible.")]
		public bool Visible 
		{
			get 
			{
