...
namespace HVTT.UI.Window.Forms
{
	...
	public class DocumentDockUIManager
	{
		...
		internal Rectangle GetDockPreviewRectangle(HVTTMarkStatus bar, ref DockSiteInfo dockInfo)
		{
        ...
        private Rectangle GetEdgeDockPreviewRectangle(HVTTMarkStatus bar, ref DockSiteInfo dockInfo)
        {
            Rectangle r = Rectangle.Empty;
            int fullSizeIndex = -1;
		...
		public DocumentBaseContainer GetDocumentFromBar(HVTTMarkStatus bar)
		{
			if(bar==null)
				return null;
		...
		private DocumentBaseContainer GetDocumentFromBar(HVTTMarkStatus bar, DocumentDockContainer dockContainer)
		{
		...
		public void Dock(HVTTMarkStatus barToDock)
		{
		...
		public void Dock(HVTTMarkStatus referenceBar, HVTTMarkStatus barToDock, eDockSide dockSide)
		{
            ...
            else if (barToDock.Parent != null && barToDock.Parent != m_Container)
            {
                if (barToDock.Parent is FloatingContainer)
                {
                    ...
                    barToDock.SetBarState(HVTTBarState.Docked);
                }
                else
			...
			if(!m_LoadingLayout)
			{
                ...
                if (referenceBar == null || dockSide == eDockSide.None || referenceBar == barToDock || referenceDoc == null)
				{
                    ...
                    {
                        HVTTOrientation containerOrientation = HVTTOrientation.Horizontal; // Needed container orientation
                        if (m_Container.Dock == DockStyle.Top || m_Container.Dock == DockStyle.Bottom)
                            containerOrientation = HVTTOrientation.Vertical;
				...
				else
				{
					...
					if((parent.Orientation==HVTTOrientation.Horizontal && (dockSide==eDockSide.Left || dockSide==eDockSide.Right)) ||
						(parent.Orientation==HVTTOrientation.Vertical && (dockSide==eDockSide.Top || dockSide==eDockSide.Bottom)))
					{
					...
					else if(parent.Documents.Count==1)
					{
						...
						if(parent.Orientation==HVTTOrientation.Vertical)
							parent.Orientation=HVTTOrientation.Horizontal;
						else
							parent.Orientation=HVTTOrientation.Vertical;
					...
					else
					{
						...
						if(parent.Orientation==HVTTOrientation.Horizontal)
							newParent.Orientation=HVTTOrientation.Vertical;
						else
							newParent.Orientation=HVTTOrientation.Horizontal;
        ...
        internal void AdjustContainerSize(HVTTMarkStatus barToDock, bool visibleChanged)
        {
            ...
            if (m_Container.Dock != DockStyle.Fill)
            {
                ...
                if (m_Container.Dock == DockStyle.Left || m_Container.Dock == DockStyle.Right)
                {
                    if (m_Container.Width == 0 || doc.Parent == m_DocumentDockContainer &&
                        (m_DocumentDockContainer.Orientation == HVTTOrientation.Horizontal || visibleChanged && m_DocumentDockContainer.Documents.Count == 1))
                    {
                        int width = barToDock.GetBarDockedSize(HVTTOrientation.Vertical);
                        if (barToDock.Width > 0 && barToDock.Width<m_Container.Width) width = barToDock.Width;
                        int clientWidth = GetClientWidth();
                        
                        if (width > clientWidth)
                            width = barToDock.MinimumDockSize(HVTTOrientation.Vertical).Width * 2;
                        if (width > clientWidth)
                            width = barToDock.MinimumDockSize(HVTTOrientation.Vertical).Width;
                ...
                else if (m_Container.Dock == DockStyle.Top || m_Container.Dock == DockStyle.Bottom)
                {
                    if (m_Container.Height == 0 || doc.Parent == m_DocumentDockContainer &&
                        (m_DocumentDockContainer.Orientation == HVTTOrientation.Vertical || visibleChanged && m_DocumentDockContainer.Documents.Count == 1))
                    {
                        int height = barToDock.GetBarDockedSize(HVTTOrientation.Horizontal);
                        if (barToDock.Height > 0 && barToDock.Height<m_Container.Height) height = barToDock.Height;
                        int clientHeight = GetClientHeight();

                        if (height > clientHeight)
                            height = barToDock.MinimumDockSize(HVTTOrientation.Horizontal).Height * 2;
                        if (height > clientHeight)
                            height = barToDock.MinimumDockSize(HVTTOrientation.Horizontal).Height;
		...
		public void UnDock(HVTTMarkStatus barToUnDock)
		{
			UnDock(barToUnDock,true);
		}

		internal void UnDock(HVTTMarkStatus barToUnDock, bool removeFromContainer)
		{
            ...
            if (doc != null && m_Container != null && m_Container.Dock != DockStyle.Fill)
            {
                if (m_Container.Dock == DockStyle.Left || m_Container.Dock == DockStyle.Right)
                {
                    if (doc.Parent != null && doc.Parent == m_DocumentDockContainer && m_DocumentDockContainer.Orientation == HVTTOrientation.Horizontal)
                        containerSizeReduction = barToUnDock.Width + m_DocumentDockContainer.SplitterSize;
                }
                else if (m_Container.Dock == DockStyle.Top || m_Container.Dock == DockStyle.Bottom)
                {
                    if (doc.Parent != null && doc.Parent == m_DocumentDockContainer && m_DocumentDockContainer.Orientation == HVTTOrientation.Vertical)
		...
		private void ApplyBarStyle(HVTTMarkStatus bar)
		{
		...
		private void RestoreBarStyle(HVTTMarkStatus bar)
		{
            if (m_Container != null && m_Container.Owner != null && m_Container.Owner.ApplyDocumentBarStyle && m_Container.Dock == DockStyle.Fill)
			{
		...
		private DocumentBarContainer CreateDocumentBarContainer(HVTTMarkStatus bar)
		{
		...
		internal Cursor GetCursor(int x, int y)
		{
			Cursor c=null;
			DocumentDockContainer context=this.GetContainerFromRoot(x,y);
			if(context!=null)
			{
				if(context.Orientation==HVTTOrientation.Horizontal)
					c=Cursors.VSplit;
				else if(context.Orientation==HVTTOrientation.Vertical)
					c=Cursors.HSplit;
			}
		...
		private DocumentBaseContainer GetClosestDocument(DocumentDockContainer parent, int x, int y)
		{
			...
			HVTTOrientation orientation=parent.Orientation;
			foreach(DocumentBaseContainer doc in parent.Documents)
			{
				if(orientation==HVTTOrientation.Horizontal && doc.DisplayBounds.X>x)
                    break;
                else if(orientation==HVTTOrientation.Vertical && doc.DisplayBounds.Y>y)
					break;
				ret=doc;
			}
			return ret;
		...
		{
			if(parent==null || !parent.DisplayBounds.Contains(x,y))
				return null;
		...
		private void SplitterMouseMove(int x, int y)
		{
			...
			// Set splitter size
            if (m_ResizeDockContainer.Orientation == HVTTOrientation.Horizontal && m_ResizeDocument != null || 
            ...
            else if (m_ResizeDockContainer.Orientation == HVTTOrientation.Vertical && m_ResizeDocument != null || 
        ...
        internal Size GetContainerMinimumSize(DocumentDockContainer dockContainer)
        {
            ...
            foreach (DocumentBaseContainer d in dockContainer.Documents)
            {
                ...
                if (dockContainer.Orientation == HVTTOrientation.Horizontal)
                {
            ...
            if (minimumSize.Width > 0 && dockContainer.Orientation == HVTTOrientation.Horizontal)
                minimumSize.Width += dockContainer.SplitterSize;
            else if (minimumSize.Height > 0 && dockContainer.Orientation == HVTTOrientation.Vertical)
		...
		private Point GetResizeToPoint(int x, int y)
		{
			...
			else if(m_ResizeDockContainer.Orientation==HVTTOrientation.Horizontal)
			{
			...
			else if(m_ResizeDockContainer.Orientation==HVTTOrientation.Vertical)
			{
		...
		private void ResizeTo(int x, int y)
		{
			...
			else if(m_ResizeDockContainer.Orientation==HVTTOrientation.Horizontal && xDiff!=0)
			{
				m_ResizeDocument.SetWidth(m_ResizeDocument.DisplayBounds.Width+xDiff);
			}
            else if (m_ResizeDockContainer.Orientation == HVTTOrientation.Vertical && yDiff != 0)
			{
		...
		public void SetBarWidth(HVTTMarkStatus bar, int width)
		{
			...
			if(docBar.Parent == m_DocumentDockContainer && (m_Container.Dock == DockStyle.Left || m_Container.Dock == DockStyle.Right)
				/*&& (m_DocumentDockContainer.Orientation == HVTTOrientation.Vertical || m_DocumentDockContainer.Documents.Count == 1)*/)
			{
				m_Container.SuspendLayout();
				int diff = width;
				if(docBar.DisplayBounds.Width == 0)
					diff -= bar.GetBarDockedSize(HVTTOrientation.Vertical);
				else
            ...
            else if (docBar.Parent is DocumentDockContainer && ((DocumentDockContainer)docBar.Parent).Orientation == HVTTOrientation.Vertical)
            {
                ((DocumentDockContainer)docBar.Parent).SetWidth(width);
            }
            else
		...
		public void SetBarHeight(HVTTMarkStatus bar, int height)
		{
			DocumentBarContainer docBar=GetDocumentFromBar(bar) as DocumentBarContainer;
			if(docBar==null)
				return;
            ...
            if (m_Container!=null && (m_Container.Dock == DockStyle.Top || m_Container.Dock== DockStyle.Bottom) && 
                ...
                /*&& (m_DocumentDockContainer.Orientation == HVTTOrientation.Horizontal || m_DocumentDockContainer.Documents.Count == 1)*/)
            {
            ...
            else if (docBar.Parent is DocumentDockContainer && ((DocumentDockContainer)docBar.Parent).Orientation == HVTTOrientation.Horizontal)
            {
                ((DocumentDockContainer)docBar.Parent).SetHeight(height);
            }
            else
		...
		private void DeserializeDocumentDockContainer(ItemSerializationContext context, DocumentDockContainer instance, bool deserializeDefinition)
		{
			...
			instance.Orientation=(HVTTOrientation)XmlConvert.ToInt32(docElement.GetAttribute(DocumentSerializationXml.Orientation));
			...
			foreach(XmlElement elem in docElement.ChildNodes)
			{
				if(elem.Name==BarSerializationXml.Bar)
				{
					...
					if(deserializeDefinition || instance.Bar==null && elem.HasAttribute(BarSerializationXml.Custom) && XmlConvert.ToBoolean(elem.GetAttribute(BarSerializationXml.Custom)))
					{
						...
						instance.Bar=new HVTTMarkStatus();
