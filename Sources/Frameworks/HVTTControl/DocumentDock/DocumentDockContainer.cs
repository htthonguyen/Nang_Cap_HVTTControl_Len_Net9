...
{
	/// <summary>
	/// Represents Dock container with either horizonta or vertical layout.
	/// </summary>
	...
	public class DocumentDockContainer:DocumentBaseContainer
	{
		...
		public DocumentDockContainer(DocumentBaseContainer[] documents, HVTTOrientation orientation)
		{
		...
		/// </summary>
		public DocumentDockContainer():this(null,HVTTOrientation.Horizontal){}
        ...
        private DocumentMeasureInfo MeasureDocuments(Rectangle bounds)
        {
            ...
            int boundsSize = (m_Orientation == HVTTOrientation.Horizontal ? bounds.Width : bounds.Height);
            ...
            do
            {
                ...
                for (int i = 0; i < m_Documents.Count; i++)
                {
                    ...
                    if (m_Orientation == HVTTOrientation.Horizontal && doc.LayoutBounds.Width == 0 && doc.MinimumSize.Width > defaultSize)
                    {
                    ...
                    else if (m_Orientation == HVTTOrientation.Vertical && doc.LayoutBounds.Height == 0 && doc.MinimumSize.Height > defaultSize)
                    {
            ...
            {
                DocumentBaseContainer doc = m_Documents[i];
                if (m_Orientation == HVTTOrientation.Horizontal && doc.LayoutBounds.Width == 0)
                    doc.SetLayoutBounds(new Rectangle(0, 0, (dockSizes[i] > 0 ? dockSizes[i] : defaultSize), bounds.Height));
                else if (m_Orientation == HVTTOrientation.Vertical && doc.LayoutBounds.Height == 0)
                    doc.SetLayoutBounds(new Rectangle(0, 0, bounds.Width, (dockSizes[i] > 0 ? dockSizes[i] : defaultSize)));

                if (doc.Visible)
                {
                    totalSize += ((m_Orientation == HVTTOrientation.Horizontal ? doc.LayoutBounds.Width : doc.LayoutBounds.Height) + m_SplitterSize);
                    visibleCount++;
                }
            }
            if (visibleCount > 0)
		...
		{
			bool ret=false;
			if(m_Orientation!=HVTTOrientation.Horizontal || width<doc.MinimumSize.Width && doc.MinimumSize.Width>0)
				return ret;
		...
		{
			bool ret=false;
			if(m_Orientation!=HVTTOrientation.Vertical || height<doc.MinimumSize.Height && doc.MinimumSize.Height>0)
				return ret;
		...
		public DocumentBarContainer GetBarDocumentContainer(HVTTMarkStatus bar)
		{
			foreach(DocumentBaseContainer doc in m_Documents)
			{
