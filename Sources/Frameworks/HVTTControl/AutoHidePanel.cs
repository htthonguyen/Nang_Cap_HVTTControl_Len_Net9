...
namespace HVTT.UI.Window.Forms
{
	...
	public class AutoHidePanel : System.Windows.Forms.Control
	{
		...
		protected override void OnPaint(PaintEventArgs p)
		{
            ...
            if (m_Style == HVTTControlStyle.Office2007 && m_ColorScheme == null && Rendering.GlobalManager.Renderer is Rendering.Office2007Renderer)
			...
			if(IsGradientStyle)
			{
                DisplayHelp.FillRectangle(g, this.ClientRectangle, scheme.BarBackground, scheme.BarBackground2, scheme.BarBackgroundGradientAngle);
			}
			else
			{
            ...
            if (m_Style == HVTTControlStyle.VS2005 || office2007Style)
            {
			...
			Pen linePen=null;
			if(m_Style==HVTTControlStyle.Office2003)
			{
                textColor = scheme.ItemText;
				linePen=new Pen(scheme.ItemHotBorder,1);
			}
            else if (m_Style == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(m_Style))
			{
                textColor = scheme.ItemText;
					...
					for(int i=0;i<panel.Tabs.Count;i++)
					{
						...
						if(panel.ActiveTab==i || panel.BoundBar.AutoHideTabTextAlwaysVisible)
						{
							if(icon!=null)
							{
								...
								{
									if((m_Style==HVTTControlStyle.Office2003 || m_Style==HVTTControlStyle.VS2005) && !scheme.ItemPressedBackground2.IsEmpty)
									{
        ...
        private bool IsGradientStyle
        {
            get
            {
                if (m_Style == HVTTControlStyle.Office2003 || m_Style == HVTTControlStyle.VS2005 || BarFunctions.IsOffice2007Style(m_Style))
                    return true;
                return false;
            }
        }
			...
			else
			{
				...
				if(activePanel!=null && !activePanel.BoundBar.IsSizingWindow)
				{
					if(activePanel.BoundBar.Visible && !(!m_EnableFocusCollapse && IsBarActive(activePanel.BoundBar)))
					{
						...
						{
                            AnimateHide(activePanel.BoundBar);
							DestroyTimer();
						}
					}
				}
		...
		private bool IsBarActive(HVTTMarkStatus bar)
		{
			if(bar==null)
				return false;
		...
		internal void ShowBar(HVTTMarkStatus bar)
		{
			PanelBar panel=null;
			foreach(PanelBar panelbar in m_Panels)
			{
		...
		internal void HideBar(HVTTMarkStatus bar)
		{
			PanelBar activePanel=ActivePanelBar;
		...
		private void AnimateHide(HVTTMarkStatus bar)
		{
			bar.AnimateHide();
		}

		private void AnimateShow(HVTTMarkStatus bar)
		{
			if(bar.Enabled)
				bar.AnimateShow();
		}
		...
		public void SetBarPosition(HVTTMarkStatus bar, int iIndex)
		{
		...
		/// automatically created as specified by Style property. Note that if your HVTTManager has UseGlobalColorScheme set to true
		/// ColorScheme from HVTTManager will be used.
		/// </summary>
		...
		public void AddBar(HVTTMarkStatus bar)
		{
			if(bar.Style==HVTTControlStyle.Office2003)
				m_Style=HVTTControlStyle.Office2003;
			else if(bar.Style==HVTTControlStyle.VS2005)
				m_Style=HVTTControlStyle.VS2005;
            else if (bar.Style == HVTTControlStyle.Office2007)
                m_Style = HVTTControlStyle.Office2007;
			else
		...
		public void RefreshBar(HVTTMarkStatus bar)
		{
			foreach(PanelBar panel in m_Panels)
			{
				if(panel.BoundBar==bar)
		...
		private class PanelBar:IDisposable
		{
			...
			public HVTTMarkStatus BoundBar=null;
			...
			public PanelBar(AutoHidePanel parent, HVTTMarkStatus bar)
			{
				m_Parent=parent;
				BoundBar=bar;
				ReloadDockItems();
			...
			public void ReloadDockItems()
			{
				...
				try
				{
					if(BoundBar.LayoutType==HVTTLayoutType.DockContainer)
					{
		...
		private class VS2005TabDisplay
		{
			...
			private TabColors GetTabColors(DockItemTab tab, bool selected)
			{
                ...
                if (tab.Item != null && tab.Item.Style == HVTTControlStyle.Office2007)
                {
			...
			protected virtual void DrawTabText(DockItemTab tab, TabColors colors, Graphics g, bool selected)
			{
                ...
                if (tab.Item != null && tab.Item.ContainerControl is HVTTMarkStatus)
                    drawTextAlways = ((HVTTMarkStatus)tab.Item.ContainerControl).AutoHideTabTextAlwaysVisible;
